...
export function VendorRegistrationForm() {
  ...
  const validateField = (field: string, value: string): string | undefined => {
    switch (field) {
      case "business_name":
        ...
        if (value.length > MAX_NAME_LENGTH) return `Business name must be less than ${MAX_NAME_LENGTH} characters`;
        break;
      case "email":
      ...
      case "tax_id":
        ...
        break;
      case "description":
        if (value && value.length > MAX_DESCRIPTION_LENGTH) return `Description must be less than ${MAX_DESCRIPTION_LENGTH} characters`;
        break;
      case "address_line1":
      ...
      case "postal_code":
        ...
        if (!isValidPostalCode(value, formData.country_code)) return "Please enter a valid postal code";
        break;
  ...
  const handleFieldChange = (field: keyof VendorRegistrationData, value: string) => {
  ...
  const validateStep3 = (): boolean => {
    ...
    const nameError = validateField("contact_person_name", formData.contact_person_name);
    if (nameError) newErrors.contact_person_name = nameError;
    
    const emailError = validateField("contact_person_email", formData.contact_person_email);
    if (emailError) newErrors.contact_person_email = emailError;
    
    const phoneError = validateField("contact_person_phone", formData.contact_person_phone || "");
  ...
  const registerMutation = useMutation({
    ...
    onError: (err: Error) => {
      addToast("error", err.message || "Failed to submit application. Please try again.");
    },
  });
