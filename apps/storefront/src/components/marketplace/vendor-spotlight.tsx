...
export function VendorSpotlight({ locale: localeProp, vendor }: VendorSpotlightProps) {
          ...
          <div className="pt-8">
            ...
            <div className="flex items-center gap-2 mt-1 text-sm text-ds-muted-foreground">
              <span className="flex items-center gap-1">
                <svg className="h-3.5 w-3.5 text-ds-warning" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                {vendor.rating.toFixed(1)}
              </span>
              <span>·</span>
              <span>{vendor.reviewCount} {t(locale, "marketplace.reviews_count")}</span>
            </div>
          </div>
        </div>

        {vendor.description && (
          <p className="text-sm text-ds-muted-foreground mt-3 line-clamp-2">{vendor.description}</p>
        )}
            ...
            <div className="grid grid-cols-3 gap-2">
              {vendor.topProducts.slice(0, 3).map((product) => (
                <div key={product.id} className="rounded-lg border border-ds-border overflow-hidden bg-ds-background">
                  ...
                  <div className="p-2">
                    <p className="text-xs text-ds-foreground line-clamp-1">{product.name}</p>
