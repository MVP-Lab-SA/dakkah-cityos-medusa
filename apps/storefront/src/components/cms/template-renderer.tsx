...
function VerticalListTemplate({ page, tenant, locale }: { page: CMSPage; tenant: { id: string; slug: string }; locale: string }) {
  ...
  const gradientClass = VERTICAL_COLORS[verticalSlug] || "from-ds-primary to-ds-primary"
  ...
  const filteredItems = (() => {
    ...
    if (sortBy === "name") {
      result = [...result].sort((a: any, b: any) => (a.name || a.title || "").localeCompare(b.name || b.title || ""))
  ...
  return (
    <div className="min-h-screen bg-ds-muted">
      <section className={`bg-gradient-to-r ${gradientClass} text-white py-16 relative overflow-hidden`}>
        <div className="absolute inset-0 bg-black/20" />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <nav className="flex items-center gap-2 text-sm mb-4 text-white/70">
            <a href={`/${tenant.slug}/${locale}`} className="hover:text-white transition-colors">Home</a>
            <span>/</span>
            <span className="text-white">{page.title}</span>
          </nav>
          <h1 className="text-4xl font-bold mb-3">{page.title}</h1>
          {page.seo?.description && (
            <p className="text-white/80 text-lg max-w-2xl">{page.seo.description}</p>
          )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="relative flex-1 max-w-md">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ds-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-ds-border rounded-xl bg-ds-background focus:outline-none focus:ring-2 focus:ring-ds-ring text-sm"
            />
          </div>
          <select
            value={sortBy}
        ...
        {isLoading ? (
          <div className={`grid gap-6 ${config?.cardLayout === "list" ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3"}`}>
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-ds-background rounded-xl border border-ds-border overflow-hidden animate-pulse">
                <div className={`aspect-[4/3] bg-gradient-to-br ${gradientClass} opacity-20`} />
          ...
          <div className="bg-ds-background rounded-xl border border-ds-border p-16 text-center">
            <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${gradientClass} flex items-center justify-center mx-auto mb-6 opacity-80`}>
              <svg className="w-10 h-10 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d={VERTICAL_ICONS[verticalSlug] || "M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"} />
              </svg>
            </div>
            <h3 className="text-xl font-semibold text-ds-foreground mb-2">
              {searchQuery ? "No results found" : `No ${page.title?.toLowerCase() || "items"} yet`}
            </h3>
...
const VERTICAL_ICONS: Record<string, string> = {
  restaurants: "M11 3a1 1 0 10-2 0v6.586l-1.293-1.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 9.586V3z",
  "real-estate": "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1",
  automotive: "M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z",
  grocery: "M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17",
  ...
  charity: "M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z",
...
function VerticalCard({ item, verticalSlug, tenant, locale, cardLayout }: {
  ...
  const price = item.price || item.rate || item.cost || item.consultation_fee || item.hourly_rate || item.monthly_price || null
  const rating = item.rating || item.stars || item.avg_rating || null
  const category = item.category || item.type || item.specialty || item.cuisine_types?.[0] || item.practice_area || item.service_type || null
  ...
  const gradientClass = VERTICAL_COLORS[verticalSlug] || "from-ds-primary to-ds-primary"
  const iconPath = VERTICAL_ICONS[verticalSlug] || "M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"
    ...
    return (
      <a href={detailUrl} className="group flex bg-ds-background rounded-xl shadow-sm border border-ds-border overflow-hidden hover:shadow-lg hover:border-ds-ring transition-all duration-200">
        ...
        <div className="p-4 flex-1 min-w-0">
          <h3 className="font-semibold text-lg text-ds-foreground group-hover:text-ds-primary transition-colors truncate">{title}</h3>
          <div className="flex items-center gap-2 mt-1">
            {category && <span className="text-xs bg-ds-muted text-ds-muted-foreground px-2 py-0.5 rounded-full">{category}</span>}
            {location && <span className="text-xs text-ds-muted-foreground flex items-center gap-1"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>{location}</span>}
          </div>
  ...
  return (
    <a href={detailUrl} className="group bg-ds-background rounded-xl shadow-sm border border-ds-border overflow-hidden hover:shadow-lg hover:border-ds-ring transition-all duration-200 flex flex-col">
      <div className="aspect-[4/3] relative overflow-hidden">
        ...
        {category && (
          <span className="absolute top-3 left-3 text-xs bg-white/90 backdrop-blur-sm text-ds-foreground px-2.5 py-1 rounded-full font-medium shadow-sm">
            {category}
          </span>
        )}
      </div>
      <div className="p-4 flex-1 flex flex-col">
        <h3 className="font-semibold text-base text-ds-foreground group-hover:text-ds-primary transition-colors line-clamp-1">{title}</h3>
        {location && (
          <p className="text-xs text-ds-muted-foreground mt-1 flex items-center gap-1">
            <svg className="w-3 h-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /></svg>
            {location}
          </p>
        )}
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-ds-border">
          ...
          {price != null ? (
            <span className="text-ds-foreground font-bold text-base">{typeof price === "number" ? `$${price.toFixed(2)}` : price}</span>
          ) : (
            <span className="text-xs text-ds-muted-foreground">View details</span>
          )}
        </div>
      </div>
    </a>
