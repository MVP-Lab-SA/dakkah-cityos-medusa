...
export function DigitalProductCard({ product }: DigitalProductCardProps) {
  ...
  return (
    <Link
      to={`${prefix}/digital/${product.id}` as any}
    ...
    >
      <div className="aspect-[4/3] bg-ds-muted relative overflow-hidden">
        ...
        <div className="absolute top-2 start-2">
          <span className={`inline-block px-2 py-1 text-xs font-medium rounded ${fileTypeColors[product.file_type] || "bg-ds-muted text-ds-muted-foreground"}`}>
      ...
      <div className="p-4">
        ...
        {product.rating && (
          <div className="flex items-center gap-1 mt-2">
            ...
            <span className="text-sm text-ds-muted-foreground">({product.rating.count})</span>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-ds-border">
          <span className="text-lg font-bold text-ds-foreground">
            {formatCurrency(product.price, product.currency_code, locale as any)}
          </span>
