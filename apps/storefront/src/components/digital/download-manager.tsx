...
function DownloadRow({ item, locale }: { item: DownloadItem; locale: string }) {
  ...
  const noDownloadsLeft = item.downloads_remaining !== undefined && item.downloads_remaining <= 0
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-1 text-sm text-ds-muted-foreground">
            ...
            <span className="text-ds-border">·</span>
            <span>Purchased {formatDate(item.purchase_date, locale as any)}</span>
          </div>

          <div className="flex flex-wrap items-center gap-4 mt-2">
            {item.downloads_remaining !== undefined && (
              <span className={`text-xs font-medium px-2 py-1 rounded ${noDownloadsLeft ? "bg-ds-destructive/10 text-ds-destructive" : "bg-ds-muted text-ds-muted-foreground"}`}>
                {item.downloads_remaining} downloads remaining
              </span>
            )}
            {item.expires_at && (
              <span className={`text-xs font-medium px-2 py-1 rounded ${isExpired ? "bg-ds-destructive/10 text-ds-destructive" : "bg-ds-muted text-ds-muted-foreground"}`}>
                {isExpired ? "Expired" : `Expires ${formatDate(item.expires_at, locale as any)}`}
              </span>
            )}
          </div>
