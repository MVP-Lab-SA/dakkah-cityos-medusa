...
export function WishlistGrid({ items, onRemove, onMoveToCart, loading }: WishlistGridProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {items.map((item) => (
        <div key={item.id} className="bg-ds-background rounded-lg border border-ds-border overflow-hidden group">
          <Link to={`${prefix}/products/${item.product_id}` as any}>
            <div className="aspect-square bg-ds-muted relative overflow-hidden">
              ...
              {onRemove && (
                ...
                >
                  <svg className="w-4 h-4 text-ds-muted-foreground hover:text-ds-destructive" fill="none" viewBox="0 0 24 24" stroke="currentColor">
