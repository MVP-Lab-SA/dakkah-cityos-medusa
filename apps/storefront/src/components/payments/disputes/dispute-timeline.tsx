...
const eventColors: Record<string, string> = {
  open: "bg-ds-warning text-white",
  under_review: "bg-ds-accent text-white",
  resolved: "bg-ds-success text-white",
  rejected: "bg-ds-destructive text-white",
  escalated: "bg-ds-destructive text-white",
...
export function DisputeTimeline({
  ...
  if (!events.length) {
    return (
      <div className="bg-ds-background rounded-lg border border-ds-border p-8 text-center">
        <p className="text-ds-muted-foreground">{t(locale, "disputes.no_events")}</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      <h4 className="text-sm font-semibold text-ds-foreground">{t(locale, "disputes.timeline")}</h4>
