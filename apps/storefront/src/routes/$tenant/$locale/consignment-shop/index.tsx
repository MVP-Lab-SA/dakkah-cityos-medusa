...
const categoryOptions = ["all", "clothing", "accessories", "electronics", "furniture", "jewelry", "art", "collectibles", "other"] as const
const conditionOptions = ["all", "new", "like-new", "excellent", "good", "fair"] as const
const priceRangeOptions = ["all", "0-25", "25-100", "100-500", "500-1000", "1000+"] as const
...
function ConsignmentShopPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["consignments", categoryFilter, conditionFilter, priceRange, page],
    queryFn: async () => {
      ...
      <div className="bg-ds-card border-b border-ds-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center gap-2 text-sm text-ds-muted-foreground mb-4">
            ...
            <span>/</span>
            <span className="text-ds-foreground">Consignment Shop</span>
          </div>
          <h1 className="text-3xl font-bold text-ds-foreground">Consignment Shop</h1>
          <p className="mt-2 text-ds-muted-foreground">Quality pre-owned items at great prices</p>
        </div>
      </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {conditionOptions.map((opt) => (
                    ...
                    >
                      {opt === "all" ? "Any Condition" : opt.charAt(0).toUpperCase() + opt.slice(1).replace("-", " ")}
                    </button>
                  ))}
                </div>
              </div>
          ...
          <main className="flex-1">
            {error ? (
              <div className="bg-ds-destructive/10 border border-ds-destructive/20 rounded-xl p-8 text-center">
                ...
                <p className="text-ds-destructive font-medium">Something went wrong loading consignment items.</p>
              </div>
            ) : isLoading ? (
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                  {filteredItems.map((item: any) => {
                    ...
                    const savingsPercent = item.original_price && savings > 0 ? Math.round((savings / item.original_price) * 100) : 0
                    return (
                      <a
                        key={item.id}
                        href={`${prefix}/consignment-shop/${item.id}`}
                      ...
                      >
                        <div className="aspect-[4/3] bg-ds-muted relative overflow-hidden">
                          ...
                          {item.condition && (
                            <span className="absolute top-2 left-2 px-2 py-1 text-xs font-medium bg-ds-primary text-ds-primary-foreground rounded-md capitalize">{item.condition.replace("-", " ")}</span>
                          )}
                          {savingsPercent > 0 && (
                            <span className="absolute top-2 right-2 px-2 py-1 text-xs font-bold bg-green-500 text-white rounded-md">Save {savingsPercent}%</span>
                          )}
                        </div>
                        <div className="p-4">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <span className="text-lg font-bold text-ds-primary">${Number(item.price || 0).toLocaleString()}</span>
                            {item.original_price && item.original_price > (item.price || 0) && (
                              <span className="text-sm text-ds-muted-foreground line-through">${Number(item.original_price).toLocaleString()}</span>
                            )}
                          </div>
                          {savings > 0 && (
                          ...
                          <div className="flex items-center justify-between mt-3 pt-3 border-t border-ds-border">
                            ...
                            <span className="text-xs text-ds-muted-foreground capitalize">{item.condition?.replace("-", " ") || ""}</span>
                          </div>
                        </div>
                      </a>
                    )
