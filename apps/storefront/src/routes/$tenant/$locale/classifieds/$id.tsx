...
export const Route = createFileRoute("/$tenant/$locale/classifieds/$id")({
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(`${baseUrl}/store/classifieds/${params.id}`, {
      ...
      <div className="min-h-screen bg-ds-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">This classified listing may have been removed or is no longer available.</p>
            <Link to={`${prefix}/classifieds` as any} className="inline-flex items-center px-4 py-2 text-sm font-medium bg-ds-primary text-ds-primary-foreground rounded-lg hover:bg-ds-primary/90 transition-colors">
              Browse Classifieds
            </Link>
          </div>
        </div>
      ...
      <div className="bg-ds-card border-b border-ds-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-sm text-ds-muted-foreground">
            ...
            <span>/</span>
            <Link to={`${prefix}/classifieds` as any} className="hover:text-ds-foreground transition-colors">Classifieds</Link>
            <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              {item.thumbnail || item.image ? (
                <img loading="lazy" src={item.thumbnail || item.image} alt={item.title} className="w-full h-full object-cover" />
              ) : (
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-ds-foreground">{item.title}</h1>
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              ...
              {item.seller && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  <h3 className="font-semibold text-ds-foreground mb-3">Seller Information</h3>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      <p className="font-medium text-ds-foreground">{item.seller.name}</p>
                      {item.seller.rating && (
                  ...
                  {item.seller.joined_at && (
                    <p className="text-xs text-ds-muted-foreground">Member since {new Date(item.seller.joined_at).toLocaleDateString()}</p>
                  )}
                </div>
              )}

              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h3 className="font-semibold text-ds-foreground mb-3">Safety Tips</h3>
