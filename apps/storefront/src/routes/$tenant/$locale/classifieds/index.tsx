...
export const Route = createFileRoute("/$tenant/$locale/classifieds/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse classified listings on Dakkah CityOS" },
    ],
  }),
  loader: async () => {
    try {
    ...
    <div className="min-h-screen bg-ds-background">
      <div className="bg-gradient-to-r from-ds-warning to-ds-warning text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">{t(locale, 'classifieds.title')}</h1>
                ...
                {filteredItems.map((item: any) => (
                  ...
                  >
                    <div className="aspect-[4/3] bg-gradient-to-br from-ds-warning/10 to-ds-warning/15 relative overflow-hidden">
                      ...
                      {item.condition && (
                        <span className={`absolute top-2 start-2 px-2 py-1 text-xs font-medium rounded-md ${conditionColor(item.condition)}`}>{conditionLabel(item.condition)}</span>
                      )}
                      {item.listing_type && (
                        <span className="absolute top-2 end-2 px-2 py-1 text-xs font-medium bg-ds-card/90 text-ds-foreground/80 rounded-md">{listingTypeLabel(item.listing_type)}</span>
                      )}
