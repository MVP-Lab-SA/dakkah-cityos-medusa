...
export const Route = createFileRoute("/$tenant/$locale/warranties/$id")({
  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(`${baseUrl}/store/warranties/${params.id}`, {
      ...
      <div className="min-h-screen bg-ds-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-ds-foreground mb-2">Warranty Not Found</h2>
            <p className="text-ds-muted-foreground mb-6">This warranty plan may have been removed or is no longer available.</p>
            <Link to={`${prefix}/warranties` as any} className="inline-flex items-center px-4 py-2 text-sm font-medium bg-ds-primary text-ds-primary-foreground rounded-lg hover:bg-ds-primary/90 transition-colors">
              Browse Warranties
            </Link>
          </div>
        </div>
      ...
      <div className="bg-ds-card border-b border-ds-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-sm text-ds-muted-foreground">
            ...
            <span>/</span>
            <Link to={`${prefix}/warranties` as any} className="hover:text-ds-foreground transition-colors">Warranties</Link>
            <span>/</span>
            <span className="text-ds-foreground truncate">{warranty.name || warranty.title}</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-ds-background border border-ds-border rounded-xl p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h1 className="text-2xl sm:text-3xl font-bold text-ds-foreground">{warranty.name || warranty.title}</h1>
                  <div className="flex flex-wrap items-center gap-3 mt-2">
                    {warranty.coverage_period && (
                      <span className="px-3 py-1 text-xs font-semibold rounded-full bg-ds-primary/10 text-ds-primary">{warranty.coverage_period}</span>
                    )}
                    {warranty.type && (
                      <span className="px-3 py-1 text-xs font-medium rounded-full bg-ds-muted text-ds-muted-foreground">{warranty.type}</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              {warranty.coverage_period && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-4 text-center">
                  <p className="text-xs text-ds-muted-foreground mb-1">Coverage Period</p>
                  <p className="text-lg font-bold text-ds-foreground">{warranty.coverage_period}</p>
                </div>
              )}
            ...
            {warranty.description && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-3">Plan Details</h2>
                <p className="text-ds-muted-foreground text-sm leading-relaxed whitespace-pre-wrap">{warranty.description}</p>
              </div>
            )}

            {warranty.covered_items && warranty.covered_items.length > 0 && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-4">What's Covered</h2>
                <div className="space-y-2">
                  {warranty.covered_items.map((item: any, idx: number) => (
                    <div key={idx} className="flex items-start gap-3 p-3 bg-ds-success/5 rounded-lg">
                      <svg className="w-5 h-5 text-ds-success flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                      <div>
                        <p className="text-sm text-ds-foreground">{typeof item === "string" ? item : item.name}</p>
                        {item.description && <p className="text-xs text-ds-muted-foreground mt-0.5">{item.description}</p>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {warranty.excluded_items && warranty.excluded_items.length > 0 && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-4">What's Not Covered</h2>
                <div className="space-y-2">
                  {warranty.excluded_items.map((item: any, idx: number) => (
                    <div key={idx} className="flex items-start gap-3 p-3 bg-ds-destructive/5 rounded-lg">
                      <svg className="w-5 h-5 text-ds-destructive flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                      <p className="text-sm text-ds-foreground">{typeof item === "string" ? item : item.name}</p>
                    </div>
                  ))}
                </div>
            ...
            {warranty.claims_process && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-4">Claims Process</h2>
                {Array.isArray(warranty.claims_process) ? (
                  <div className="space-y-4">
                    {warranty.claims_process.map((step: any, idx: number) => (
                      <div key={idx} className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="font-medium text-ds-foreground text-sm">{typeof step === "string" ? step : step.title || step.name}</p>
                  ...
                  <p className="text-ds-muted-foreground text-sm leading-relaxed whitespace-pre-wrap">{warranty.claims_process}</p>
                )}
              </div>
            )}
            ...
            {warranty.terms && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <p className="text-ds-muted-foreground text-sm leading-relaxed whitespace-pre-wrap">{warranty.terms}</p>
              </div>
            )}
                ...
                <div className="space-y-4">
                  {warranty.faq.map((item: any, idx: number) => (
                    <div key={idx} className="pb-4 border-b border-ds-border last:border-0">
                      <p className="font-medium text-ds-foreground text-sm">{typeof item === "string" ? item : item.question}</p>
                      {item.answer && <p className="text-sm text-ds-muted-foreground mt-1">{item.answer}</p>}
                    </div>
                  ))}
                </div>
              </div>
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              ...
              {warranty.highlights && warranty.highlights.length > 0 && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  <h3 className="font-semibold text-ds-foreground mb-3">Plan Highlights</h3>
                  <div className="space-y-2">
              ...
              {warranty.contact && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  <h3 className="font-semibold text-ds-foreground mb-3">Claims Support</h3>
