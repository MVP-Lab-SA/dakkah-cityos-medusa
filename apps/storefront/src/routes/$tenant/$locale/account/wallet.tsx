...
import { useWalletBalance, useWalletTransactions } from "@/lib/hooks/use-payments"
...
function WalletPage() {
  ...
  return <WalletPageClient locale={locale} filter={filter} setFilter={setFilter} page={page} setPage={setPage} />
}
...
function WalletPageClient({
  ...
  const filteredTransactions = txData?.transactions?.filter((tx) => {
    ...
    if (filter === "credit") return tx.type === "credit" || tx.type === "refund" || tx.type === "top-up"
  ...
  return (
    <AccountLayout title={t(locale, "payment.wallet")} description={t(locale, "payment.wallet_description")}>
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-ds-foreground">{t(locale, "payment.transactions")}</h2>
          </div>
