...
export const Route = createFileRoute("/$tenant/$locale/healthcare/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse healthcare services on Dakkah CityOS" },
    ],
  }),
  loader: async () => {
    try {
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-72 flex-shrink-0">
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">{t(locale, 'verticals.specialization_label')}</label>
                <div className="space-y-1">
                  {specOptions.map((opt) => (
                    ...
                    >
                      {opt === "all" ? t(locale, 'verticals.all_specializations') : `${specIcon(opt)} ${opt.charAt(0).toUpperCase() + opt.slice(1)}`}
                    </button>
                  ))}
                </div>
              </div>
