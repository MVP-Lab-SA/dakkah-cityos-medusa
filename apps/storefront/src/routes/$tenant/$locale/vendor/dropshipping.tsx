...
function VendorDropshippingRoute() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return sdk.client.fetch<{ items: DropshippingProduct[]; count: number }>(url, {
        credentials: "include",
      })
    },
  })
    ...
    <div className="container mx-auto py-12">
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 text-gray-500">
          ...
          <p className="text-sm">Add products from suppliers to start dropshipping.</p>
        </div>
      ) : (
            ...
            <tbody>
              {items.map((product) => (
                <tr key={product.id} className="border-b hover:bg-gray-50 transition">
                  ...
                  <td className="py-4 pr-4 text-sm text-gray-500">{product.supplier}</td>
