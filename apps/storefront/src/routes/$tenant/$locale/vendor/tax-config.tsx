...
export const Route = createFileRoute("/$tenant/$locale/vendor/tax-config")({
  component: VendorTaxConfigRoute,
})

function VendorTaxConfigRoute() {
  const auth = useAuth()
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["vendor-tax-config", statusFilter],
    queryFn: async () => {
      const params = new URLSearchParams()
      if (statusFilter) params.set("status", statusFilter)
      const url = `/vendor/tax-config${params.toString() ? `?${params}` : ""}`
      return sdk.client.fetch<{ items: TaxRule[]; count: number }>(url, {
        credentials: "include",
      })
    },
  })
