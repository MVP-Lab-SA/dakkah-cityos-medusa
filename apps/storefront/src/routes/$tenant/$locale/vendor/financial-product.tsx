...
export const Route = createFileRoute("/$tenant/$locale/vendor/financial-product")({
...
function VendorFinancialProductRoute() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return sdk.client.fetch<{ items: FinancialProduct[]; count: number }>(url, {
        credentials: "include",
      })
    },
  })
  ...
  return (
    <div className="container mx-auto py-12">
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["", "active", "draft", "under_review", "suspended", "archived"].map((s) => (
          <button
            key={s}
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-ds-muted-foreground">
                    ...
                    {product.max_amount !== undefined && (
                      <span>Max: {product.currency_code?.toUpperCase()} {(product.max_amount / 100).toFixed(2)}</span>
                    )}
