...
function VendorWhiteLabelRoute() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return sdk.client.fetch<{ items: WhiteLabelProduct[]; count: number }>(url, {
        credentials: "include",
      })
    },
  })
    ...
    <div className="container mx-auto py-12">
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 text-gray-500">
          ...
          <p className="text-sm">Add products and customize branding to get started.</p>
        </div>
      ) : (
            ...
            <tbody>
              {items.map((product) => (
                <tr key={product.id} className="border-b hover:bg-gray-50 transition">
                  ...
                  <td className="py-4 pr-4 text-sm text-gray-500">{product.category}</td>
                  ...
                  <td className="py-4">
                    <button className="text-sm text-blue-600 hover:underline">Edit Branding</button>
                  </td>
                </tr>
              ))}
            </tbody>
