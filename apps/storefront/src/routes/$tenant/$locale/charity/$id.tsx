          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              {campaign.thumbnail || campaign.image ? (
                <img src={campaign.thumbnail || campaign.image} alt={campaign.name || campaign.title} className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-6">
              <div className="flex items-end justify-between mb-2">
                <div>
                  <span className="text-2xl font-bold text-ds-primary">${raised.toLocaleString()}</span>
                  <span className="text-sm text-ds-muted-foreground ml-1">raised of ${goal.toLocaleString()} goal</span>
                </div>
                ...
                <div className="space-y-3">
                  {campaign.donors.map((donor: any, idx: number) => (
                    <div key={idx} className="flex items-center justify-between py-2 border-b border-ds-border last:border-0">
                      ...
                      {donor.amount != null && (
                        <span className="font-medium text-ds-primary text-sm">${Number(donor.amount).toLocaleString()}</span>
                      )}
                    </div>
                  ))}
                </div>
