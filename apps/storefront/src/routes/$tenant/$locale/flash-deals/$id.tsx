...
export const Route = createFileRoute("/$tenant/$locale/flash-deals/$id")({
  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(`${baseUrl}/store/flash-sales/${params.id}`, {
...
function FlashDealDetailPage() {
      ...
      <div className="min-h-screen bg-ds-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-ds-foreground mb-2">Flash Deal Not Found</h2>
            <p className="text-ds-muted-foreground mb-6">This flash deal may have ended or is no longer available.</p>
            <Link to={`${prefix}/flash-deals` as any} className="inline-flex items-center px-4 py-2 text-sm font-medium bg-ds-primary text-ds-primary-foreground rounded-lg hover:bg-ds-primary/90 transition-colors">
              Browse Flash Deals
            </Link>
          </div>
        </div>
  ...
  const originalPrice = deal.original_price || deal.originalPrice || deal.compare_at_price
  const salePrice = deal.sale_price || deal.salePrice || deal.price
  const discount = deal.discount || deal.discount_percentage || (originalPrice && salePrice ? Math.round((1 - salePrice / originalPrice) * 100) : null)
      ...
      <div className="bg-ds-card border-b border-ds-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-sm text-ds-muted-foreground">
            ...
            <span>/</span>
            <Link to={`${prefix}/flash-deals` as any} className="hover:text-ds-foreground transition-colors">Flash Deals</Link>
            <span>/</span>
            <span className="text-ds-foreground truncate">{deal.name || deal.title}</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              {deal.thumbnail || deal.image ? (
                <img loading="lazy" src={deal.thumbnail || deal.image} alt={deal.name || deal.title} className="w-full h-full object-cover" />
              ) : (
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-ds-foreground">{deal.name || deal.title}</h1>
            ...
            {deal.description && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <p className="text-ds-muted-foreground text-sm leading-relaxed whitespace-pre-wrap">{deal.description}</p>
              </div>
            )}

            {(deal.metadata || deal.details) && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-3">Details</h2>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {Object.entries(deal.metadata || deal.details || {}).map(([key, value]) => (
                    <div key={key}>
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                ...
                {timeLeft && (
                  <div className="bg-ds-destructive/10 rounded-lg p-3 text-center">
                    <p className="text-xs text-ds-muted-foreground mb-1">Time Remaining</p>
                    <p className="text-lg font-bold text-ds-destructive">{timeLeft}</p>
                  </div>
                )}
              ...
              {(deal.quantity_available || deal.quantityAvailable || deal.stock) && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  <h3 className="font-semibold text-ds-foreground mb-3">Availability</h3>
                  <div className="text-sm text-ds-muted-foreground">
                    <p className="font-medium text-ds-foreground">{deal.quantity_available || deal.quantityAvailable || deal.stock} left</p>
