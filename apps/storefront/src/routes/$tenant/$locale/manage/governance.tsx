...
const config = crudConfigs["governance"]
...
function ManageGovernancePage() {
  ...
  const allItems = ((data as any)?.governance || (data as any)?.policies || (data as any)?.items || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage governance policies"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No governance policies found" countLabel="policies" />
      </Container>
      <FormDrawer
        open={drawerOpen}
