...
export const Route = createFileRoute("/$tenant/$locale/manage/inventory-extension")({
  component: ManageInventoryExtensionPage,
})

const config = crudConfigs["inventory-extension"]

const STATUS_FILTERS = ["all", "active", "reserved", "depleted", "pending"] as const

function ManageInventoryExtensionPage() {
  ...
  const allItems = ((data as any)?.items || (data as any)?.inventory || []).map((item: any) => ({
    id: item.id,
    sku: item.sku || item.variant_id || "—",
    product: item.product || item.product_name || "—",
    warehouse: item.warehouse || item.location || "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage extended inventory records"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No inventory extensions found" countLabel="records" />
      </Container>
