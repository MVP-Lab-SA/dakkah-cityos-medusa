...
export const Route = createFileRoute("/$tenant/$locale/manage/volume-pricing")({
  component: ManageVolumePricingPage,
})

const config = crudConfigs["volume-pricing"]
const STATUS_FILTERS = ["all", "active", "inactive"] as const

function ManageVolumePricingPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["manage", "volume-pricing"],
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/volume-pricing", { method: "GET" })
      return response
    },
    enabled: typeof window !== "undefined",
  })

  const allItems = ((data as any)?.["volume-pricing"] || (data as any)?.tiers || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage volume pricing"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No volume pricing tiers found" countLabel="pricing tiers" />
      </Container>
      <FormDrawer
        open={drawerOpen}
