...
export const Route = createFileRoute("/$tenant/$locale/manage/tenants-admin")({
  component: ManageTenantsPage,
})

const config = crudConfigs["tenants"]
...
function ManageTenantsPage() {
  ...
  const allItems = ((data as any)?.tenants || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No tenants found" countLabel="tenants" />
      </Container>
      <FormDrawer
        open={drawerOpen}
