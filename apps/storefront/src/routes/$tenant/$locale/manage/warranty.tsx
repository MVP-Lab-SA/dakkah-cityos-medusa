...
const config = crudConfigs["warranty"]

const STATUS_FILTERS = ["all", "active", "expired", "pending", "cancelled"] as const
...
function ManageWarrantyPage() {
  ...
  const allItems = ((data as any)?.items || (data as any)?.warranties || []).map((item: any) => ({
    id: item.id,
    product: item.product || item.name || "—",
    type: item.plan_type || item.type || "—",
    duration: item.duration_months ? `${item.duration_months} months` : "—",
    coverage: item.coverage ? (typeof item.coverage === "string" ? item.coverage : "Full") : "—",
    status: item.is_active === false ? "expired" : (item.status || "active"),
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage warranty plans and claims"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No warranties found" countLabel="warranties" />
      </Container>
