...
export const Route = createFileRoute("/$tenant/$locale/manage/white-label")({
  component: ManageWhiteLabelPage,
})

const config = crudConfigs["white-label"]

const STATUS_FILTERS = ["all", "active", "pending", "discontinued"] as const

function ManageWhiteLabelPage() {
  ...
  const allItems = ((data as any)?.products || (data as any)?.white_label || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage white-label products"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No white-label products found" countLabel="products" />
      </Container>
