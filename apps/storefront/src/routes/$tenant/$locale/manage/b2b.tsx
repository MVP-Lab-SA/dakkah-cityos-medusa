...
export const Route = createFileRoute("/$tenant/$locale/manage/b2b")({
  component: ManageB2BPage,
})

const config = crudConfigs["b2b"]

const STATUS_FILTERS = ["all", "active", "draft", "inactive"] as const

function ManageB2BPage() {
  ...
  const allItems = ((data as any)?.b2b || (data as any)?.listings || []).map((item: any) => ({
    id: item.id,
    name: item.name || "—",
    category: item.category || "—",
    min_order_value: item.min_order_value ? `$${(item.min_order_value / 100).toFixed(2)}` : "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage B2B listings and wholesale offers"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No B2B listings found" countLabel="B2B listings" />
      </Container>
