...
export const Route = createFileRoute("/$tenant/$locale/manage/disputes")({
  component: ManageDisputesPage,
})

const config = crudConfigs["disputes"]
const STATUS_FILTERS = ["all", "open", "investigating", "resolved", "escalated", "closed"] as const
...
function ManageDisputesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["manage", "disputes"],
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/disputes", { method: "GET" })
      return response
    },
    enabled: typeof window !== "undefined",
  })

  const allItems = ((data as any)?.disputes || (data as any)?.items || []).map((item: any) => ({
    id: item.id,
    title: item.title || item.reason || "—",
    order_id: item.order_id || "—",
    customer: item.customer || item.customer_email || "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage customer disputes"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No disputes found" countLabel="disputes" />
      </Container>
      <FormDrawer
        open={drawerOpen}
