...
const STATUS_FILTERS = ["all", "active", "redeemed", "expired", "disabled"] as const
...
function ManageGiftCardsPage() {
  ...
  const allItems = ((data as any)?.gift_cards || (data as any)?.cards || []).map((item: any) => ({
    id: item.id,
    code: item.code || "—",
    denomination: item.denomination ? `$${(item.denomination / 100).toFixed(2)}` : "$0.00",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No gift cards found" countLabel="gift cards" />
      </Container>
