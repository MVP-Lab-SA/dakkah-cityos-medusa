...
export const Route = createFileRoute("/$tenant/$locale/manage/newsletters")({
  component: ManageNewslettersPage,
})

const config = crudConfigs["newsletters"]

const STATUS_FILTERS = ["all", "draft", "active", "paused", "archived"] as const

function ManageNewslettersPage() {
  ...
  const allItems = ((data as any)?.newsletters || []).map((item: any) => ({
    id: item.id,
    name: item.name || item.title || "—",
    frequency: item.frequency || "—",
    subscribers_count: item.subscribers_count ?? item.subscribers ?? 0,
    last_sent: item.last_sent ? new Date(item.last_sent).toLocaleDateString() : "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage newsletters and email campaigns"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No newsletters found" countLabel="newsletters" />
      </Container>
