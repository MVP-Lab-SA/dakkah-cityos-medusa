...
const config = crudConfigs["channels"]
...
function ManageChannelsPage() {
  ...
  const allItems = ((data as any)?.channels || (data as any)?.items || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage sales channels"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No channels found" countLabel="channels" />
      </Container>
      <FormDrawer
        open={drawerOpen}
