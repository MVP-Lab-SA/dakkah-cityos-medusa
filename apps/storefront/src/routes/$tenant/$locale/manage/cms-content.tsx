...
export const Route = createFileRoute("/$tenant/$locale/manage/cms-content")({
  component: ManageCmsContentPage,
})
...
function ManageCmsContentPage() {
  ...
  const allItems = ((data as any)?.items || (data as any)?.cms_content || []).map((item: any) => ({
    id: item.id,
    title: item.title || "—",
    type: item.type || "page",
    slug: item.slug || "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage CMS content pages, posts and blocks"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No CMS content found" countLabel="items" />
      </Container>
