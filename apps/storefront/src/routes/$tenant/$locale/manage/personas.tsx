...
const config = crudConfigs["personas"]
...
function ManagePersonasPage() {
  ...
  const allItems = ((data as any)?.personas || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No personas found" countLabel="personas" />
      </Container>
      <FormDrawer
        open={drawerOpen}
