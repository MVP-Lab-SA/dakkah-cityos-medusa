...
export const Route = createFileRoute("/$tenant/$locale/manage/promotion-extensions")({
  component: ManagePromotionExtPage,
})

const config = crudConfigs["promotion-ext"]
const STATUS_FILTERS = ["all", "active", "scheduled", "expired", "draft"] as const

function ManagePromotionExtPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["manage", "promotion-ext"],
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/promotions-ext", { method: "GET" })
      return response
    },
    enabled: typeof window !== "undefined",
  })

  const allItems = ((data as any)?.promotions || (data as any)?.["promotion-ext"] || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage promotion extensions"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No promotion extensions found" countLabel="promotion extensions" />
      </Container>
      <FormDrawer
        open={drawerOpen}
