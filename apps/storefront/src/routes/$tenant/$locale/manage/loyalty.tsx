...
export const Route = createFileRoute("/$tenant/$locale/manage/loyalty")({
  component: ManageLoyaltyPage,
})

const config = crudConfigs["loyalty"]
const STATUS_FILTERS = ["all", "active", "paused", "expired"] as const

function ManageLoyaltyPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      header: t(locale, "manage.multiplier"),
      align: "end" as const,
    },
    {
      key: "members",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage loyalty programs"
          actions={
