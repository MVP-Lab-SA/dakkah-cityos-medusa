...
export const Route = createFileRoute("/$tenant/$locale/manage/notification-preferences")({
...
function ManageNotificationPreferencesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/notification-preferences", { method: "GET" })
      return response
    },
    enabled: typeof window !== "undefined",
  })

  const allItems = ((data as any)?.preferences || (data as any)?.notification_preferences || (data as any)?.items || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No notification preferences found" countLabel="preferences" />
      </Container>
      <FormDrawer
        open={drawerOpen}
