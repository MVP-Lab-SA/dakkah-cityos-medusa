...
export const Route = createFileRoute("/$tenant/$locale/manage/tax-config")({
  component: ManageTaxConfigPage,
})

const config = crudConfigs["tax-config"]
const STATUS_FILTERS = ["all", "active", "inactive"] as const

function ManageTaxConfigPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["manage", "tax-config"],
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/tax-config", { method: "GET" })
      return response
    },
    enabled: typeof window !== "undefined",
  })

  const allItems = ((data as any)?.["tax-config"] || (data as any)?.taxes || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No tax configurations found" countLabel="tax configurations" />
      </Container>
      <FormDrawer
        open={drawerOpen}
