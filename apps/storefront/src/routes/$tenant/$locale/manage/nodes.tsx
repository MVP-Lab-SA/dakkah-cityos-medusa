...
const config = crudConfigs["nodes"]
const STATUS_FILTERS = ["all", "CITY", "DISTRICT", "ZONE", "FACILITY", "ASSET"] as const
...
function ManageNodesPage() {
  ...
  const allItems = ((data as any)?.nodes || (data as any)?.items || []).map((item: any) => ({
    id: item.id,
    name: item.name || item.title || "—",
    node_type: item.node_type || item.type || "CITY",
    parent: item.parent_name || item.parent_id || "—",
    code: item.code || "—",
    status: item.node_type || item.type || "CITY",
  }))

  const items = statusFilter === "all"
    ? allItems
    : allItems.filter((i: any) => i.node_type === statusFilter)
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage hierarchy nodes"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No nodes found" countLabel="nodes" />
      </Container>
      <FormDrawer
        open={drawerOpen}
