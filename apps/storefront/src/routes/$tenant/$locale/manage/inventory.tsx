...
export const Route = createFileRoute("/$tenant/$locale/manage/inventory")({
...
function ManageInventoryPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["manage", "inventory-extension"],
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/inventory-ext", { method: "GET" })
      return response
    },
    enabled: typeof window !== "undefined",
  })

  const allItems = ((data as any)?.inventory || (data as any)?.items || []).map((item: any) => ({
    id: item.id,
  ...
  const columns = [
    ...
    {
      key: "sku",
      header: t(locale, "manage.sku"),
    },
    {
      key: "quantity",
      header: t(locale, "manage.quantity"),
      align: "end" as const,
    },
    {
      key: "reserved",
      header: t(locale, "manage.reserved"),
      align: "end" as const,
    },
    {
      key: "status",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No inventory items found" countLabel="items" />
      </Container>
      <FormDrawer
        open={drawerOpen}
