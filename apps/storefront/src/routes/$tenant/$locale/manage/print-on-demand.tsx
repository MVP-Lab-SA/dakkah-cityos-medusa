...
function ManagePrintOnDemandPage() {
  ...
  const allItems = ((data as any)?.designs || (data as any)?.products || []).map((item: any) => ({
    id: item.id,
    design_name: item.design_name || item.name || "—",
    product_type: item.product_type || "—",
    base_cost: item.base_cost ? `$${(item.base_cost / 100).toFixed(2)}` : "$0.00",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No POD designs found" countLabel="designs" />
      </Container>
