...
export const Route = createFileRoute("/$tenant/$locale/manage/flash-deals")({
  component: ManageFlashDealsPage,
})

const config = crudConfigs["flash-deals"]

const STATUS_FILTERS = ["all", "scheduled", "active", "ended", "cancelled"] as const

function ManageFlashDealsPage() {
  ...
  const allItems = ((data as any)?.flash_deals || (data as any)?.deals || []).map((item: any) => ({
    id: item.id,
    title: item.title || item.name || "—",
    discount_type: item.discount_type || "—",
    discount_value: item.discount_value ? (item.discount_type === "percentage" ? `${item.discount_value}%` : `$${(item.discount_value / 100).toFixed(2)}`) : "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage flash deals and limited-time offers"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No flash deals found" countLabel="flash deals" />
      </Container>
