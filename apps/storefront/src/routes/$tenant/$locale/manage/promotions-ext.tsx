...
export const Route = createFileRoute("/$tenant/$locale/manage/promotions-ext")({
  component: ManagePromotionsExtPage,
})

const config = {
  moduleKey: "promotions-ext",
  singularLabel: "Promotion",
  pluralLabel: "Promotions",
  label: "Extended Promotions",
  apiEndpoint: "/admin/promotions-ext",
  fields: [
    { key: "name", label: "Name", type: "text" as const, required: true, placeholder: "Promotion name" },
    ...
    { key: "value", label: "Value", type: "number" as const, placeholder: "0" },
  ...
  defaultValues: { name: "", type: "percentage", value: 0, start_date: "", end_date: "", status: "active" },
}

const STATUS_FILTERS = ["all", "active", "inactive", "scheduled", "expired"] as const

function ManagePromotionsExtPage() {
  ...
  const allItems = ((data as any)?.items || (data as any)?.promotions || []).map((item: any) => ({
    id: item.id,
    name: item.name || "—",
    type: item.type || "—",
    value: item.value ?? "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage extended promotions and deals"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No promotions found" countLabel="promotions" />
      </Container>
