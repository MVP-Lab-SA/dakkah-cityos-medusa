...
export const Route = createFileRoute("/$tenant/$locale/manage/company")({
  component: ManageCompanyPage,
})

const config = crudConfigs["company"]
...
function ManageCompanyPage() {
  ...
  const allItems = ((data as any)?.items || (data as any)?.companies || []).map((item: any) => ({
    id: item.id,
    name: item.name || "—",
    registration_number: item.registration_number || item.tax_id || "—",
    type: item.type || item.tier || "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No companies found" countLabel="companies" />
      </Container>
