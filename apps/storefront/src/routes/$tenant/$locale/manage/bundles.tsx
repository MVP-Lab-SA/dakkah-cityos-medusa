...
export const Route = createFileRoute("/$tenant/$locale/manage/bundles")({
  component: ManageBundlesPage,
})

const config = crudConfigs["bundles"]

const STATUS_FILTERS = ["all", "active", "draft", "archived"] as const

function ManageBundlesPage() {
  ...
  const allItems = ((data as any)?.bundles || []).map((item: any) => ({
    id: item.id,
    name: item.name || item.title || "—",
    items_count: item.items_count ?? item.items?.length ?? 0,
    price: item.price ? `$${(item.price / 100).toFixed(2)}` : "$0.00",
    savings: item.savings ? `$${(item.savings / 100).toFixed(2)}` : "$0.00",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage product bundles"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No bundles found" countLabel="bundles" />
      </Container>
