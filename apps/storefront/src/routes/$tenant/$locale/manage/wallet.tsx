...
export const Route = createFileRoute("/$tenant/$locale/manage/wallet")({
  component: ManageWalletPage,
})

const config = crudConfigs["wallet"]

const STATUS_FILTERS = ["all", "active", "frozen", "closed"] as const
...
function ManageWalletPage() {
  ...
  const allItems = ((data as any)?.wallets || (data as any)?.wallet || []).map((item: any) => ({
    id: item.id,
    user: item.user || item.user_name || item.customer_name || item.email || "—",
    balance: item.balance ? `$${(item.balance / 100).toFixed(2)}` : "$0.00",
    pending: item.pending ? `$${(item.pending / 100).toFixed(2)}` : "$0.00",
    last_transaction: item.last_transaction || item.last_transaction_date ? new Date(item.last_transaction || item.last_transaction_date).toLocaleDateString() : "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage user wallets and balances"
        />
        ...
        <DataTable columns={columns} data={items} emptyTitle="No wallets found" countLabel="wallets" />
      </Container>
