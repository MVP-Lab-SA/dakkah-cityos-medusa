...
const config = crudConfigs["region-zones"]
...
function ManageRegionZonesPage() {
  ...
  const allItems = ((data as any)?.["region-zones"] || (data as any)?.regions || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No region zones found" countLabel="region zones" />
      </Container>
      <FormDrawer
        open={drawerOpen}
