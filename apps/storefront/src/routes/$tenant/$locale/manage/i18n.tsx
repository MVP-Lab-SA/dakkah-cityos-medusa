...
const config = crudConfigs["i18n"]
const STATUS_FILTERS = ["all", "translated", "pending", "outdated"] as const
...
function ManageI18nPage() {
  ...
  const allItems = ((data as any)?.translations || (data as any)?.i18n || (data as any)?.items || []).map((item: any) => ({
    id: item.id,
    key: item.key || item.translation_key || "—",
  ...
  const columns = [
    {
      key: "key",
      header: t(locale, "manage.key"),
      render: (val: unknown) => <span className="font-medium">{val as string}</span>,
    },
    {
      key: "locale",
      header: t(locale, "manage.locale"),
    },
    {
      key: "value",
      header: t(locale, "manage.value"),
    },
    {
      key: "status",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage translations and internationalization"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No translations found" countLabel="translations" />
      </Container>
      <FormDrawer
        open={drawerOpen}
