...
export const Route = createFileRoute("/$tenant/$locale/manage/credit")({
  component: ManageCreditPage,
})

const config = crudConfigs["credit"]

const STATUS_FILTERS = ["all", "active", "pending", "suspended"] as const

function ManageCreditPage() {
  ...
  const allItems = ((data as any)?.credit_options || (data as any)?.credit || []).map((item: any) => ({
    id: item.id,
    name: item.name || "—",
    apr: item.apr ? `${item.apr}%` : "—",
    term_months: item.term_months ? `${item.term_months} months` : "—",
    applications_count: item.applications_count ?? item.applications ?? 0,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage credit options and financing"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No credit options found" countLabel="credit options" />
      </Container>
