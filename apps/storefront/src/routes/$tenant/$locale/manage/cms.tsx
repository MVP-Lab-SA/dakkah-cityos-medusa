...
export const Route = createFileRoute("/$tenant/$locale/manage/cms")({
  component: ManageCmsPage,
})

const config = crudConfigs["cms-content"]
const STATUS_FILTERS = ["all", "draft", "published", "archived"] as const

function ManageCmsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["manage", "cms-content"],
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/cms/pages", { method: "GET" })
      return response
    },
    enabled: typeof window !== "undefined",
  })

  const allItems = ((data as any)?.pages || (data as any)?.items || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No CMS content found" countLabel="pages" />
      </Container>
      <FormDrawer
        open={drawerOpen}
