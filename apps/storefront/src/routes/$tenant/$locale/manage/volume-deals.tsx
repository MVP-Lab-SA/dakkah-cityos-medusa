...
export const Route = createFileRoute("/$tenant/$locale/manage/volume-deals")({
  component: ManageVolumeDealsPage,
})

const config = crudConfigs["volume-deals"]

const STATUS_FILTERS = ["all", "active", "draft", "expired"] as const

function ManageVolumeDealsPage() {
  ...
  const allItems = ((data as any)?.volume_deals || (data as any)?.deals || []).map((item: any) => ({
    id: item.id,
    name: item.name || "—",
    product_id: item.product_id || "—",
    min_order_qty: item.min_order_qty ?? 0,
    max_discount_percent: item.max_discount_percent ? `${item.max_discount_percent}%` : "—",
    tiers_count: Array.isArray(item.tiers) ? item.tiers.length : 0,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage volume deals and bulk pricing offers"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No volume deals found" countLabel="volume deals" />
      </Container>
