...
export const Route = createFileRoute("/$tenant/$locale/manage/consignments")({
  component: ManageConsignmentsPage,
})

const config = crudConfigs["consignments"]

const STATUS_FILTERS = ["all", "pending", "active", "sold", "returned"] as const

function ManageConsignmentsPage() {
  ...
  const allItems = ((data as any)?.consignments || []).map((item: any) => ({
    id: item.id,
    name: item.name || item.item_name || "—",
    consigner: item.consigner || item.consigner_name || "—",
    condition: item.condition || "—",
    price: item.price ? `$${(item.price / 100).toFixed(2)}` : "$0.00",
    commission_rate: item.commission_rate ? `${item.commission_rate}%` : "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage consignment items"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No consignments found" countLabel="consignments" />
      </Container>
