...
export const Route = createFileRoute("/$tenant/$locale/manage/cart-extensions")({
  component: ManageCartExtensionsPage,
})

const config = crudConfigs["cart-extension"]
const STATUS_FILTERS = ["all", "active", "abandoned", "completed"] as const

function ManageCartExtensionsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["manage", "cart-extension"],
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/cart-extensions", { method: "GET" })
      return response
    },
    enabled: typeof window !== "undefined",
  })

  const allItems = ((data as any)?.["cart-extensions"] || (data as any)?.carts || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage cart extensions"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No cart extensions found" countLabel="cart extensions" />
      </Container>
      <FormDrawer
        open={drawerOpen}
