...
export const Route = createFileRoute("/$tenant/$locale/manage/companies-admin")({
  component: ManageCompaniesAdminPage,
})

const config = crudConfigs["companies"]
const STATUS_FILTERS = ["all", "active", "inactive", "pending"] as const

function ManageCompaniesAdminPage() {
  ...
  const allItems = ((data as any)?.companies || []).map((item: any) => ({
    id: item.id,
