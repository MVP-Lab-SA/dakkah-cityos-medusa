...
export const Route = createFileRoute("/$tenant/$locale/manage/flash-sales")({
  component: ManageFlashSalesPage,
})

const config = crudConfigs["flash-sales"]

const STATUS_FILTERS = ["all", "scheduled", "active", "ended", "cancelled"] as const

function ManageFlashSalesPage() {
  ...
  const allItems = ((data as any)?.flash_sales || (data as any)?.sales || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage flash sales and limited-time offers"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No flash sales found" countLabel="flash sales" />
      </Container>
