...
export const Route = createFileRoute("/$tenant/$locale/manage/shipping-extensions")({
  component: ManageShippingExtPage,
})

const config = crudConfigs["shipping-ext"]
const STATUS_FILTERS = ["all", "active", "inactive", "draft"] as const

function ManageShippingExtPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["manage", "shipping-ext"],
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/shipping-ext", { method: "GET" })
      return response
    },
    enabled: typeof window !== "undefined",
  })

  const allItems = ((data as any)?.["shipping-ext"] || (data as any)?.shipping || []).map((item: any) => ({
    id: item.id,
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage shipping extensions"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No shipping extensions found" countLabel="shipping extensions" />
      </Container>
      <FormDrawer
        open={drawerOpen}
