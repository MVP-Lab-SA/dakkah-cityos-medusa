...
const config = {
  ...
  fields: [
    { key: "name", label: "Name", type: "text" as const, required: true, placeholder: "Term name" },
    { key: "days", label: "Days", type: "number" as const, required: true, placeholder: "30" },
    { key: "discount_percentage", label: "Discount (%)", type: "number" as const, placeholder: "0" },
    { key: "description", label: "Description", type: "textarea" as const, placeholder: "Payment term description" },
    { key: "is_default", label: "Default Term", type: "checkbox" as const, placeholder: "Set as default payment term" },
  ],
  defaultValues: { name: "", days: 30, discount_percentage: 0, description: "", is_default: false },
}
...
function ManagePaymentTermsPage() {
  ...
  const items = ((data as any)?.items || (data as any)?.payment_terms || []).map((item: any) => ({
    id: item.id,
    name: item.name || "—",
    days: item.days ?? "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No payment terms found" countLabel="terms" />
      </Container>
