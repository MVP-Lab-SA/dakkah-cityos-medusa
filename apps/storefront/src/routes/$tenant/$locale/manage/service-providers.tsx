...
export const Route = createFileRoute("/$tenant/$locale/manage/service-providers")({
  component: ManageServiceProvidersPage,
})

const config = {
  moduleKey: "service-providers",
  singularLabel: "Service Provider",
  pluralLabel: "Service Providers",
  label: "Service Providers",
  apiEndpoint: "/admin/service-providers",
  fields: [
    { key: "name", label: "Name", type: "text" as const, required: true, placeholder: "Provider name" },
    { key: "type", label: "Type", type: "select" as const, options: [
      { value: "individual", label: "Individual" },
      { value: "company", label: "Company" },
      { value: "agency", label: "Agency" },
    ]},
    { key: "contact_email", label: "Contact Email", type: "email" as const, placeholder: "email@example.com" },
    ...
    { key: "rating", label: "Rating", type: "number" as const, placeholder: "0" },
  ],
  defaultValues: { name: "", type: "individual", contact_email: "", status: "active", rating: 0 },
}

const STATUS_FILTERS = ["all", "active", "inactive", "pending", "suspended"] as const

function ManageServiceProvidersPage() {
  ...
  const allItems = ((data as any)?.items || (data as any)?.service_providers || []).map((item: any) => ({
    id: item.id,
    name: item.name || "—",
    type: item.type || "—",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable columns={columns} data={items} emptyTitle="No service providers found" countLabel="providers" />
      </Container>
