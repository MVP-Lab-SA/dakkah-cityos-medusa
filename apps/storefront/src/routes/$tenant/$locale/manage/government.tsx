...
export const Route = createFileRoute("/$tenant/$locale/manage/government")({
  component: ManageGovernmentPage,
})

const config = crudConfigs["government"]
const STATUS_FILTERS = ["all", "active", "processing", "completed", "rejected"] as const

function ManageGovernmentPage() {
  ...
  const allItems = ((data as any)?.government || (data as any)?.services || (data as any)?.items || []).map((item: any) => ({
    id: item.id,
  ...
  const columns = [
    ...
    {
      key: "department",
      header: t(locale, "manage.department"),
    },
    {
      key: "status",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage government services"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No government services found" countLabel="services" />
      </Container>
      <FormDrawer
        open={drawerOpen}
