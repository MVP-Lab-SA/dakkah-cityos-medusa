...
export const Route = createFileRoute("/$tenant/$locale/manage/insurance")({
  component: ManageInsurancePage,
})

const config = crudConfigs["insurance"]
...
function ManageInsurancePage() {
  ...
  const allItems = ((data as any)?.plans || (data as any)?.insurance || []).map((item: any) => ({
    id: item.id,
    name: item.name || "—",
    type: item.type || "—",
    provider: item.provider || "—",
    premium: item.premium ? `$${(item.premium / 100).toFixed(2)}` : "$0.00",
  ...
  return (
    <ManageLayout locale={locale}>
      <Container>
        <PageHeader
          title={config.label}
          subtitle="Manage insurance plans"
          actions={
        ...
        <DataTable columns={columns} data={items} emptyTitle="No insurance plans found" countLabel="plans" />
      </Container>
