              ...
              <div className="flex items-start gap-4">
                <div className="w-20 h-20 bg-ds-primary/10 rounded-xl flex items-center justify-center text-ds-primary flex-shrink-0">
                  {service.thumbnail || service.image ? (
                    <img src={service.thumbnail || service.image} alt={service.name} className="w-full h-full object-cover rounded-xl" />
                  ) : (
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                {service.hourly_rate != null && (
                  <div className="text-center">
                    <p className="text-3xl font-bold text-ds-foreground">${Number(service.hourly_rate).toLocaleString()}</p>
                ...
                {service.consultation_fee != null && (
                  <div className="text-center text-sm text-ds-muted-foreground">
                    Initial consultation: {Number(service.consultation_fee) === 0 ? "Free" : `$${Number(service.consultation_fee).toLocaleString()}`}
                  </div>
                )}
