...
function DropshippingDetailPage() {
  ...
  const { data: supplier, isLoading, error } = useQuery({
    queryKey: ["dropshipping-supplier", id],
    queryFn: async () => {
      const response = await sdk.client.fetch<{ supplier: any }>(
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {supplier.products.map((product: any) => (
                    <div key={product.id} className="border border-ds-border rounded-lg p-4 hover:bg-ds-muted/50 transition-colors">
                      ...
                      {product.price && <p className="text-sm text-ds-primary font-semibold mt-1">${Number(product.price).toLocaleString()}</p>}
                    </div>
                  ))}
                </div>
              </div>
