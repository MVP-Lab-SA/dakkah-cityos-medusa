...
export const Route = createFileRoute("/$tenant/$locale/fitness/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse fitness services on Dakkah CityOS" },
    ],
  }),
  loader: async () => {
    try {
      const baseUrl = getServerBaseUrl()
      const resp = await fetchWithTimeout(`${baseUrl}/store/fitness`, {
        headers: {
      ...
      const raw = data.fitness_listings || data.classes || data.items || data.listings || data.products || []
      const items = raw.map((c: any) => {
        ...
        return {
          ...
          class_type: meta.class_type || c.class_type || c.type || c.category || null,
...
const classTypeOptions = ["all", "yoga", "pilates", "hiit", "spinning", "boxing", "crossfit", "swimming"] as const
                    ...
                    <div className="aspect-[4/3] bg-gradient-to-br from-ds-destructive/10 to-rose-100 relative overflow-hidden">
                      {item.thumbnail ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <svg className="w-16 h-16 text-rose-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                        </div>
                      )}
                      {item.class_type && (
                        <span className="absolute top-2 left-2 px-2 py-1 text-xs font-medium bg-ds-destructive text-white rounded-md capitalize">{item.class_type}</span>
                      )}
                      {item.level && (
                        <span className="absolute top-2 right-2 px-2 py-1 text-xs font-medium bg-ds-card/90 text-ds-foreground/80 rounded-md capitalize">{item.level}</span>
                      )}
                    </div>
                    <div className="p-4">
                      <h3 className="font-semibold text-ds-foreground group-hover:text-rose-600 transition-colors line-clamp-1">{item.name}</h3>
                      {item.instructor && (
                        <p className="text-xs text-ds-muted-foreground mt-0.5">with {item.instructor}</p>
                      )}
                      {item.description && (
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">Expert Instructors</h3>
              <p className="text-sm text-ds-muted-foreground">Certified professionals guiding you every step of the way.</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-rose-500 text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">📅</div>
              <h3 className="font-semibold text-ds-foreground mb-2">Flexible Schedule</h3>
              <p className="text-sm text-ds-muted-foreground">Morning, afternoon, and evening classes to fit your lifestyle.</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-rose-500 text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">🏆</div>
              <h3 className="font-semibold text-ds-foreground mb-2">All Levels Welcome</h3>
              <p className="text-sm text-ds-muted-foreground">From beginners to advanced athletes, there is a class for you.</p>
            </div>
          </div>
        </div>
      </section>
