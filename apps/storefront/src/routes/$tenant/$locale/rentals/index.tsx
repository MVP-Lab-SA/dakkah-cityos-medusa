...
const rentalTypeOptions = ["all", "daily", "weekly", "monthly", "hourly"] as const
const categoryOptions = ["all", "electronics", "vehicles", "equipment", "furniture", "tools", "sports", "clothing"] as const
...
const rentalTypeColors: Record<string, string> = {
  ...
  hourly: "bg-red-100 text-red-700",
}
...
function RentalsPage() {
  ...
  const filteredItems = items.filter((item: any) => {
    const matchesSearch = searchQuery
      ...
      : true
    const matchesType = rentalTypeFilter === "all" || item.rental_type?.toLowerCase() === rentalTypeFilter
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-72 flex-shrink-0">
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">Rental Type</label>
                <div className="space-y-1">
                  {rentalTypeOptions.map((opt) => (
                    <button
                      key={opt}
                      onClick={() => setRentalTypeFilter(opt)}
                      className={`block w-full text-start px-3 py-2 text-sm rounded-lg transition-colors ${rentalTypeFilter === opt ? "bg-orange-600 text-white" : "text-ds-foreground hover:bg-ds-muted"}`}
                    >
                ...
                <div className="space-y-1">
                  {categoryOptions.map((opt) => (
                    <button
                      ...
                      className={`block w-full text-start px-3 py-2 text-sm rounded-lg transition-colors ${categoryFilter === opt ? "bg-orange-600 text-white" : "text-ds-foreground hover:bg-ds-muted"}`}
                    >
                ...
                {filteredItems.map((item: any) => (
                  ...
                  >
                    <div className="aspect-[4/3] bg-gradient-to-br from-orange-50 to-amber-100 relative overflow-hidden">
                      ...
                      {item.rental_type && (
                        <span className={`absolute top-2 left-2 px-2 py-1 text-xs font-medium rounded-md capitalize ${rentalTypeColors[item.rental_type?.toLowerCase()] || "bg-gray-100 text-gray-700"}`}>
                          {item.rental_type}
                        </span>
                      )}
                      <span className={`absolute top-2 right-2 px-2 py-1 text-xs font-medium rounded-md ${item.is_available ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex justify-between items-center pt-3 mt-3 border-t border-ds-border">
                        ...
                        <span className="px-3 py-1.5 text-xs font-semibold text-white bg-orange-600 rounded-lg group-hover:bg-amber-600 transition-colors">Rent Now</span>
                      </div>
                    </div>
                  </a>
                ))}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-orange-600 text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">1</div>
              <h3 className="font-semibold text-ds-foreground mb-2">Find What You Need</h3>
              <p className="text-sm text-ds-muted-foreground">Browse our wide selection of rental items across multiple categories.</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-orange-600 text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">2</div>
              <h3 className="font-semibold text-ds-foreground mb-2">Choose Your Duration</h3>
              <p className="text-sm text-ds-muted-foreground">Select hourly, daily, weekly, or monthly rental periods that fit your schedule.</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-orange-600 text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">3</div>
              <h3 className="font-semibold text-ds-foreground mb-2">Enjoy & Return</h3>
              <p className="text-sm text-ds-muted-foreground">Use your rented item and return it when done — hassle-free process.</p>
            </div>
          </div>
        </div>
      </section>
