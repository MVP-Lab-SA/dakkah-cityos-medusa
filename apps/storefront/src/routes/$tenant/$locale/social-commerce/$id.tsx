...
export const Route = createFileRoute("/$tenant/$locale/social-commerce/$id")({
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetch(`${baseUrl}/store/social-commerce/${params.id}`, {
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                {product.price != null && (
                  <p className="text-3xl font-bold text-ds-foreground text-center">${Number(product.price || 0).toLocaleString()}</p>
                )}
