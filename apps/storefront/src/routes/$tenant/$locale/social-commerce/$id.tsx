...
function SocialCommerceDetailPage() {
  ...
  const { data: product, isLoading, error } = useQuery({
    queryKey: ["social-commerce", id],
    queryFn: async () => {
      const response = await sdk.client.fetch<{ item: any }>(
        `/store/social-commerce/${id}`,
      ...
      <div className="min-h-screen bg-ds-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <Link to={`${prefix}/social-commerce` as any} className="inline-flex items-center px-4 py-2 text-sm font-medium bg-ds-primary text-ds-primary-foreground rounded-lg hover:bg-ds-primary/90 transition-colors">
              Browse Products
            </Link>
          </div>
        </div>
      ...
      <div className="bg-ds-card border-b border-ds-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-sm text-ds-muted-foreground">
            ...
            <span>/</span>
            <Link to={`${prefix}/social-commerce` as any} className="hover:text-ds-foreground transition-colors">Social Commerce</Link>
            <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              ...
              {product.category && (
                <span className="absolute top-4 start-4 px-3 py-1 text-xs font-semibold rounded-full bg-ds-background/80 text-ds-foreground backdrop-blur-sm">{product.category}</span>
              )}
            </div>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-3">
                ...
                {product.rating && (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm font-medium text-ds-foreground">{product.rating}</span>
                    {product.review_count && <span className="text-sm text-ds-muted-foreground">({product.review_count})</span>}
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-ds-primary/10 rounded-full flex items-center justify-center text-ds-primary font-semibold">
                    {(product.seller.name || product.seller || "S").charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1">
                    <p className="font-medium text-ds-foreground">{typeof product.seller === "string" ? product.seller : product.seller.name}</p>
                    {product.seller.rating && <p className="text-xs text-ds-muted-foreground">Seller Rating: {product.seller.rating}/5</p>}
                  </div>
                  <Link to={`${prefix}/social-commerce` as any} className="text-sm text-ds-primary hover:underline">
                    View Store
                  </Link>
                </div>
              </div>
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <button className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium bg-ds-muted text-ds-foreground rounded-lg hover:bg-ds-muted/80 transition-colors">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                  Like
                </button>
              </div>
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                {product.price != null && (
                  <p className="text-3xl font-bold text-ds-foreground text-center">${Number(product.price).toLocaleString()}</p>
                )}
                {product.original_price && Number(product.original_price) > Number(product.price) && (
                  <div className="text-center">
                    <span className="text-sm text-ds-muted-foreground line-through">${Number(product.original_price).toLocaleString()}</span>
              ...
              {product.tags && product.tags.length > 0 && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  <h3 className="font-semibold text-ds-foreground mb-3">Tags</h3>
                  <div className="flex flex-wrap gap-2">
                    {product.tags.map((tag: string, idx: number) => (
                      <span key={idx} className="px-3 py-1 text-xs font-medium rounded-full bg-ds-muted text-ds-muted-foreground">{tag}</span>
                    ))}
                  </div>
                </div>
              )}
