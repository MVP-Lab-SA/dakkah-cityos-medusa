...
export const Route = createFileRoute("/$tenant/$locale/social-commerce/")({
  ...
  loader: async () => {
    try {
      ...
      const resp = await fetchWithTimeout(`${baseUrl}/store/social-commerce`, {
        headers: {
                ...
                {filteredItems.map((item: any) => (
                  ...
                  >
                    <div className="aspect-[4/3] bg-gradient-to-br from-ds-destructive/10 to-fuchsia-100 relative overflow-hidden">
                      ...
                      {item.platform && (
                        <span className={`absolute top-2 left-2 px-2.5 py-1 text-xs font-semibold text-white rounded-full capitalize ${platformColors[item.platform?.toLowerCase()] || "bg-ds-muted-foreground"}`}>
