...
function CMSPageComponent() {
  ...
  const { tenant, locale, _splat } = Route.useParams()
  const splatPath = _splat || data?.splatPath || ""
  const tenantId = TENANT_SLUG_TO_ID[tenant] || DEFAULT_TENANT_ID

  const [page, setPage] = useState<CMSPage | null>(data?.page || null)
  const [isLoading, setIsLoading] = useState(!data?.page && !!splatPath)
  const [hasError, setHasError] = useState(false)

  useEffect(() => {
    if (page || !splatPath || typeof window === "undefined") return
  ...
  }, [splatPath, tenantId, locale, page])
