      ...
      <div className="min-h-screen bg-ds-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">This white-label product may have been discontinued or is no longer available.</p>
            <Link to={`${prefix}/white-label` as any} className="inline-flex items-center px-4 py-2 text-sm font-medium bg-ds-primary text-ds-primary-foreground rounded-lg hover:bg-ds-primary/90 transition-colors">
              Browse Products
            </Link>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-3">
                <span className="text-2xl font-bold text-ds-primary">
                  {product.price != null ? `$${Number(product.price).toLocaleString()}` : "Contact for pricing"}
                </span>
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-6">
              <h2 className="font-semibold text-ds-foreground mb-4">Product Details</h2>
              <div className="grid grid-cols-2 gap-4 text-sm">
                {product.manufacturer && (
                  <div>
                    <span className="text-ds-muted-foreground">Manufacturer</span>
                    <p className="font-medium text-ds-foreground">{product.manufacturer}</p>
                  </div>
                )}
                {product.lead_time && (
                  <div>
                    <span className="text-ds-muted-foreground">Lead Time</span>
                    <p className="font-medium text-ds-foreground">{product.lead_time}</p>
                  </div>
                )}
                {product.customization_options && (
                  <div>
                    ...
                    <p className="font-medium text-ds-foreground">
                      {Array.isArray(product.customization_options) ? product.customization_options.join(", ") : product.customization_options}
                    </p>
                  </div>
                )}
                {product.certifications && (
                  <div>
                    <span className="text-ds-muted-foreground">Certifications</span>
                    <p className="font-medium text-ds-foreground">
