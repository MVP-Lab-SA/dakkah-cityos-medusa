  ...
  return (
    <div className="min-h-screen bg-ds-background">
      <div className="bg-gradient-to-r from-ds-destructive to-fuchsia-600 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center gap-2 text-sm text-white/70 mb-4">
            ...
            <span>/</span>
            <span className="text-white">{t(locale, 'print_on_demand.breadcrumb')}</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">{t(locale, 'print_on_demand.hero_title')}</h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto">{t(locale, 'print_on_demand.hero_subtitle')}</p>
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            <span>{t(locale, 'print_on_demand.badge_no_minimums')}</span><span>|</span><span>{t(locale, 'print_on_demand.badge_products')}</span><span>|</span><span>{t(locale, 'print_on_demand.badge_shipping')}</span>
          </div>
          <button className="mt-8 px-8 py-3 bg-ds-card text-ds-destructive font-semibold rounded-lg hover:bg-ds-card/90 transition-colors">{t(locale, 'print_on_demand.start_creating')}</button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t(locale, 'print_on_demand.search_placeholder')} className="w-full max-w-md px-4 py-2.5 text-sm rounded-lg border border-ds-border bg-ds-background text-ds-foreground placeholder:text-ds-muted-foreground focus:outline-none focus:ring-2 focus:ring-ds-destructive" />
        </div>

        <h2 className="text-2xl font-bold text-ds-foreground mb-6">{t(locale, 'print_on_demand.products_heading')}</h2>
        {filtered.length === 0 ? (
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground text-sm">{t(locale, 'print_on_demand.no_results_hint')}</p>
          </div>
        ) : (
            ...
            {filtered.map((p: any) => (
              <div key={p.id} className="group bg-ds-background border border-ds-border rounded-xl overflow-hidden hover:shadow-lg hover:border-ds-destructive/40 transition-all duration-200">
                ...
                <div className="p-5">
                  ...
                  <button className="w-full py-2.5 text-sm font-medium rounded-lg bg-ds-destructive text-white hover:bg-ds-destructive transition-colors">{t(locale, 'print_on_demand.design_now')}</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">{t(locale, 'print_on_demand.step1_title')}</h3>
              <p className="text-sm text-ds-muted-foreground">{t(locale, 'print_on_demand.step1_desc')}</p>
            </div>
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">{t(locale, 'print_on_demand.step2_title')}</h3>
              <p className="text-sm text-ds-muted-foreground">{t(locale, 'print_on_demand.step2_desc')}</p>
            </div>
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">{t(locale, 'print_on_demand.step3_title')}</h3>
              <p className="text-sm text-ds-muted-foreground">{t(locale, 'print_on_demand.step3_desc')}</p>
            </div>
          </div>
        </div>
      </section>
