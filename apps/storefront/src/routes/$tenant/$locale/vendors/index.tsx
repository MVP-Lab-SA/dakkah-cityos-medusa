...
export const Route = createFileRoute("/$tenant/$locale/vendors/")({
  ...
  loader: async () => {
    try {
      ...
      const resp = await fetch(`${baseUrl}/store/vendors`, {
        headers: {
              ...
              <div>
                ...
                <div className="space-y-1">
                  {verifiedOptions.map((opt) => (
                    ...
                    >
                      {opt === "all" ? "All Vendors" : opt.charAt(0).toUpperCase() + opt.slice(1)}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {filteredItems.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredItems.map((item: any) => (
                  ...
                  >
                    <div className="aspect-[3/1] bg-gradient-to-br from-purple-100 to-violet-200 relative overflow-hidden">
                      {item.banner_url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <svg className="w-12 h-12 text-purple-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
                        </div>
                      )}
                      {item.is_verified && (
                        <span className="absolute top-2 right-2 px-2 py-1 text-xs font-medium bg-green-500 text-white rounded-md flex items-center gap-1">
                    ...
                    <div className="p-4 relative">
                      <div className="flex items-start gap-3">
                        <div className="w-12 h-12 rounded-full bg-purple-100 border-2 border-white shadow-sm flex items-center justify-center overflow-hidden flex-shrink-0 -mt-8">
                          {item.logo_url ? (
                            <img src={item.logo_url} alt={item.business_name} className="w-full h-full object-cover" />
                          ) : (
                            <span className="text-lg font-bold text-purple-600">{(item.business_name || "V").charAt(0)}</span>
                          )}
                        </div>
                        <div className="flex-1 min-w-0 pt-1">
                          <h3 className="font-semibold text-ds-foreground group-hover:text-purple-600 transition-colors line-clamp-1">{item.business_name}</h3>
                        </div>
                      </div>
                      ...
                      {item.categories && item.categories.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-3">
                          {item.categories.slice(0, 3).map((cat: string, i: number) => (
                            <span key={i} className="px-2 py-0.5 text-xs bg-purple-50 text-purple-600 rounded-full capitalize">{cat}</span>
                          ))}
                        </div>
                      )}

                      <div className="pt-3 mt-3 border-t border-ds-border">
                        <span className="px-4 py-1.5 text-xs font-semibold text-white bg-purple-600 rounded-lg group-hover:bg-purple-700 transition-colors">Visit Store</span>
                      </div>
                    </div>
                  </a>
                ))}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-purple-600 text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">✓</div>
            ...
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-purple-600 text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">🛡️</div>
