            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-sm rounded-lg border border-ds-border bg-ds-background text-ds-foreground placeholder:text-ds-muted-foreground focus:outline-none focus:ring-2 focus:ring-cyan-400"
                />
              </div>
                ...
                <div className="space-y-1">
                  {typeOptions.map((opt) => (
                    <button
                      ...
                      className={`block w-full text-start px-3 py-2 text-sm rounded-lg transition-colors ${typeFilter === opt ? "bg-cyan-600 text-white" : "text-ds-foreground hover:bg-ds-muted"}`}
                    >
                      {opt === "all" ? "All Types" : `${typeIcon(opt)} ${opt.charAt(0).toUpperCase() + opt.slice(1)}`}
                    </button>
                  ))}
                </div>
              </div>
                ...
                {filteredItems.map((item: any) => {
                  ...
                  return (
                    <a
                      ...
                      className="group bg-ds-background border border-ds-border rounded-xl overflow-hidden hover:shadow-lg hover:border-cyan-300 transition-all duration-200"
                    >
                      <div className="aspect-[16/9] bg-gradient-to-br from-cyan-50 to-teal-100 relative overflow-hidden">
                        {item.thumbnail ? (
                        ...
                        {item.insurance_type && (
                          <span className="absolute top-2 left-2 px-2 py-1 text-xs font-medium bg-cyan-600 text-white rounded-md capitalize">
                            {typeIcon(item.insurance_type)} {item.insurance_type}
                          </span>
                        )}
                      </div>
                      <div className="p-4">
                        <h3 className="font-semibold text-ds-foreground group-hover:text-cyan-600 transition-colors line-clamp-1">{item.name}</h3>
                        {item.description && (
                        ...
                        <div className="flex justify-between items-center pt-3 mt-3 border-t border-ds-border">
                          ...
                          <span className="px-3 py-1.5 text-xs font-semibold text-white bg-cyan-600 rounded-lg group-hover:bg-cyan-700 transition-colors">Get Quote</span>
                        </div>
                      </div>
                    </a>
                  )
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-cyan-600 text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">1</div>
            ...
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-cyan-600 text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">2</div>
            ...
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-cyan-600 text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">3</div>
