...
export const Route = createFileRoute("/$tenant/$locale/insurance/$id")({
  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(`${baseUrl}/store/insurance/${params.id}`, {
      ...
      <div className="min-h-screen bg-ds-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-ds-foreground mb-2">Plan Not Found</h2>
            <p className="text-ds-muted-foreground mb-6">This insurance plan may have been removed or is no longer available.</p>
            <Link to={`${prefix}/insurance` as any} className="inline-flex items-center px-4 py-2 text-sm font-medium bg-ds-primary text-ds-primary-foreground rounded-lg hover:bg-ds-primary/90 transition-colors">
      ...
      <div className="bg-ds-card border-b border-ds-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-sm text-ds-muted-foreground">
            ...
            <span>/</span>
            <Link to={`${prefix}/insurance` as any} className="hover:text-ds-foreground transition-colors">Insurance</Link>
            <span>/</span>
            <span className="text-ds-foreground truncate">{plan.name || plan.title}</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-ds-background border border-ds-border rounded-xl p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h1 className="text-2xl sm:text-3xl font-bold text-ds-foreground">{plan.name || plan.title}</h1>
                  <div className="flex flex-wrap items-center gap-3 mt-2">
                    ...
                    {plan.provider && (
                      <span className="text-sm text-ds-muted-foreground">by {plan.provider}</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {plan.premium != null && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-4 text-center">
                  <p className="text-xs text-ds-muted-foreground mb-1">Premium</p>
                  <p className="text-lg font-bold text-ds-foreground">${Number(plan.premium || 0).toLocaleString()}</p>
              ...
              {plan.deductible != null && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-4 text-center">
                  ...
                  <p className="text-lg font-bold text-ds-foreground">${Number(plan.deductible || 0).toLocaleString()}</p>
                </div>
              )}
              {plan.coverage_amount != null && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-4 text-center">
                  <p className="text-xs text-ds-muted-foreground mb-1">Coverage</p>
                  <p className="text-lg font-bold text-ds-foreground">${Number(plan.coverage_amount || 0).toLocaleString()}</p>
                </div>
              )}
              {plan.rating && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-4 text-center">
                  <p className="text-xs text-ds-muted-foreground mb-1">Rating</p>
                  <div className="flex items-center justify-center gap-1">
                    ...
                    <span className="text-lg font-bold text-ds-foreground">{plan.rating}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {plan.coverage_details && plan.coverage_details.length > 0 && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-4">Coverage Details</h2>
                <div className="space-y-3">
                  {plan.coverage_details.map((detail: any, idx: number) => (
                    <div key={idx} className="flex items-start gap-3 p-3 bg-ds-muted/30 rounded-lg">
                      <svg className="w-5 h-5 text-ds-primary flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                      <div>
                        <p className="font-medium text-ds-foreground text-sm">{typeof detail === "string" ? detail : detail.name}</p>
                        {detail.description && <p className="text-xs text-ds-muted-foreground mt-0.5">{detail.description}</p>}
                        {detail.limit && <p className="text-xs text-ds-primary mt-0.5">Limit: {detail.limit}</p>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {plan.exclusions && plan.exclusions.length > 0 && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-3">Exclusions</h2>
                <div className="space-y-2">
                  {plan.exclusions.map((exclusion: string, idx: number) => (
                    <div key={idx} className="flex items-center gap-2 text-sm text-ds-muted-foreground">
                      ...
                      {typeof exclusion === "string" ? exclusion : exclusion.name}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {plan.terms && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <p className="text-ds-muted-foreground text-sm leading-relaxed whitespace-pre-wrap">{plan.terms}</p>
              </div>
            )}
          </div>
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                {plan.premium != null && (
                  <div className="text-center">
                    <p className="text-3xl font-bold text-ds-foreground">${Number(plan.premium || 0).toLocaleString()}</p>
                    <p className="text-sm text-ds-muted-foreground">per month</p>
                  </div>
                )}
              ...
              {plan.features && plan.features.length > 0 && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  <h3 className="font-semibold text-ds-foreground mb-3">Key Features</h3>
                  <div className="space-y-2">
              ...
              {plan.provider_info && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  <h3 className="font-semibold text-ds-foreground mb-3">Provider</h3>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-ds-primary/10 rounded-full flex items-center justify-center text-ds-primary font-semibold">
                      {(plan.provider_info.name || plan.provider || "P").charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <p className="font-medium text-ds-foreground text-sm">{plan.provider_info.name || plan.provider}</p>
                      {plan.provider_info.rating && <p className="text-xs text-ds-muted-foreground">Rating: {plan.provider_info.rating}/5</p>}
                    </div>
                  </div>
                </div>
              )}
