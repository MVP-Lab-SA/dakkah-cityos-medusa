...
function AutomotivePage() {
  ...
  useEffect(() => {
    fetch("/store/automotive")
      .then((res) => res.json())
      .then((data) => {
        setVehicles(data.automotive || data.vehicles || data.items || [])
        setIsLoading(false)
      })
    ...
    <div className="min-h-screen bg-zinc-50">
      <section className="bg-zinc-900 text-white py-16">
        <div className="content-container">
          <h1 className="text-4xl font-bold mb-3">Automotive</h1>
          <p className="text-zinc-300 text-lg max-w-2xl">
            Browse vehicles from trusted dealers. Find your next car, truck, or SUV with detailed specs and pricing.
          </p>
        </div>
      </section>
      ...
      <div className="content-container py-12">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-zinc-500">No vehicles available at the moment</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {vehicles.map((vehicle) => (
              <Link
                key={vehicle.id}
                to={`/${tenant}/${locale}/automotive/${vehicle.id}` as any}
              ...
              >
                <div className="h-48 bg-zinc-100">
                  {(vehicle.photo || vehicle.thumbnail || vehicle.image) ? (
                    <img src={vehicle.photo || vehicle.thumbnail || vehicle.image} alt={`${vehicle.make} ${vehicle.model}`} className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="p-5">
                  <h3 className="font-semibold text-zinc-900 text-lg">
                    {vehicle.year && `${vehicle.year} `}{vehicle.make} {vehicle.model || vehicle.name || vehicle.title}
                  </h3>
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    {vehicle.mileage != null && (
                      <span className="text-sm text-zinc-400">{Number(vehicle.mileage).toLocaleString()} mi</span>
                    )}
                  </div>
                </div>
              </Link>
