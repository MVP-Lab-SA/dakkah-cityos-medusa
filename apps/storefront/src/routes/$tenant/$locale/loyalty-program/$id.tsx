...
export const Route = createFileRoute("/$tenant/$locale/loyalty-program/$id")({
  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(`${baseUrl}/store/loyalty/${params.id}`, {
      ...
      <div className="min-h-screen bg-ds-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-ds-foreground mb-2">Loyalty Program Not Found</h2>
            <p className="text-ds-muted-foreground mb-6">This loyalty program may have been removed or is no longer available.</p>
            <Link to={`${prefix}/loyalty-program` as any} className="inline-flex items-center px-4 py-2 text-sm font-medium bg-ds-primary text-ds-primary-foreground rounded-lg hover:bg-ds-primary/90 transition-colors">
      ...
      <div className="bg-ds-card border-b border-ds-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-sm text-ds-muted-foreground">
            ...
            <span>/</span>
            <Link to={`${prefix}/loyalty-program` as any} className="hover:text-ds-foreground transition-colors">Loyalty Programs</Link>
            <span>/</span>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div>
              ...
              {(program.points_per_dollar || program.pointsPerDollar) && (
                <p className="text-xl font-semibold text-ds-primary mt-3">
                  Earn {program.points_per_dollar || program.pointsPerDollar} points per $1 spent
                </p>
              )}
            </div>
            ...
            {program.description && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-3">About This Program</h2>
            ...
            {program.tiers && program.tiers.length > 0 && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-4">Membership Tiers</h2>
                <div className="space-y-3">
                  {program.tiers.map((tier: any, idx: number) => (
                    <div key={idx} className="bg-ds-muted/30 rounded-lg p-4 flex items-center justify-between">
                      <div>
                        <p className="font-medium text-ds-foreground">{tier.name || tier.title}</p>
                        {tier.threshold && <p className="text-xs text-ds-muted-foreground mt-1">{tier.threshold} points to qualify</p>}
                      </div>
                      {tier.multiplier && (
                        <span className="px-3 py-1 text-xs font-semibold rounded-full bg-ds-primary/10 text-ds-primary">{tier.multiplier}x points</span>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {program.how_to_earn && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-3">How to Earn Points</h2>
                <p className="text-ds-muted-foreground text-sm leading-relaxed whitespace-pre-wrap">{program.how_to_earn}</p>
              </div>
            )}
          </div>
            ...
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                <div className="text-center">
                  <p className="text-sm text-ds-muted-foreground mb-1">Earn Up To</p>
                  <p className="text-3xl font-bold text-ds-foreground">
                    {program.points_per_dollar || program.pointsPerDollar || "1"}x
                  </p>
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h3 className="font-semibold text-ds-foreground mb-3">Quick Facts</h3>
