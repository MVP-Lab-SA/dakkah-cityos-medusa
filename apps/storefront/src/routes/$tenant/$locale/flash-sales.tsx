...
function FlashSalesPage() {
  ...
  const nextEnding = data?.flash_sales?.reduce((earliest, sale) => {
      ...
      <div className="bg-ds-destructive text-ds-destructive-foreground">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center gap-2 text-sm opacity-80 mb-4">
            <Link to={`${prefix}` as any} className="hover:opacity-100 transition-opacity">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {error ? (
          ...
          <div className="bg-ds-background rounded-lg border border-ds-border p-12 text-center">
            ...
            <p className="text-ds-muted-foreground">No flash sales active right now</p>
            <p className="text-sm text-ds-muted-foreground mt-1">Check back soon for amazing deals!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
