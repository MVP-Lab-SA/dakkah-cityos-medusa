  ...
  return (
    <div className="min-h-screen bg-ds-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search tiers..." className="w-full max-w-md px-4 py-2.5 text-sm rounded-lg border border-ds-border bg-ds-background text-ds-foreground placeholder:text-ds-muted-foreground focus:outline-none focus:ring-2 focus:ring-ds-primary" />
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {filteredTiers.map((tier: any) => (
              <div key={tier.id} className={`relative bg-ds-background border ${tier.popular ? colorBorder[tier.color] : "border-ds-border"} rounded-xl p-6 hover:shadow-lg transition-all duration-200`}>
                {tier.popular && <span className="absolute -top-3 left-6 px-3 py-1 text-xs font-bold bg-ds-primary text-white rounded-full">Best Value</span>}
