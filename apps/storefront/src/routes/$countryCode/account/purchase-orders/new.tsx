...
export const Route = createFileRoute("/$countryCode/account/purchase-orders/new")({
...
function NewPurchaseOrderPage() {
  ...
  const handleSubmit = async (data: { items: any[]; notes?: string }) => {
    ...
    navigate({ to: `/${countryCode}/account/purchase-orders/${newPO.id}` })
  }
  ...
  const handleCancel = () => {
    navigate({ to: `/${countryCode}/account/purchase-orders` })
  }
