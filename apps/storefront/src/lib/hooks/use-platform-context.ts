...
export function usePlatformContext(tenantSlug: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/platform/context?tenant=${tenantSlug}`)
...
export function useDefaultTenant() {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/platform/tenants/default`)
...
export function usePlatformCapabilities() {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/platform/capabilities`)
