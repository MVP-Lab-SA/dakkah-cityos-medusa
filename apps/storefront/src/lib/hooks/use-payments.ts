...
export const paymentKeys = {
  ...
  walletTransactions: (page?: number) => [...paymentKeys.wallet(), "transactions", page] as const,
...
export function useWalletBalance() {
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetchApi<{ wallet: WalletBalance }>("/store/wallet/balance")
...
export function useGiftCards() {
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetchApi<{ giftCards: GiftCard[] }>("/store/gift-cards")
...
export function useStoreCreditBalance() {
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetchApi<{ credit: StoreCreditBalance }>("/store/credit/balance")
...
export function usePurchaseGiftCard() {
  ...
  return useMutation({
    mutationFn: async (data: { amount: number; recipientEmail: string; message?: string; senderName?: string }) => {
...
export function useApplyCoupon() {
  return useMutation({
    mutationFn: async (code: string) => {
      return fetchApi<{ discount: { amount: number; type: string; code: string } }>("/store/cart/coupons", {
        method: "POST",
