...
import type { Invoice, InvoiceFilters, EarlyPaymentDiscount } from "@/lib/types/invoices"
...
export function useInvoices(filters?: InvoiceFilters) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await sdk.client.fetch<{ invoices: Invoice[]; count: number }>(
        `/store/invoices?${params}`,
        { credentials: "include" }
      )
      return response
    },
...
export function useEarlyPaymentDiscount(invoiceId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch<{ discount: EarlyPaymentDiscount }>(
