describe("Job Configs and Execution", () => {
  ...
  describe("Booking Reminders Job", () => {
    ...
    it("should handle no bookings needing reminders", async () => {
      ...
      container.resolve = jest.fn((name: string) => {
        if (name === "booking") return { listBookings: jest.fn().mockResolvedValue([]) }
        if (name === "logger") return mockLogger()
  ...
  describe("Inactive Vendor Check Job", () => {
    ...
    it("should deactivate vendor with 2+ warnings", async () => {
      ...
      const emit = jest.fn()
      const vendors = [{
        id: "v1", business_name: "Test", status: "active",
        metadata: { inactivity_warnings: 2, last_order_at: "2024-01-01", created_at: "2024-01-01" },
      }]
      const container = mockContainer({
