...
describe("registry", () => {
  describe("PLATFORM_SYSTEMS_REGISTRY", () => {
    ...
    it("has at least one active system", () => {
      const active = PLATFORM_SYSTEMS_REGISTRY.filter((s) => s.status === "active")
  ...
  describe("PLATFORM_CAPABILITIES", () => {
    ...
    it("has features section with boolean flags", () => {
      expect(typeof PLATFORM_CAPABILITIES.features.twoFactorAuth).toBe("boolean")
      expect(typeof PLATFORM_CAPABILITIES.features.rbac).toBe("boolean")
      expect(typeof PLATFORM_CAPABILITIES.features.multiTenancy).toBe("boolean")
    })
    ...
    it("localization config has expected locales", () => {
      const loc = PLATFORM_CAPABILITIES.features.localization as any
  ...
  describe("HIERARCHY_LEVELS", () => {
    it("contains all expected levels in order", () => {
      expect([...HIERARCHY_LEVELS]).toEqual(["CITY", "DISTRICT", "ZONE", "FACILITY", "ASSET"])
    })
  })
