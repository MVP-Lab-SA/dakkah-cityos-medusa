jest.mock("../../lib/event-dispatcher.js", () => ({
  dispatchEventToTemporal: jest.fn().mockResolvedValue(undefined),
}), { virtual: true })

import { CircuitBreaker, RateLimiter, OutboxProcessor, getCircuitBreakerStates } from "../../../src/lib/platform/outbox-processor"
...
describe("getCircuitBreakerStates", () => {
  it("returns states for all managed systems", () => {
    ...
    expect(Object.keys(states)).toEqual(
      expect.arrayContaining(["stripe", "erpnext", "fleetbase", "payload-cms", "waltid"])
    )
