...
describe("vendor-approved subscriber", () => {
  ...
  it("sends email and admin notification", async () => {
    ...
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "vendor-approved", to: "vendor@test.com" })
    )
  })
  ...
  it("handles errors gracefully", async () => {
    ...
    await expect(vendorApprovedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("vendor-suspended subscriber", () => {
  ...
  it("skips email when vendor has no contact_email", async () => {
    mockGraph.mockResolvedValue({ data: [{ ...fullVendor, contact_email: null }] })
  ...
  it("handles errors gracefully", async () => {
    mockGraph.mockRejectedValue(new Error("fail"))
    await expect(vendorSuspendedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
