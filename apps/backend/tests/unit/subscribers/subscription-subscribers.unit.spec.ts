...
describe("subscription-cancelled subscriber", () => {
  ...
  it("sends email and admin notification", async () => {
    await subscriptionCancelledHandler(makeArgs({ id: "sub_1", reason: "too expensive" }))
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "subscription-cancelled", to: "sub@test.com" })
    )
  })

  it("handles errors gracefully", async () => {
    mockRetrieveSubscription.mockRejectedValue(new Error("fail"))
    await expect(subscriptionCancelledHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("subscription-created subscriber", () => {
  ...
  it("returns early when no customer email", async () => {
    mockRetrieveSubscription.mockResolvedValue({ ...fullSubscription, customer: {}, metadata: {} })
    await subscriptionCreatedHandler(makeArgs({ id: "sub_1" }))
    expect(mockCreateNotifications).not.toHaveBeenCalled()
  })
  ...
  it("handles errors gracefully", async () => {
    mockRetrieveSubscription.mockRejectedValue(new Error("fail"))
    await expect(subscriptionCreatedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("subscription-paused subscriber", () => {
  ...
  it("sends email and admin notification", async () => {
    await subscriptionPausedHandler(makeArgs({ id: "sub_1", reason: "vacation" }))
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
  ...
  it("handles errors gracefully", async () => {
    mockRetrieveSubscription.mockRejectedValue(new Error("fail"))
    await expect(subscriptionPausedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("subscription-payment-failed subscriber", () => {
  ...
  it("sends email with retry info and admin notification", async () => {
    await subscriptionPaymentFailedHandler(makeArgs({ id: "sub_1", error: "card declined", retry_count: 2 }))
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({
        template: "subscription-payment-failed",
        data: expect.objectContaining({ retry_count: 2, max_retries: 3, will_retry: true }),
      })
    )
  })
  ...
  it("handles errors gracefully", async () => {
    mockRetrieveSubscription.mockRejectedValue(new Error("fail"))
    await expect(subscriptionPaymentFailedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("subscription-plan-changed subscriber", () => {
  ...
  it("sends email and admin notification with plan names", async () => {
    await subscriptionPlanChangedHandler(makeArgs({ id: "sub_1", old_plan_id: "plan_old", new_plan_id: "plan_new" }))
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({
        template: "subscription-plan-changed",
        data: expect.objectContaining({ old_plan_name: "Basic Plan", new_plan_name: "Pro Plan" }),
      })
    )
  })
  ...
  it("handles missing old plan gracefully", async () => {
    ...
    await subscriptionPlanChangedHandler(makeArgs({ id: "sub_1", old_plan_id: "gone" }))
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({
        template: "subscription-plan-changed",
  ...
  it("handles errors gracefully", async () => {
    mockRetrieveSubscription.mockRejectedValue(new Error("fail"))
    await expect(subscriptionPlanChangedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("subscription-renewal-upcoming subscriber", () => {
  ...
  it("sends email reminder with days until renewal", async () => {
    await subscriptionRenewalUpcomingHandler(makeArgs({ id: "sub_1", days_until_renewal: 3 }))
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("handles errors gracefully", async () => {
    mockRetrieveSubscription.mockRejectedValue(new Error("fail"))
    await expect(subscriptionRenewalUpcomingHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("subscription-resumed subscriber", () => {
  ...
  it("handles errors gracefully", async () => {
    mockRetrieveSubscription.mockRejectedValue(new Error("fail"))
    await expect(subscriptionResumedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
