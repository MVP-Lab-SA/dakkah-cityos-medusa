...
describe("payment-authorized subscriber", () => {
  ...
  it("handles errors gracefully", async () => {
    ...
    await expect(paymentAuthorizedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("payment-captured subscriber", () => {
  ...
  it("sends email and admin notification on capture", async () => {
    ...
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "payment-confirmed", to: "buyer@test.com" })
    )
  })
  ...
  it("handles errors gracefully", async () => {
    mockGraph.mockRejectedValue(new Error("fail"))
    await expect(paymentCapturedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("payment-failed subscriber", () => {
  ...
  it("sends email and admin notification on failure", async () => {
    await paymentFailedHandler(makeArgs({ id: "pay_1", error: "card declined" }))
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "payment-failed", to: "buyer@test.com" })
    )
  })

  it("handles errors gracefully", async () => {
    mockGraph.mockRejectedValue(new Error("fail"))
    await expect(paymentFailedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("payment-refunded subscriber", () => {
  ...
  it("sends email and admin notification on refund", async () => {
    ...
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "payment-refunded", to: "buyer@test.com" })
    )
  })

  it("handles errors gracefully", async () => {
    mockGraph.mockRejectedValue(new Error("fail"))
    await expect(paymentRefundedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
