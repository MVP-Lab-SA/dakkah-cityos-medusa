...
const fullOrder = {
  ...
  fulfillments: [
    { id: "ful_1", tracking_numbers: ["TRACK123"], tracking_links: ["https://track.test"], provider_id: "fedex" },
  ],
}
...
describe("order-cancelled subscriber", () => {
  ...
  it("sends email and admin notification", async () => {
    await orderCancelledHandler(makeArgs({ id: "order_1", reason: "out of stock" }))
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "order-cancelled", to: "buyer@test.com" })
    )
  })

  it("handles errors gracefully", async () => {
    mockGraph.mockRejectedValue(new Error("fail"))
    await expect(orderCancelledHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("order-placed subscriber", () => {
  ...
  it("handles errors gracefully", async () => {
    mockGraph.mockRejectedValue(new Error("fail"))
    await expect(orderPlacedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("order-returned subscriber", () => {
  ...
  it("handles errors gracefully", async () => {
    mockGraph.mockRejectedValue(new Error("fail"))
    await expect(orderReturnedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("order-shipped subscriber", () => {
  ...
  it("sends email and admin notification with tracking info", async () => {
    await orderShippedHandler(makeArgs({ id: "order_1", fulfillment_id: "ful_1" }))
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
  ...
  it("handles errors gracefully", async () => {
    mockGraph.mockRejectedValue(new Error("fail"))
    await expect(orderShippedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
