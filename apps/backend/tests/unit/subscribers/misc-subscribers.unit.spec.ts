...
import { dispatchEventToTemporal, getWorkflowForEvent } from "../../../src/lib/event-dispatcher"
...
function makeContainer() {
  return {
    resolve: jest.fn((dep: string) => {
      ...
      if (dep === "purchaseOrder") return { retrievePurchaseOrder: mockRetrievePurchaseOrder }
...
describe("company-created subscriber", () => {
  ...
  it("sends email and admin notification", async () => {
    ...
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "company-welcome", to: "acme@test.com" })
    )
  })

  it("handles errors gracefully", async () => {
    mockRetrieveCompany.mockRejectedValue(new Error("fail"))
    await expect(companyCreatedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("customer-created subscriber", () => {
  ...
  it("sends welcome email and admin notification", async () => {
    ...
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "customer-welcome", to: "user@test.com" })
    )
  })
  ...
  it("handles errors gracefully", async () => {
    ...
    await expect(customerCreatedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("payout-completed subscriber", () => {
  beforeEach(() => {
    mockGraph
      .mockResolvedValueOnce({ data: [{ id: "vendor_1", name: "V1", contact_email: "v@test.com" }] })
      .mockResolvedValueOnce({ data: [{ id: "pay_1", payout_number: "PO-001", net_amount: 10000, currency: "USD" }] })
  })
  ...
  it("sends email and admin notification", async () => {
    await payoutCompletedHandler(makeArgs({ id: "pay_1", vendor_id: "vendor_1" }))
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "payout-completed", to: "v@test.com" })
    )
  })

  it("handles errors gracefully", async () => {
    mockGraph.mockReset().mockRejectedValue(new Error("fail"))
    await expect(payoutCompletedHandler(makeArgs({ id: "x", vendor_id: "y" }))).resolves.toBeUndefined()
  })
})
...
describe("payout-failed subscriber", () => {
  beforeEach(() => {
    mockGraph
      .mockResolvedValueOnce({ data: [{ id: "vendor_1", name: "V1", contact_email: "v@test.com" }] })
      .mockResolvedValueOnce({ data: [{ id: "pay_1", payout_number: "PO-002", net_amount: 5000 }] })
  })
  ...
  it("sends email and admin notification", async () => {
    await payoutFailedHandler(makeArgs({ id: "pay_1", vendor_id: "vendor_1", error: "bank rejected" }))
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
  ...
  it("handles errors gracefully", async () => {
    mockGraph.mockReset().mockRejectedValue(new Error("fail"))
    await expect(payoutFailedHandler(makeArgs({ id: "x", vendor_id: "y" }))).resolves.toBeUndefined()
  })
})
...
describe("purchase-order-submitted subscriber", () => {
  ...
  it("sends email and admin notification", async () => {
    ...
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "purchase-order-submitted", to: "acme@test.com" })
    )
  })

  it("handles errors gracefully", async () => {
    mockRetrievePurchaseOrder.mockRejectedValue(new Error("fail"))
    await expect(purchaseOrderSubmittedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})

describe("quote-accepted subscriber", () => {
  beforeEach(() => {
    mockRetrieveQuote.mockResolvedValue({ id: "q_1", quote_number: "Q-001", company: { name: "Acme" } })
  })
  ...
  it("handles errors gracefully", async () => {
    ...
    await expect(quoteAcceptedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("quote-approved subscriber", () => {
  ...
  it("sends email and admin notification", async () => {
    ...
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "quote-approved", to: "acme@test.com" })
    )
  })
  ...
  it("handles errors gracefully", async () => {
    mockRetrieveQuote.mockRejectedValue(new Error("fail"))
    await expect(quoteApprovedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("quote-declined subscriber", () => {
  ...
  it("sends admin notification", async () => {
    await quoteDeclinedHandler(makeArgs({ id: "q_1", reason: "too expensive" }))
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "admin-ui" })
    )
  })
  ...
  it("handles errors gracefully", async () => {
    mockRetrieveQuote.mockRejectedValue(new Error("fail"))
    await expect(quoteDeclinedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("review-created subscriber", () => {
  ...
  it("sends admin notification with product name", async () => {
    ...
    await reviewCreatedHandler(makeArgs({ id: "rev_1", product_id: "prod_1", rating: 4 }))
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({
        template: "admin-ui",
        data: expect.objectContaining({ description: expect.stringContaining("4-star") }),
      })
    )
  })
  ...
  it("uses default product name when no product_id", async () => {
    ...
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ description: expect.stringContaining("Product") }),
      })
    )
  })
  ...
  it("handles errors gracefully", async () => {
    mockGraph.mockRejectedValue(new Error("fail"))
    await expect(reviewCreatedHandler(makeArgs({ id: "x", product_id: "p" }))).resolves.toBeUndefined()
  })
})
...
describe("temporal-event-bridge subscriber", () => {
  ...
  it("dispatches event to Temporal when workflow exists", async () => {
    ;(getWorkflowForEvent as jest.Mock).mockReturnValue({ workflowId: "test-wf" })
    ;(dispatchEventToTemporal as jest.Mock).mockResolvedValue({ dispatched: true, runId: "run_1" })
  ...
  it("handles dispatch errors gracefully", async () => {
    ;(getWorkflowForEvent as jest.Mock).mockReturnValue({ workflowId: "test-wf" })
    ;(dispatchEventToTemporal as jest.Mock).mockRejectedValue(new Error("connection refused"))
