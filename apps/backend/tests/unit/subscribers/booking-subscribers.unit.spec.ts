...
describe("booking-cancelled subscriber", () => {
  ...
  it("sends email and admin notification on cancellation", async () => {
    await bookingCancelledHandler(makeArgs({ id: "booking_1", reason: "conflict" }))
    ...
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "booking-cancelled", to: "test@test.com" })
    )
    expect(mockCreateNotifications).toHaveBeenCalledWith(
  ...
  it("handles errors gracefully", async () => {
    ...
    await expect(bookingCancelledHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("booking-checked-in subscriber", () => {
  ...
  it("handles errors gracefully", async () => {
    ...
    await expect(bookingCheckedInHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("booking-completed subscriber", () => {
  ...
  it("handles errors gracefully", async () => {
    mockRetrieveBooking.mockRejectedValue(new Error("fail"))
    await expect(bookingCompletedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("booking-confirmed subscriber", () => {
  ...
  it("sends email and admin notification on confirmation", async () => {
    ...
    expect(mockCreateNotifications).toHaveBeenCalledTimes(2)
    expect(mockCreateNotifications).toHaveBeenCalledWith(
      expect.objectContaining({ template: "booking-confirmed", to: "test@test.com" })
    )
  })

  it("handles errors gracefully", async () => {
    mockRetrieveBooking.mockRejectedValue(new Error("fail"))
    await expect(bookingConfirmedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
...
describe("booking-created subscriber", () => {
  ...
  it("handles errors gracefully", async () => {
    mockRetrieveBooking.mockRejectedValue(new Error("fail"))
    await expect(bookingCreatedHandler(makeArgs({ id: "x" }))).resolves.toBeUndefined()
  })
})
