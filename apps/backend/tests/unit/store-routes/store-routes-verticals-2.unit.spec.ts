...
describe("Store Routes Verticals 2", () => {
  ...
  describe("Social Commerce /store/social-commerce", () => {
    const mockService = { listLiveStreams: jest.fn(), listGroupBuys: jest.fn() }
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", type: "group_buy", platform: "instagram" } })
      const res = createRes()
      await socialCommerceGET(req as any, res)
      expect(mockService.listGroupBuys).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", platform: "instagram", status: "active" }), expect.any(Object))
    })
  ...
  describe("Warranties /store/warranties", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", coverage_type: "full", product_id: "prod_1" } })
      const res = createRes()
      await warrantiesGET(req as any, res)
      expect(mockService.listWarrantyPlans).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", coverage_type: "full", product_id: "prod_1" }), expect.any(Object))
    })
  ...
  describe("Travel /store/travel", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", destination: "Bali", city: "Ubud" } })
      const res = createRes()
      await travelGET(req as any, res)
      expect(mockService.listTravelProperties).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", destination: "Bali", city: "Ubud", is_active: true }), expect.any(Object))
    })
  ...
  describe("Classifieds /store/classifieds", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "electronics", condition: "used" } })
      const res = createRes()
      await classifiedsGET(req as any, res)
      expect(mockService.listClassifiedListings).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", category: "electronics", condition: "used", status: "active" }), expect.any(Object))
    })

    it("GET handles service errors gracefully", async () => {
      mockService.listClassifiedListings.mockRejectedValue(new Error("DB down"))
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await classifiedsGET(req as any, res)
      expect(res.status).toHaveBeenCalledWith(500)
  ...
  describe("Crowdfunding /store/crowdfunding", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "tech", status: "active" } })
      const res = createRes()
      await crowdfundingGET(req as any, res)
      expect(mockService.listCrowdfundCampaigns).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", category: "tech", status: "active" }), expect.any(Object))
    })

    it("GET handles service errors gracefully", async () => {
      mockService.listCrowdfundCampaigns.mockRejectedValue(new Error("DB down"))
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await crowdfundingGET(req as any, res)
      expect(res.status).toHaveBeenCalledWith(500)
  ...
  describe("Freelance /store/freelance", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "design", skill: "illustrator" } })
      const res = createRes()
      await freelanceGET(req as any, res)
      expect(mockService.listGigListings).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", category: "design", skill: "illustrator", status: "active" }), expect.any(Object))
    })
  ...
  describe("Government /store/government", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", department: "planning", service_type: "permit" } })
      const res = createRes()
      await governmentGET(req as any, res)
      expect(mockService.listServiceRequests).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", department: "planning", service_type: "permit" }), expect.any(Object))
    })
  ...
  describe("Financial Products /store/financial-products", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", product_type: "mortgage", term: "30" } })
      const res = createRes()
      await financialProductsGET(req as any, res)
      expect(mockService.listLoanProducts).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", product_type: "mortgage", term: 30 }), expect.any(Object))
    })
  ...
  describe("Automotive /store/automotive", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", make: "Honda", condition: "new", fuel_type: "electric" } })
      const res = createRes()
      await automotiveGET(req as any, res)
      expect(mockService.listVehicleListings).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", make: "Honda", condition: "new", fuel_type: "electric" }), expect.any(Object))
    })
  ...
  describe("Insurance /store/insurance", () => {
    ...
    it("GET returns items with pagination", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await insuranceGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })

    it("GET applies filters from query params", async () => {
      mockService.listInsuranceProducts.mockResolvedValue([])
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", insurance_type: "auto", coverage_type: "comprehensive" } })
      const res = createRes()
      await insuranceGET(req as any, res)
      expect(mockService.listInsuranceProducts).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", insurance_type: "auto", coverage_type: "comprehensive" }), expect.any(Object))
    })
