...
describe("Store Routes Verticals 1", () => {
  ...
  describe("Parking /store/parking", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", status: "active", zone_type: "covered" } })
      const res = createRes()
      await parkingGET(req as any, res)
      expect(mockService.listParkingZones).toHaveBeenCalledWith({ tenant_id: "t1", status: "active", zone_type: "covered" }, expect.any(Object))
    })
  ...
  describe("Pet Services /store/pet-services", () => {
    ...
    it("GET returns items with pagination", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await petServicesGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })

    it("GET applies filters from query params", async () => {
      mockService.listPetProfiles.mockResolvedValue([])
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", service_type: "grooming", species: "dog" } })
      const res = createRes()
      await petServicesGET(req as any, res)
      expect(mockService.listPetProfiles).toHaveBeenCalledWith({ tenant_id: "t1", service_type: "grooming", species: "dog" }, expect.any(Object))
    })
  ...
  describe("Legal /store/legal", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", specialization: "corporate", practice_area: "mergers" } })
      const res = createRes()
      await legalGET(req as any, res)
      expect(mockService.listAttorneyProfiles).toHaveBeenCalledWith({ tenant_id: "t1", specialization: "corporate", practice_area: "mergers" }, expect.any(Object))
    })
  ...
  describe("Healthcare /store/healthcare", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", specialty: "cardiology", city: "NYC" } })
      const res = createRes()
      await healthcareGET(req as any, res)
      expect(mockService.listPractitioners).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", specialty: "cardiology", city: "NYC" }), expect.any(Object))
    })
  ...
  describe("Fitness /store/fitness", () => {
    ...
    it("GET returns items with pagination", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ classes, trainers, count: 2, limit: 20, offset: 0 })
    })

    it("GET applies filters from query params", async () => {
      mockService.listClassSchedules.mockResolvedValue([])
      mockService.listTrainerProfiles.mockResolvedValue([])
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", type: "cardio", level: "beginner" } })
      const res = createRes()
      await fitnessGET(req as any, res)
      expect(mockService.listClassSchedules).toHaveBeenCalledWith({ tenant_id: "t1", type: "cardio", level: "beginner" }, expect.any(Object))
    })
  ...
  describe("Education /store/education", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "engineering", level: "advanced" } })
      const res = createRes()
      await educationGET(req as any, res)
      expect(mockService.listCourses).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", category: "engineering", level: "advanced" }), expect.any(Object))
    })
  ...
  describe("Charity /store/charity", () => {
    ...
    it("GET returns items with pagination", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ charities, campaigns, count: 2, limit: 20, offset: 0 })
    })

    it("GET applies filters from query params", async () => {
      mockService.listCharityOrgs.mockResolvedValue([])
      mockService.listDonationCampaigns.mockResolvedValue([])
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "education", is_verified: "true" } })
      const res = createRes()
      await charityGET(req as any, res)
      expect(mockService.listCharityOrgs).toHaveBeenCalledWith({ tenant_id: "t1", category: "education", is_verified: true }, expect.any(Object))
    })
  ...
  describe("Grocery /store/grocery", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "produce", is_organic: "true" } })
      const res = createRes()
      await groceryGET(req as any, res)
      expect(mockService.listFreshProducts).toHaveBeenCalledWith({ tenant_id: "t1", category: "produce", is_organic: true }, expect.any(Object))
    })
  ...
  describe("Restaurants /store/restaurants", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", city: "NYC", cuisine_type: "italian" } })
      const res = createRes()
      await restaurantsGET(req as any, res)
      expect(mockService.listRestaurants).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", city: "NYC", cuisine_types: "italian" }), expect.any(Object))
    })
  ...
  describe("Real Estate /store/real-estate", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", city: "LA", property_type: "condo", listing_type: "sale" } })
      const res = createRes()
      await realEstateGET(req as any, res)
      expect(mockService.listPropertyListings).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", city: "LA", property_type: "condo", listing_type: "sale" }), expect.any(Object))
    })
