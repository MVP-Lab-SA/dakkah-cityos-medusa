...
describe("Store Disputes Routes", () => {
  ...
  describe("POST /store/disputes", () => {
    ...
    it("returns 400 when service throws on creation", async () => {
      mockReq.body = {
        order_id: "order-1",
        reason: "test",
        description: "test",
      }
      mockDisputeService.openDispute.mockRejectedValue(new Error("Duplicate dispute"))

      await POST(mockReq, mockRes)

      expect(mockRes.status).toHaveBeenCalledWith(400)
      expect(mockRes.json).toHaveBeenCalledWith(
        expect.objectContaining({ message: "Duplicate dispute" })
      )
    })
  })
})
