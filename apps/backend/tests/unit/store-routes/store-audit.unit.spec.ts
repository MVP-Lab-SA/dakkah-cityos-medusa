...
describe("GET /store/audit", () => {
  ...
  it("returns audit logs for authenticated customer", async () => {
    const logs = [
      { id: "log-1", action: "order.created", resource_type: "order", resource_id: "ord-1", created_at: "2025-01-01", metadata: {} },
    ]
  ...
  it("filters by action when provided", async () => {
    ...
    expect(mockAuditService.searchAuditLogs).toHaveBeenCalledWith(
      "",
      expect.objectContaining({ action: "order.created", actorId: "customer-1" })
    )
  })

  it("applies pagination with defaults", async () => {
    const logs = Array.from({ length: 30 }, (_, i) => ({
      id: `log-${i}`, action: "test", resource_type: "test", resource_id: `r-${i}`, created_at: "2025-01-01", metadata: {},
    }))
    mockAuditService.searchAuditLogs.mockResolvedValue(logs)
  ...
  it("returns 400 when service throws", async () => {
    mockAuditService.searchAuditLogs.mockRejectedValue(new Error("Search failed"))
