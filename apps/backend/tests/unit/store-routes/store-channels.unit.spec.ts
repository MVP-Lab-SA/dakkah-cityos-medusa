...
describe("GET /store/channels", () => {
  ...
  it("wraps non-array result in array", async () => {
    mockChannelService.listSalesChannelMappings.mockResolvedValue({ id: "ch-1" })
  ...
  it("returns 500 when service throws", async () => {
    mockChannelService.listSalesChannelMappings.mockRejectedValue(new Error("DB down"))

    await GET(mockReq, mockRes)

    expect(mockRes.status).toHaveBeenCalledWith(500)
    expect(mockRes.json).toHaveBeenCalledWith(
      expect.objectContaining({ message: "Failed to fetch channels" })
    )
  })
})
