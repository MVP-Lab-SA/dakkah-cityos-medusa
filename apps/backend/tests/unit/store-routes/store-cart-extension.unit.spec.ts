...
describe("Store Cart Extension Routes", () => {
  ...
  beforeEach(() => {
    mockCartExtService = {
      calculateCartInsights: jest.fn().mockResolvedValue({ total_savings: 500 }),
  ...
  describe("GET /store/cart-extension", () => {
    it("returns 400 when cart_id is missing", async () => {
      ...
      expect(mockRes.json).toHaveBeenCalledWith({ message: "cart_id query parameter is required" })
    })
    ...
    it("returns cart insights for valid cart_id", async () => {
      ...
      expect(mockRes.json).toHaveBeenCalledWith({ insights: { total_savings: 500 } })
      expect(mockCartExtService.calculateCartInsights).toHaveBeenCalledWith("cart-1")
    })

    it("returns 400 when service throws", async () => {
      mockReq.query = { cart_id: "cart-bad" }
      mockCartExtService.calculateCartInsights.mockRejectedValue(new Error("Cart not found"))
  ...
  describe("POST /store/cart-extension", () => {
    it("returns 400 when cart_id is missing in body", async () => {
      mockReq.body = { action: "apply_bundle_discounts" }

      await POST(mockReq, mockRes)

      expect(mockRes.status).toHaveBeenCalledWith(400)
      expect(mockRes.json).toHaveBeenCalledWith({ message: "cart_id is required" })
    })
    ...
    it("applies bundle discounts when action is apply_bundle_discounts", async () => {
      ...
      expect(mockRes.json).toHaveBeenCalledWith({ bundle_discounts: { applied: true } })
      expect(mockCartExtService.applyBundleDiscounts).toHaveBeenCalledWith("cart-1")
    })
    ...
    it("validates cart limits when action is validate_limits", async () => {
      ...
      expect(mockRes.json).toHaveBeenCalledWith({ validation: { valid: true } })
      expect(mockCartExtService.validateCartLimits).toHaveBeenCalledWith("cart-1", "tenant-1")
    })
    ...
    it("returns 400 when service throws on POST", async () => {
      mockReq.body = { cart_id: "cart-1", action: "apply_bundle_discounts" }
      mockCartExtService.applyBundleDiscounts.mockRejectedValue(new Error("Bundle error"))

      await POST(mockReq, mockRes)

      expect(mockRes.status).toHaveBeenCalledWith(400)
      expect(mockRes.json).toHaveBeenCalledWith({ message: "Bundle error" })
    })
  })
})
