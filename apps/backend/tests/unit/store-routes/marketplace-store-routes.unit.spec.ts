import { GET as automotiveGET, POST as automotivePOST } from "../../../src/api/store/automotive/route"
import { GET as classifiedsGET, POST as classifiedsPOST } from "../../../src/api/store/classifieds/route"
import { GET as realEstateGET } from "../../../src/api/store/real-estate/route"
import { GET as rentalsGET, POST as rentalsPOST } from "../../../src/api/store/rentals/route"
import { GET as tradeInGET, POST as tradeInPOST } from "../../../src/api/store/trade-in/route"
...
describe("Marketplace Store Routes", () => {
  ...
  describe("Automotive /store/automotive", () => {
    const mockService = { listVehicleListings: jest.fn(), createVehicleListings: jest.fn() }
    ...
    it("GET passes make/model/year/condition filters", async () => {
      ...
      const req = createReq({
        ...
        query: { make: "Honda", model: "Civic", year: "2023", condition: "new" },
      })
      const res = createRes()
      await automotiveGET(req as any, res)
    ...
    it("POST creates vehicle listing", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, body: { make: "Ford" } })
      const res = createRes()
    ...
    it("POST handles error", async () => {
      mockService.createVehicleListings.mockRejectedValue(new Error("Invalid VIN"))
  ...
  describe("Classifieds /store/classifieds", () => {
    ...
    it("GET returns active classifieds", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
      expect(mockService.listClassifiedListings).toHaveBeenCalledWith(expect.objectContaining({ status: "active" }), expect.any(Object))
    })

    it("GET passes category filter", async () => {
      mockService.listClassifiedListings.mockResolvedValue([])
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { category: "electronics" } })
      const res = createRes()
      await classifiedsGET(req as any, res)
      expect(mockService.listClassifiedListings).toHaveBeenCalledWith(expect.objectContaining({ category: "electronics" }), expect.any(Object))
    })

    it("POST creates classified listing", async () => {
      const item = { id: "cl_2" }
      mockService.createClassifiedListings.mockResolvedValue(item)
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, body: { title: "Listing" } })
      const res = createRes()
  ...
  describe("Real Estate /store/real-estate", () => {
    ...
    it("GET returns active property listings", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
      expect(mockService.listPropertyListings).toHaveBeenCalledWith(expect.objectContaining({ status: "active" }), expect.any(Object))
    })
    ...
    it("GET passes city, property_type, listing_type filters", async () => {
      ...
      expect(mockService.listPropertyListings).toHaveBeenCalledWith(
        expect.objectContaining({ city: "Dubai", property_type: "villa", listing_type: "sale" }),
        expect.any(Object),
      )
    })
  ...
  describe("Rentals /store/rentals", () => {
    ...
    it("GET returns available rental products", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
      expect(mockService.listRentalProducts).toHaveBeenCalledWith(expect.objectContaining({ is_available: true }), expect.any(Object))
    })

    it("POST creates rental product", async () => {
      const item = { id: "r_2" }
      mockService.createRentalProducts.mockResolvedValue(item)
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, body: { name: "Bike" } })
      const res = createRes()
  ...
  describe("Trade-In /store/trade-in", () => {
    ...
    it("GET returns customer trade-ins", async () => {
      ...
      const res = createRes()
      await tradeInGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ trade_ins: items, count: 1 }))
    })
    ...
    it("POST creates trade-in submission", async () => {
      ...
      const req = createReq({
        scope: { resolve: jest.fn(() => mockService) },
        auth_context: { actor_id: "cust_1" },
        body: { tenant_id: "t1", make: "Toyota", model_name: "Camry", year: 2020, mileage_km: 50000, condition: "good" },
      })
      const res = createRes()
    ...
    it("POST returns 400 when missing required fields", async () => {
      const req = createReq({ auth_context: { actor_id: "cust_1" }, body: { tenant_id: "t1" } })
      const res = createRes()
      await tradeInPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(400)
    })
  ...
  describe("Social Commerce /store/social-commerce", () => {
    const mockService = { listGroupBuys: jest.fn(), listLiveStreams: jest.fn() }

    it("GET returns group buys when type=group_buy", async () => {
      const items = [{ id: "gb_1" }]
      mockService.listGroupBuys.mockResolvedValue(items)
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { type: "group_buy" } })
      const res = createRes()
      await socialCommerceGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
      expect(mockService.listGroupBuys).toHaveBeenCalledWith(expect.objectContaining({ status: "active" }), expect.any(Object))
    })
    ...
    it("GET returns live streams by default", async () => {
      ...
      await socialCommerceGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
      expect(mockService.listLiveStreams).toHaveBeenCalledWith(expect.objectContaining({ status: "live" }), expect.any(Object))
    })
  })
})
