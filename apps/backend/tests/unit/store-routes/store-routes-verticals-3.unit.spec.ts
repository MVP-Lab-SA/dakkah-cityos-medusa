...
describe("Store Routes Verticals 3", () => {
  ...
  describe("Trade-Ins /store/trade-ins", () => {
    ...
    it("GET returns items with pagination", async () => {
      ...
      const res = createRes()
      await tradeInsGET(req as any, res)
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "electronics", condition: "good" } })
      const res = createRes()
      await tradeInsGET(req as any, res)
      expect(mockService.listTradeIns).toHaveBeenCalledWith({ tenant_id: "t1", category: "electronics", condition: "good" }, expect.any(Object))
    })
  ...
  describe("Newsletters /store/newsletters", () => {
    ...
    it("GET returns items with pagination", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await newslettersGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })

    it("GET applies filters from query params", async () => {
      mockService.listNewsletters.mockResolvedValue([])
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "tech" } })
      const res = createRes()
      await newslettersGET(req as any, res)
      expect(mockService.listNewsletters).toHaveBeenCalledWith({ tenant_id: "t1", category: "tech" }, expect.any(Object))
    })
  ...
  describe("Wallet /store/wallet", () => {
    ...
    it("GET returns wallet balance for authenticated user", async () => {
      mockLoyaltyService.listLoyaltyAccounts.mockResolvedValue([{ points_balance: 100 }])
      mockPromotionExt.listGiftCardExts.mockResolvedValue([{ remaining_value: 50, is_active: true }])
      const req = createReq({
        auth_context: { actor_id: "cust_1" },
        scope: { resolve: jest.fn((name: string) => name === "loyalty" ? mockLoyaltyService : mockPromotionExt) },
      })
      const res = createRes()
    ...
    it("GET handles service errors gracefully", async () => {
      mockLoyaltyService.listLoyaltyAccounts.mockRejectedValue(new Error("DB down"))
      const req = createReq({
        auth_context: { actor_id: "cust_1" },
        scope: { resolve: jest.fn((name: string) => name === "loyalty" ? mockLoyaltyService : mockPromotionExt) },
      })
  ...
  describe("Volume Pricing /store/volume-pricing", () => {
    ...
    it("GET returns pricing rules for a product", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ rules: [expect.objectContaining({ id: "vp_1", name: "Bulk Discount" })] })
    })
  ...
  describe("Credit /store/credit", () => {
    const mockService = { listCompanyEmployees: jest.fn(), retrieveCompany: jest.fn() }

    it("GET returns credit balance for authenticated user", async () => {
      mockService.listCompanyEmployees.mockResolvedValue([{ company_id: "comp_1" }])
      mockService.retrieveCompany.mockResolvedValue({ id: "comp_1", name: "Acme", credit_limit: 10000, credit_used: 3000, payment_terms: "net30" })
      const req = createReq({
        auth_context: { actor_id: "cust_1" },
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
        credit: expect.objectContaining({ limit: 10000, used: 3000, available: 7000 }),
  ...
  describe("Try Before You Buy /store/try-before-you-buy", () => {
    ...
    it("GET applies filters from query params", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "shoes", status: "active" } })
      const res = createRes()
      await tryBeforeYouBuyGET(req as any, res)
      expect(mockService.listVendorProducts).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", category: "shoes", status: "active", type: "try_before_you_buy" }), expect.any(Object))
    })
  ...
  describe("B2B /store/b2b", () => {
    ...
    it("GET returns items with pagination", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await b2bGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })

    it("GET applies filters from query params", async () => {
      mockService.listCompanies.mockResolvedValue([])
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "manufacturing", status: "approved" } })
      const res = createRes()
      await b2bGET(req as any, res)
      expect(mockService.listCompanies).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", category: "manufacturing", status: "approved" }), expect.any(Object))
    })
  ...
  describe("Dropshipping /store/dropshipping", () => {
    ...
    it("GET applies filters from query params", async () => {
      mockService.listVendorProducts.mockResolvedValue([])
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { tenant_id: "t1", category: "electronics", supplier: "vendor_1" } })
      const res = createRes()
      await dropshippingGET(req as any, res)
      expect(mockService.listVendorProducts).toHaveBeenCalledWith(expect.objectContaining({ tenant_id: "t1", category: "electronics", vendor_id: "vendor_1", type: "dropshipping" }), expect.any(Object))
    })
