...
import { GET as crowdfundingGET, POST as crowdfundingPOST } from "../../../src/api/store/crowdfunding/route"
import { GET as digitalProductsGET, POST as digitalProductsPOST } from "../../../src/api/store/digital-products/route"
import { GET as eventsGET } from "../../../src/api/store/event-ticketing/route"
import { GET as financialProductsGET, POST as financialProductsPOST } from "../../../src/api/store/financial-products/route"
import { GET as governmentGET, POST as governmentPOST } from "../../../src/api/store/government/route"
import { GET as groceryGET } from "../../../src/api/store/grocery/route"
import { GET as membershipsGET, POST as membershipsPOST } from "../../../src/api/store/memberships/route"
import { POST as newsletterPOST } from "../../../src/api/store/newsletter/route"
...
describe("Specialized Store Routes", () => {
  ...
  describe("Charities /store/charity", () => {
    const mockService = { listCharityOrgs: jest.fn(), listDonationCampaigns: jest.fn() }
    ...
    it("GET returns charities and campaigns", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ charities, campaigns }))
    })
  ...
  describe("Crowdfunding /store/crowdfunding", () => {
    ...
    it("POST creates campaign", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, body: { title: "Project" } })
      const res = createRes()
  ...
  describe("Digital Products /store/digital-products", () => {
    ...
    it("GET returns active digital assets", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
      expect(mockService.listDigitalAssets).toHaveBeenCalledWith(expect.objectContaining({ is_active: true }), expect.any(Object))
    })

    it("POST creates digital asset", async () => {
      const item = { id: "dp_2" }
      mockService.createDigitalAssets.mockResolvedValue(item)
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, body: { name: "eBook" } })
      const res = createRes()
  ...
  describe("Events /store/event-ticketing", () => {
    ...
    it("GET returns published events", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
      expect(mockService.listEvents).toHaveBeenCalledWith(expect.objectContaining({ status: "published" }), expect.any(Object))
    })

    it("GET passes event_type filter", async () => {
      mockService.listEvents.mockResolvedValue([])
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { event_type: "concert" } })
      const res = createRes()
      await eventsGET(req as any, res)
      expect(mockService.listEvents).toHaveBeenCalledWith(expect.objectContaining({ event_type: "concert" }), expect.any(Object))
    })
  })

  describe("Financial Products /store/financial-products", () => {
    const mockService = { listLoanProducts: jest.fn(), createLoanProducts: jest.fn() }
    ...
    it("POST creates loan product", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, body: { type: "personal" } })
      const res = createRes()
  ...
  describe("Government /store/government", () => {
    const mockService = { listServiceRequests: jest.fn(), createServiceRequests: jest.fn() }
    ...
    it("POST creates service request", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, body: { type: "permit" } })
      const res = createRes()
  ...
  describe("Grocery /store/grocery", () => {
    ...
    it("GET passes is_organic filter as boolean", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { is_organic: "true" } })
      const res = createRes()
      await groceryGET(req as any, res)
      expect(mockService.listFreshProducts).toHaveBeenCalledWith(expect.objectContaining({ is_organic: true }), expect.any(Object))
    })
  })

  describe("Memberships /store/memberships", () => {
    const mockService = { listMemberships: jest.fn(), createMemberships: jest.fn() }
    ...
    it("POST creates membership", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, body: { plan: "gold" } })
      const res = createRes()
  ...
  describe("Restaurants /store/restaurants (dining)", () => {
    ...
    it("GET returns active restaurants", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
      expect(mockService.listRestaurants).toHaveBeenCalledWith(expect.objectContaining({ is_active: true }), expect.any(Object))
    })

    it("GET passes cuisine_type filter", async () => {
      mockService.listRestaurants.mockResolvedValue([])
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { cuisine_type: "italian" } })
      const res = createRes()
      await restaurantsGET(req as any, res)
      expect(mockService.listRestaurants).toHaveBeenCalledWith(expect.objectContaining({ cuisine_types: "italian" }), expect.any(Object))
    })
  })
  ...
  describe("Travel /store/travel", () => {
    ...
    it("GET returns active travel properties", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
      expect(mockService.listTravelProperties).toHaveBeenCalledWith(expect.objectContaining({ is_active: true }), expect.any(Object))
    })
  ...
  describe("Utilities /store/utilities", () => {
    ...
    it("GET passes utility_type filter", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { utility_type: "electricity" } })
      const res = createRes()
      await utilitiesGET(req as any, res)
      expect(mockService.listUtilityAccounts).toHaveBeenCalledWith(expect.objectContaining({ utility_type: "electricity" }), expect.any(Object))
    })
  })
  ...
  describe("Warranties /store/warranties", () => {
    ...
    it("GET passes is_active filter as boolean", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, query: { is_active: "true" } })
      const res = createRes()
      await warrantiesGET(req as any, res)
      expect(mockService.listWarrantyPlans).toHaveBeenCalledWith(expect.objectContaining({ is_active: true }), expect.any(Object))
    })
  })
  ...
  describe("Reviews /store/reviews", () => {
    ...
    it("POST returns 400 when missing product_id and vendor_id", async () => {
      const req = createReq({ auth_context: { actor_id: "cust_1" }, body: { rating: 5, content: "Great" } })
      const res = createRes()
      await reviewsPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(400)
    })
    ...
    it("POST creates review successfully", async () => {
      ...
      mockQuery.graph.mockResolvedValue({ data: [{ first_name: "John", last_name: "Doe", email: "john@test.com" }] })
      const req = createReq({
        scope: {
