...
describe("Fleet Dispatch Workflow", () => {
  ...
  it("should prepare order for dispatch", async () => {
    const result = await prepareOrderForDispatchStep({
      orderId: "o1", pickupAddress: "123 Main St", deliveryAddress: "456 Elm St",
      packageWeight: 5, priority: "express", tenantId: "t1",
    })
  ...
  it("should find an available driver", async () => {
    const result = await findAvailableDriverStep({ pickupAddress: "123 Main St", priority: "express" })
  ...
  it("should assign driver to order", async () => {
    const result = await assignDriverStep({ orderId: "o1", driverId: "d1" })
  ...
  it("should initialize delivery tracking", async () => {
    const result = await initializeTrackingStep({ orderId: "o1", driverId: "d1" })
...
describe("Inventory Replenishment Workflow", () => {
  ...
  beforeAll(async () => {
    ...
    calculateOrderQuantityStep = calls.find((c: any) => c[0] === "calculate-reorder-quantity-step")?.[1]
    createPurchaseOrderStep = calls.find((c: any) => c[0] === "create-purchase-order-step")?.[1]
  })

  it("should trigger alert when stock below reorder point", async () => {
    const result = await checkStockAlertStep({
      productId: "p1", variantId: "v1", locationId: "l1",
      currentStock: 5, reorderPoint: 10, reorderQuantity: 50,
    })
  ...
  it("should throw when stock above reorder point", async () => {
    await expect(checkStockAlertStep({
      productId: "p1", variantId: "v1", locationId: "l1",
      currentStock: 15, reorderPoint: 10, reorderQuantity: 50,
  ...
  it("should calculate reorder quantity", async () => {
    const result = await calculateOrderQuantityStep({
      productId: "p1", variantId: "v1", locationId: "l1",
      currentStock: 3, reorderPoint: 10, reorderQuantity: 50,
    })
  ...
  it("should calculate larger quantity when deficit exceeds reorder quantity", async () => {
    const result = await calculateOrderQuantityStep({
      productId: "p1", variantId: "v1", locationId: "l1",
      currentStock: 0, reorderPoint: 100, reorderQuantity: 50,
    })
  ...
  it("should create a purchase order", async () => {
    ...
    const result = await createPurchaseOrderStep({ variantId: "v1", quantity: 50, locationId: "l1", supplierId: "s1" }, { container })
...
describe("Dispute Resolution Workflow", () => {
  ...
  it("should open a dispute", async () => {
    ...
    const container = mockContainer({ dispute: { createDisputes: jest.fn().mockResolvedValue(dispute) } })
    const result = await openDisputeStep({
      orderId: "o1", customerId: "c1", vendorId: "v1",
      reason: "defective", description: "Item broken",
    }, { container })
  ...
  it("should review a dispute", async () => {
    ...
    const container = mockContainer({ dispute: { updateDisputes: jest.fn().mockResolvedValue(updated) } })
  ...
  it("should resolve a dispute", async () => {
    ...
    const container = mockContainer({ dispute: { updateDisputes: jest.fn().mockResolvedValue(resolved) } })
    const result = await resolveDisputeStep({ disputeId: "d_1", resolution: "refund" }, { container })
