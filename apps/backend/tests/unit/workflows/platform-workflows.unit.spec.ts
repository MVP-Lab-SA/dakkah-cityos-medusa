...
describe("Tenant Provisioning Workflow", () => {
  ...
  it("should create a tenant with provisioning status", async () => {
    ...
    const container = mockContainer({ tenant: { createTenants: jest.fn().mockResolvedValue(tenant) } })
    const result = await createTenantStep({ name: "Acme", domain: "acme.com", adminEmail: "admin@acme.com", plan: "pro", region: "us-east" }, { container })
    expect(result.tenant.status).toBe("provisioning")
  })

  it("should provision resources for a region", async () => {
    const result = await provisionResourcesStep({ tenantId: "t_1", region: "us-east" })
  ...
  it("should seed tenant data with features", async () => {
    const result = await seedTenantDataStep({ tenantId: "t_1", features: ["booking", "loyalty"] })
    expect(result.seeded.features_enabled).toEqual(["booking", "loyalty"])
    expect(result.seeded.default_settings).toBe(true)
  })

  it("should seed with empty features when none provided", async () => {
    const result = await seedTenantDataStep({ tenantId: "t_1" })
  ...
  it("should configure tenant to active status", async () => {
    ...
    const container = mockContainer({ tenant: { updateTenants: jest.fn().mockResolvedValue(configured) } })
    const result = await configureTenantStep({ tenantId: "t_1", domain: "acme.com", plan: "pro" }, { container })
...
describe("Hierarchy Sync Workflow", () => {
  ...
  beforeAll(async () => {
    ...
    validateHierarchyStep = calls.find((c: any) => c[0] === "validate-hierarchy-change-step")?.[1]
    propagateChangeStep = calls.find((c: any) => c[0] === "propagate-hierarchy-change-step")?.[1]
    auditHierarchyChangeStep = calls.find((c: any) => c[0] === "audit-hierarchy-change-step")?.[1]
  })

  it("should detect hierarchy change", async () => {
    const node = { id: "n_1", parent_id: "p_old" }
    const container = mockContainer({ node: { retrieveNode: jest.fn().mockResolvedValue(node) } })
    const result = await detectChangeStep({ nodeId: "n_1", changeType: "move", parentId: "p_new", tenantId: "t1" }, { container })
  ...
  it("should validate hierarchy with no circular references", async () => {
    const result = await validateHierarchyStep({ nodeId: "n_1", parentId: "p_2", changeType: "move" })
  ...
  it("should propagate hierarchy change to affected nodes", async () => {
    const container = mockContainer({ node: { listNodes: jest.fn().mockResolvedValue([{ id: "child_1" }, { id: "child_2" }]) } })
    const result = await propagateChangeStep({ nodeId: "n_1", changeType: "move", parentId: "p_new" }, { container })
  ...
  it("should create audit entry for hierarchy change", async () => {
    ...
    const container = mockContainer({ audit: { createAuditEntries: jest.fn().mockResolvedValue(entry) } })
    const result = await auditHierarchyChangeStep({ nodeId: "n_1", changeType: "move", tenantId: "t1" }, { container })
...
describe("Content Moderation Workflow", () => {
  ...
  beforeAll(async () => {
    ...
    aiScanContentStep = calls.find((c: any) => c[0] === "ai-scan-content-step")?.[1]
    reviewContentStep = calls.find((c: any) => c[0] === "review-content-decision-step")?.[1]
    publishContentStep = calls.find((c: any) => c[0] === "publish-or-reject-content-step")?.[1]
  })

  it("should submit content for moderation", async () => {
    const result = await submitContentStep({ contentId: "c_1", contentType: "review", content: "Great!", authorId: "a_1", tenantId: "t1" })
    expect(result.submission.status).toBe("pending_review")
  })

  it("should scan content with AI and return low scores", async () => {
    const result = await aiScanContentStep({ content: "This is a normal review", contentId: "c_1" })
  ...
  it("should auto-approve when not flagged", async () => {
    const result = await reviewContentStep({ contentId: "c_1", scanResult: { flagged: false } })
  ...
  it("should flag for review when flagged", async () => {
    const result = await reviewContentStep({ contentId: "c_1", scanResult: { flagged: true } })
  ...
  it("should publish approved content", async () => {
    const result = await publishContentStep({ contentId: "c_1", approved: true }, { container: mockContainer() })
    expect(result.result.status).toBe("published")
  })

  it("should reject disapproved content", async () => {
    const result = await publishContentStep({ contentId: "c_1", approved: false }, { container: mockContainer() })
