...
describe("Order Fulfillment Workflow", () => {
  ...
  it("should validate an existing order", async () => {
    const container = mockContainer({ order: { retrieveOrder: jest.fn().mockResolvedValue({ id: "order_1" }) } })
  ...
  it("should throw if order not found", async () => {
    ...
    await expect(validateOrderStep(validInput, { container })).rejects.toThrow("Order order_1 not found")
  })

  it("should allocate inventory for items", async () => {
    const allocs = [{ id: "alloc_1" }]
    const container = mockContainer({ inventory: { createReservationItems: jest.fn().mockResolvedValue(allocs) } })
  ...
  it("should create a shipment", async () => {
    ...
    const container = mockContainer({ fulfillment: { createFulfillment: jest.fn().mockResolvedValue(shipment) } })
...
describe("Commission Calculation Workflow", () => {
  ...
  it("should calculate commission with custom rate", async () => {
    const container = mockContainer({ commission: { listCommissionRules: jest.fn().mockResolvedValue([{ rate: 0.15 }]) } })
  ...
  it("should use default rate 0.1 when no rules", async () => {
    const container = mockContainer({ commission: { listCommissionRules: jest.fn().mockResolvedValue([]) } })
    const result = await calculateCommissionStep(validInput, { container })
  ...
  it("should deduct commission and create transaction", async () => {
    const tx = { id: "tx_1" }
    const container = mockContainer({ commission: { createCommissionTransaction: jest.fn().mockResolvedValue(tx) } })
    const result = await deductCommissionStep({ vendorId: "v1", commissionAmount: 100, orderId: "o1" }, { container })
    expect(result).toEqual({ transaction: tx })
  })

  it("should record payout for vendor", async () => {
    const payout = { id: "payout_1" }
    const container = mockContainer({ payout: { createPayouts: jest.fn().mockResolvedValue(payout) } })
    const result = await recordPayoutStep({ vendorId: "v1", netAmount: 700, orderId: "o1" }, { container })
...
describe("Payment Reconciliation Workflow", () => {
  ...
  it("should fetch payment records", async () => {
    ...
    const container = mockContainer({ payment: { listPayments: jest.fn().mockResolvedValue(payments) } })
    const result = await fetchPaymentRecordsStep({ tenantId: "t1", dateFrom: "2025-01-01", dateTo: "2025-01-31", paymentProvider: "stripe" }, { container })
    expect(result.count).toBe(2)
  })

  it("should match all transactions", async () => {
    const payments = [{ id: "p1" }, { id: "p2" }]
    const result = await matchTransactionsStep({ payments })
    expect(result.matched).toHaveLength(2)
    expect(result.unmatchedCount).toBe(0)
  })

  it("should reconcile matched payments", async () => {
    const result = await reconcileStep({ matched: [{ id: "m1" }], tenantId: "t1" })
  ...
  it("should generate reconciliation report", async () => {
    const reconciliation = { total_reconciled: 5, status: "completed" }
    const result = await generateReportStep({ reconciliation, dateFrom: "2025-01-01", dateTo: "2025-01-31" })
...
describe("Return Processing Workflow", () => {
  ...
  it("should inspect returned items", async () => {
    const result = await inspectReturnStep({ returnRequest: { id: "ret_1" } })
  ...
  it("should process refund", async () => {
    const container = mockContainer()
    const result = await processRefundStep({ orderId: "order_1", items: [], customerId: "cust_1" }, { container })
