...
describe("Vendor Onboarding Workflow", () => {
  ...
  it("should submit vendor application with onboarding status", async () => {
    ...
    const container = mockContainer({ vendor: { createVendors: jest.fn().mockResolvedValue(vendor) } })
  ...
  it("should verify vendor documents", async () => {
    const verified = { id: "vendor_1", verification_status: "documents_verified" }
    const container = mockContainer({ vendor: { updateVendors: jest.fn().mockResolvedValue(verified) } })
    const result = await verifyDocumentsStep({ vendorId: "vendor_1" }, { container })
    expect(result.verified).toBe(true)
  })

  it("should setup vendor store", async () => {
    const store = { id: "store_1", vendor_id: "vendor_1" }
    const container = mockContainer({ store: { createStores: jest.fn().mockResolvedValue(store) } })
    const result = await setupVendorStoreStep({ vendorId: "vendor_1", tenantId: "tenant_1" }, { container })
...
describe("KYC Verification Workflow", () => {
  ...
  it("should submit KYC documents", async () => {
    const result = await submitDocumentsStep({
      vendorId: "v1", documentType: "passport", documentId: "DOC1",
      documentUrl: "https://example.com/doc.jpg", fullName: "John", dateOfBirth: "1990-01-01", country: "US",
    }, { container: mockContainer() })
    expect(result.submission.status).toBe("submitted")
  })

  it("should verify documents as valid", async () => {
    const result = await verifyDocumentsStep({ vendorId: "v1", documentUrl: "https://example.com/doc.jpg", documentType: "passport" })
  ...
  it("should score applicant as low risk when docs valid", async () => {
    const result = await scoreApplicantStep({ vendorId: "v1", verification: { document_valid: true } })
    expect(result.score).toBe(85)
    expect(result.riskLevel).toBe("low")
  })

  it("should score applicant as high risk when docs invalid", async () => {
    const result = await scoreApplicantStep({ vendorId: "v1", verification: { document_valid: false } })
    expect(result.score).toBe(30)
    expect(result.riskLevel).toBe("high")
  })

  it("should approve KYC for score >= 60", async () => {
    const result = await decideKycStep({ vendorId: "v1", score: 85, riskLevel: "low" }, { container: mockContainer() })
    expect(result.decision.approved).toBe(true)
  })

  it("should reject KYC for score < 60", async () => {
    const result = await decideKycStep({ vendorId: "v1", score: 30, riskLevel: "high" }, { container: mockContainer() })
...
describe("Product Sync Workflow", () => {
  ...
  beforeAll(async () => {
    ...
    transformProductsStep = calls.find((c: any) => c[0] === "transform-products-step")?.[1]
    upsertProductsStep = calls.find((c: any) => c[0] === "upsert-synced-products-step")?.[1]
    verifySyncStep = calls.find((c: any) => c[0] === "verify-product-sync-step")?.[1]
  })

  it("should fetch products from source", async () => {
    const products = [{ id: "p1", title: "Product 1" }]
    const container = mockContainer({ product: { listProducts: jest.fn().mockResolvedValue(products) } })
    const result = await fetchProductsStep({ sourceSystem: "medusa", targetSystem: "erp", tenantId: "t1" }, { container })
    expect(result.count).toBe(1)
  })

  it("should transform products for target system", async () => {
    const products = [{ id: "p1", title: "Prod", description: "desc", handle: "prod", status: "published" }]
    const result = await transformProductsStep({ products, targetSystem: "erp" })
  ...
  it("should upsert synced products", async () => {
    ...
    const result = await upsertProductsStep({ transformed, targetSystem: "erp" }, { container })
  ...
  it("should verify sync with no failures", async () => {
    ...
    const result = await verifySyncStep({ syncResults })
    expect(result.verified).toBe(true)
