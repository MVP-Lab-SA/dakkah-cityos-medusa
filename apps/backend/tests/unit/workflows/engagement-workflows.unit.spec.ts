...
describe("Loyalty Reward Workflow", () => {
  ...
  beforeAll(async () => {
    ...
    creditPointsStep = calls.find((c: any) => c[0] === "credit-loyalty-points-step")?.[1]
    notifyRewardStep = calls.find((c: any) => c[0] === "notify-loyalty-reward-step")?.[1]
  })

  it("should calculate points with default multiplier", async () => {
    const result = await calculatePointsStep({ customerId: "c1", orderId: "o1", orderTotal: 150, tenantId: "t1" })
    expect(result.points).toBe(150)
  })

  it("should calculate points with custom multiplier", async () => {
    const result = await calculatePointsStep({ customerId: "c1", orderId: "o1", orderTotal: 150, tenantId: "t1", pointsMultiplier: 2 })
    expect(result.points).toBe(300)
  })

  it("should credit loyalty points", async () => {
    const tx = { id: "lt_1" }
    const container = mockContainer({ loyalty: { createLoyaltyTransactions: jest.fn().mockResolvedValue(tx) } })
    const result = await creditPointsStep({ customerId: "c1", points: 100, orderId: "o1" }, { container })
...
describe("Subscription Renewal Workflow", () => {
  ...
  beforeAll(async () => {
    ...
    chargeRenewalStep = calls.find((c: any) => c[0] === "charge-renewal-step")?.[1]
    updateSubscriptionStep = calls.find((c: any) => c[0] === "update-subscription-period-step")?.[1]
  })

  it("should check active subscription status", async () => {
    const sub = { id: "sub_1", status: "active" }
    const container = mockContainer({ subscription: { retrieveSubscription: jest.fn().mockResolvedValue(sub) } })
    const result = await checkSubscriptionStep({ subscriptionId: "sub_1", customerId: "c1", planId: "p1", amount: 29, currency: "usd" }, { container })
  ...
  it("should throw if subscription not active", async () => {
    ...
    const container = mockContainer({ subscription: { retrieveSubscription: jest.fn().mockResolvedValue(sub) } })
    await expect(checkSubscriptionStep({ subscriptionId: "sub_1", customerId: "c1", planId: "p1", amount: 29, currency: "usd" }, { container }))
      .rejects.toThrow("Subscription is not active")
  })

  it("should charge renewal payment", async () => {
    const payment = { id: "pay_1" }
    const container = mockContainer({ payment: { capturePayment: jest.fn().mockResolvedValue(payment) } })
    const result = await chargeRenewalStep({ subscriptionId: "sub_1", customerId: "c1", planId: "p1", amount: 29, currency: "usd" }, { container })
    expect(result.payment).toEqual(payment)
  })

  it("should update subscription period", async () => {
    const updated = { id: "sub_1" }
    const container = mockContainer({ subscription: { updateSubscriptions: jest.fn().mockResolvedValue(updated) } })
    const result = await updateSubscriptionStep({ subscriptionId: "sub_1" }, { container })
...
describe("Auction Lifecycle Workflow", () => {
  ...
  it("should create an auction in draft status", async () => {
    ...
    const container = mockContainer({ auction: { createAuctions: jest.fn().mockResolvedValue(auction) } })
    const result = await createAuctionStep({
      productId: "p1", vendorId: "v1", startingPrice: 50, startTime: "2026-01-01T00:00:00Z",
      endTime: "2026-01-02T00:00:00Z", tenantId: "t1",
    }, { container })
  ...
  it("should open an auction to active status", async () => {
    ...
    const container = mockContainer({ auction: { updateAuctions: jest.fn().mockResolvedValue(opened) } })
  ...
  it("should close an auction", async () => {
    ...
    const container = mockContainer({ auction: { updateAuctions: jest.fn().mockResolvedValue(closed) } })
    const result = await closeAuctionStep({ auctionId: "auc_1" }, { container })
...
describe("Campaign Activation Workflow", () => {
  ...
  beforeAll(async () => {
    ...
    scheduleCampaignStep = calls.find((c: any) => c[0] === "schedule-campaign-step")?.[1]
    activateCampaignStep = calls.find((c: any) => c[0] === "activate-campaign-step")?.[1]
  })

  it("should create a campaign in draft status", async () => {
    const result = await createCampaignStep({
      name: "Summer Sale", type: "promotion", tenantId: "t1",
      startDate: "2026-06-01", endDate: "2026-06-30", targetAudience: ["all"], budget: 5000,
    })
  ...
  it("should schedule a campaign", async () => {
    ...
    const result = await scheduleCampaignStep({ campaign, startDate: "2026-06-01", endDate: "2026-06-30" })
  ...
  it("should activate a campaign with promotions", async () => {
    ...
    const result = await activateCampaignStep({ campaign, promotionIds: ["promo_1"] })
...
describe("Trade-In Evaluation Workflow", () => {
  ...
  beforeAll(async () => {
    ...
    inspectItemStep = calls.find((c: any) => c[0] === "inspect-trade-in-item-step")?.[1]
    priceItemStep = calls.find((c: any) => c[0] === "price-trade-in-item-step")?.[1]
    generateOfferStep = calls.find((c: any) => c[0] === "generate-trade-in-offer-step")?.[1]
  })

  it("should submit a trade-in", async () => {
    const result = await submitTradeInStep({
      customerId: "c1", productCategory: "phone", brand: "Apple",
      model: "iPhone 14", condition: "good", description: "Minor scratches",
    })
  ...
  it("should inspect item with excellent condition", async () => {
    const result = await inspectItemStep({ submission: {}, condition: "excellent" })
  ...
  it("should price item based on condition grade", async () => {
    const result = await priceItemStep({ brand: "Apple", model: "iPhone 14", inspection: { condition_grade: 0.7 } })
    expect(result.offerPrice).toBe(70)
    expect(result.basePrice).toBe(100)
  })

  it("should generate trade-in offer", async () => {
    const result = await generateOfferStep({ customerId: "c1", offerPrice: 70, brand: "Apple", model: "iPhone 14" })
