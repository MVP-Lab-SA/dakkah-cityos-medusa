import { GET as listPrograms, POST as createProgram } from "../../../src/api/admin/loyalty/programs/route"
import { GET as getProgram, POST as updateProgram, DELETE as deleteProgram } from "../../../src/api/admin/loyalty/programs/[id]/route"
...
describe("Admin Loyalty Programs Routes", () => {
  ...
  describe("GET /admin/loyalty/programs", () => {
    ...
    it("should return 400 on error", async () => {
      mockService.listLoyaltyPrograms.mockRejectedValue(new Error("Service down"))
      const req = createReq(mockService)
      const res = createRes()
  ...
  describe("POST /admin/loyalty/programs", () => {
    it("should create a program and return 201", async () => {
      ...
      const req = createReq(mockService, { body: { name: "Silver", points_per_dollar: 10 } })
      const res = createRes()

      await createProgram(req, res)

      expect(mockService.createLoyaltyPrograms).toHaveBeenCalledWith({ name: "Silver", points_per_dollar: 10 })
      expect(res.status).toHaveBeenCalledWith(201)
      expect(res.json).toHaveBeenCalledWith({ program })
    })

    it("should return 400 on creation error", async () => {
      mockService.createLoyaltyPrograms.mockRejectedValue(new Error("Name required"))
  ...
  describe("GET /admin/loyalty/programs/:id", () => {
    ...
    it("should return 404 when program not found", async () => {
      mockService.retrieveLoyaltyProgram.mockRejectedValue(new Error("Not found"))
  ...
  describe("POST /admin/loyalty/programs/:id (update)", () => {
    it("should update a program", async () => {
      ...
      const req = createReq(mockService, { params: { id: "prog_1" }, body: { name: "Platinum" } })
      const res = createRes()

      await updateProgram(req, res)

      expect(mockService.updateLoyaltyPrograms).toHaveBeenCalledWith("prog_1", { name: "Platinum" })
    ...
    it("should return 400 on update error", async () => {
      ...
      const req = createReq(mockService, { params: { id: "prog_1" }, body: {} })
      const res = createRes()
