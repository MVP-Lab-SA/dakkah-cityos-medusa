import { GET as getSubscription, POST as updateSubscription } from "../../../src/api/admin/subscriptions/[id]/route"
...
import { GET as listSubscriptionPlans, POST as createSubscriptionPlan } from "../../../src/api/admin/subscription-plans/route"
...
describe("Admin Subscription Detail Routes", () => {
  describe("GET /admin/subscriptions/:id", () => {
    it("should return subscription by id", async () => {
      ...
      const mockService = { listSubscriptions: jest.fn().mockResolvedValue([sub]) }
      const req = {
        scope: { resolve: jest.fn(() => mockService) },
        query: {}, params: { id: "sub_1" }, body: {},
        tenant: { id: "ten_1" },
      } as any
      const res = createRes()
    ...
    it("should return 404 if subscription not found", async () => {
      const mockService = { listSubscriptions: jest.fn().mockResolvedValue([]) }
      const req = {
        scope: { resolve: jest.fn(() => mockService) },
        query: {}, params: { id: "sub_missing" }, body: {},
        tenant: { id: "ten_1" },
      } as any
      const res = createRes()
      await getSubscription(req, res)
...
describe("Admin Subscription Pause Route", () => {
  it("should pause an active subscription", async () => {
    const mockQuery = { graph: jest.fn().mockResolvedValue({ data: [{ id: "sub_1", status: "active" }] }) }
    const mockService = { updateSubscriptions: jest.fn().mockResolvedValue({ id: "sub_1", status: "paused" }) }
    const req = {
      scope: { resolve: jest.fn((name: string) => name === "subscriptionModuleService" ? mockService : mockQuery) },
      query: {}, params: { id: "sub_1" },
      body: { reason: "Customer request" },
    } as any
    const res = createRes()
    await pauseSubscription(req, res)
    expect(mockService.updateSubscriptions).toHaveBeenCalledWith(expect.objectContaining({ status: "paused" }))
  })
  ...
  it("should reject pausing non-active subscription", async () => {
    const mockQuery = { graph: jest.fn().mockResolvedValue({ data: [{ id: "sub_1", status: "paused" }] }) }
    const req = {
      scope: { resolve: jest.fn(() => mockQuery) },
      query: {}, params: { id: "sub_1" },
      body: {},
...
describe("Admin Subscription Resume Route", () => {
  it("should resume a paused subscription", async () => {
    const mockQuery = { graph: jest.fn().mockResolvedValue({ data: [{ id: "sub_1", status: "paused" }] }) }
    const mockService = { updateSubscriptions: jest.fn().mockResolvedValue({ id: "sub_1", status: "active" }) }
    const req = {
      scope: { resolve: jest.fn((name: string) => name === "subscriptionModuleService" ? mockService : mockQuery) },
      query: {}, params: { id: "sub_1" },
      body: {},
    ...
    expect(mockService.updateSubscriptions).toHaveBeenCalledWith(expect.objectContaining({ status: "active" }))
  })
...
describe("Admin Subscription Events Route", () => {
  it("should return subscription event timeline", async () => {
    const sub = { id: "sub_1", status: "active", created_at: "2025-01-01", metadata: {} }
...
describe("Admin Subscription Plans Routes", () => {
  describe("GET /admin/subscription-plans", () => {
    it("should list all subscription plans", async () => {
      ...
      const req = { scope: { resolve: jest.fn(() => mockQuery) }, query: {}, params: {}, body: {} } as any
      const res = createRes()
      await listSubscriptionPlans(req, res)
      expect(mockQuery.graph).toHaveBeenCalledWith(expect.objectContaining({ entity: "subscription_plan" }))
