import { GET as getTenant, PUT as updateTenant } from "../../../src/api/admin/tenants/[id]/route"
...
describe("Admin Tenant Detail Routes", () => {
  describe("GET /admin/tenants/:id", () => {
    it("should return tenant by id", async () => {
      const tenant = { id: "ten_1", name: "Main", slug: "main", status: "active" }
      const mockQuery = { graph: jest.fn().mockResolvedValue({ data: [tenant] }) }
      const req = { scope: { resolve: jest.fn(() => mockQuery) }, query: {}, params: { id: "ten_1" }, body: {} } as any
      const res = createRes()
      await getTenant(req, res)
      expect(mockQuery.graph).toHaveBeenCalledWith(expect.objectContaining({ entity: "tenant", filters: { id: "ten_1" } }))
      expect(res.json).toHaveBeenCalledWith({ tenant })
    })

    it("should return 404 for non-existent tenant", async () => {
      const mockQuery = { graph: jest.fn().mockResolvedValue({ data: [undefined] }) }
      const req = { scope: { resolve: jest.fn(() => mockQuery) }, query: {}, params: { id: "ten_missing" }, body: {} } as any
      const res = createRes()
      await getTenant(req, res)
  ...
  describe("PUT /admin/tenants/:id", () => {
    it("should update tenant", async () => {
      ...
      const mockService = { updateTenants: jest.fn().mockResolvedValue(updated) }
      const req = {
        scope: { resolve: jest.fn(() => mockService) },
        query: {}, params: { id: "ten_1" },
        body: { name: "Updated" },
      } as any
      const res = createRes()
      await updateTenant(req, res)
      expect(mockService.updateTenants).toHaveBeenCalledWith({ id: "ten_1", name: "Updated" })
...
describe("Admin Tenant Limits Route", () => {
  it("should return tenant limits and usage", async () => {
    const mockQuery = {
      graph: jest.fn()
        .mockResolvedValueOnce({ data: [{ id: "ten_1", plan: "pro", limits: { max_products: 100 } }] })
...
describe("Admin Tenant Team Routes", () => {
  describe("GET /admin/tenants/:id/team", () => {
    it("should list team members", async () => {
      const members = [{ id: "mem_1", role: "admin" }, { id: "mem_2", role: "member" }]
      const mockQuery = { graph: jest.fn().mockResolvedValue({ data: members }) }
      const req = { scope: { resolve: jest.fn(() => mockQuery) }, query: {}, params: { id: "ten_1" }, body: {} } as any
      const res = createRes()
      await getTenantTeam(req, res)
      expect(mockQuery.graph).toHaveBeenCalledWith(expect.objectContaining({ filters: { tenant_id: "ten_1" } }))
  ...
  describe("PUT /admin/tenants/:id/team/:userId", () => {
    it("should update team member role", async () => {
      const mockQuery = { graph: jest.fn().mockResolvedValue({ data: [{ id: "mem_1", role: "member" }] }) }
      const mockService = { updateTenantUsers: jest.fn().mockResolvedValue({ id: "mem_1", role: "admin" }) }
      const req = {
        scope: {
          resolve: jest.fn((name: string) => name === "tenantModuleService" ? mockService : mockQuery),
        },
        query: {}, params: { id: "ten_1", userId: "user_1" },
        body: { role: "admin" },
      } as any
      const res = createRes()
    ...
    it("should return 404 if member not found", async () => {
      ...
      const req = {
        ...
        query: {}, params: { id: "ten_1", userId: "user_missing" },
        body: { role: "admin" },
      } as any
      const res = createRes()
      await updateTeamMember(req, res)
...
describe("Platform Tenants Route", () => {
  it("should list tenants for super_admin", async () => {
    const mockService = { listAndCountTenants: jest.fn().mockResolvedValue([[{ id: "ten_1" }], 1]) }
    const req = {
      scope: { resolve: jest.fn(() => mockService) },
      query: { limit: 50, offset: 0 }, params: {}, body: {},
