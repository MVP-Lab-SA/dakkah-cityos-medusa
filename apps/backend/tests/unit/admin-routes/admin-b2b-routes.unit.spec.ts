...
describe("Admin Companies Routes", () => {
  describe("GET /admin/companies", () => {
    it("should list companies with pagination", async () => {
      ...
      const mockService = { listCompanies: jest.fn().mockResolvedValue([companies, 1]) }
      const req = {
        scope: { resolve: jest.fn((name: string) => name === "tenantId" ? "ten_1" : mockService) },
  ...
  describe("GET /admin/companies (with filters)", () => {
    it("should apply status and tier filters", async () => {
      const mockService = { listCompanies: jest.fn().mockResolvedValue([[{ id: "comp_1" }], 1]) }
      const req = {
        scope: { resolve: jest.fn((name: string) => name === "tenantId" ? "ten_1" : mockService) },
        query: { status: "approved", tier: "gold", limit: 10, offset: 0 }, params: {}, body: {},
      } as any
      const res = createRes()
      await listCompanies(req, res)
...
describe("Admin Company Approve Route", () => {
  it("should approve a pending company", async () => {
    const mockService = {
      retrieveCompany: jest.fn().mockResolvedValue({ id: "comp_1", status: "pending" }),
      updateCompanies: jest.fn().mockResolvedValue({ id: "comp_1", status: "approved" }),
    }
    const req = {
      scope: { resolve: jest.fn(() => mockService) },
      query: {}, params: { id: "comp_1" },
      body: {},
  ...
  it("should reject approving non-pending company", async () => {
    const mockService = {
      retrieveCompany: jest.fn().mockResolvedValue({ id: "comp_1", status: "approved" }),
    }
    const req = {
      scope: { resolve: jest.fn(() => mockService) },
      query: {}, params: { id: "comp_1" },
...
describe("Admin Company Credit Route", () => {
  it("should return company credit details", async () => {
    const mockQuery = {
      graph: jest.fn().mockResolvedValue({ data: [{ id: "comp_1", credit_limit: "5000", credit_used: "1000" }] }),
    }
    const req = {
      scope: { resolve: jest.fn(() => mockQuery) },
      query: {}, params: { id: "comp_1" }, body: {},
    } as any
    const res = createRes()
  ...
  it("should return 404 for non-existent company", async () => {
    ...
    const req = {
      scope: { resolve: jest.fn(() => mockQuery) },
      query: {}, params: { id: "comp_missing" }, body: {},
    } as any
    const res = createRes()
    await getCompanyCredit(req, res)
...
describe("Admin Company Roles Route", () => {
  it("should return company roles and users", async () => {
    const mockQuery = {
      graph: jest.fn()
        .mockResolvedValueOnce({ data: [{ id: "comp_1", name: "Corp", metadata: {} }] })
...
describe("Admin Company Spending Limits Route", () => {
  it("should return spending limits", async () => {
    const mockQuery = {
      graph: jest.fn()
        .mockResolvedValueOnce({ data: [{ id: "comp_1", auto_approve_limit: 500 }] })
        .mockResolvedValueOnce({ data: [{ id: "cu_1", spending_limit: 1000 }] }),
    }
    const req = {
      scope: { resolve: jest.fn(() => mockQuery) },
      query: {}, params: { id: "comp_1" }, body: {},
...
describe("Admin Company Tax Exemptions Route", () => {
  it("should return tax exemptions", async () => {
    const mockQuery = { graph: jest.fn().mockResolvedValue({ data: [{ id: "ex_1", company_id: "comp_1" }] }) }
    const req = {
      scope: { resolve: jest.fn(() => mockQuery) },
      query: {}, params: { id: "comp_1" }, body: {},
    } as any
...
describe("Admin Company Workflow Route", () => {
  it("should return approval workflow config", async () => {
    const mockQuery = {
      graph: jest.fn().mockResolvedValue({ data: [{ id: "comp_1", requires_approval: true, auto_approve_limit: 500, metadata: {} }] }),
    }
    const req = {
      scope: { resolve: jest.fn(() => mockQuery) },
      query: {}, params: { id: "comp_1" }, body: {},
...
describe("Admin Purchase Orders Routes", () => {
  it("should list purchase orders", async () => {
    const mockQuery = { graph: jest.fn().mockResolvedValue({ data: [{ id: "po_1" }], metadata: { count: 1 } }) }
    const req = {
      scope: { resolve: jest.fn(() => mockQuery) },
      query: { limit: 20, offset: 0 }, params: {}, body: {},
    } as any
...
describe("Admin Quotes Routes", () => {
  describe("POST /admin/quotes/:id/reject", () => {
    it("should reject a quote", async () => {
      const mockModule = { updateQuotes: jest.fn().mockResolvedValue({ id: "q_1", status: "rejected" }) }
      const req = {
      ...
      expect(mockModule.updateQuotes).toHaveBeenCalledWith(expect.objectContaining({ status: "rejected" }))
      expect(res.json).toHaveBeenCalledWith({ quote: expect.objectContaining({ status: "rejected" }) })
    })
  })
    ...
    it("should list expiring quotes", async () => {
      const mockQuery = {
        graph: jest.fn().mockResolvedValue({
          data: [{ id: "q_1", status: "pending", valid_until: new Date(Date.now() + 86400000).toISOString() }],
        }),
      }
      const req = {
        scope: { resolve: jest.fn(() => mockQuery) },
        query: { days_until_expiry: 7 }, params: {}, body: {},
      } as any
      const res = createRes()
