import { GET as listWishlists, POST as createWishlist } from "../../../src/api/admin/wishlists/route"
import { GET as getWishlist, POST as updateWishlist, DELETE as deleteWishlist } from "../../../src/api/admin/wishlists/[id]/route"
...
describe("Admin Wishlists Routes", () => {
  ...
  describe("GET /admin/wishlists", () => {
    it("should list wishlists with default pagination", async () => {
      ...
      expect(mockService.listAndCountWishlists).toHaveBeenCalledWith({}, { take: 20, skip: 0 })
      expect(res.json).toHaveBeenCalledWith({ items, count: 2, limit: 20, offset: 0 })
    })
    ...
    it("should parse custom limit and offset", async () => {
      ...
      expect(mockService.listAndCountWishlists).toHaveBeenCalledWith({}, { take: 5, skip: 10 })
      expect(res.json).toHaveBeenCalledWith({ items: [], count: 0, limit: 5, offset: 10 })
    })

    it("should return 400 on error", async () => {
      mockService.listAndCountWishlists.mockRejectedValue(new Error("DB error"))
      const req = createReq(mockService)
      const res = createRes()
  ...
  describe("POST /admin/wishlists", () => {
    it("should create a wishlist and return 201", async () => {
      ...
      const req = createReq(mockService, { body: { customer_id: "cust_1", product_id: "prod_1" } })
      const res = createRes()

      await createWishlist(req, res)

      expect(mockService.createWishlists).toHaveBeenCalledWith({ customer_id: "cust_1", product_id: "prod_1" })
  ...
  describe("POST /admin/wishlists/:id (update)", () => {
    it("should update a wishlist", async () => {
      ...
      const req = createReq(mockService, { params: { id: "wl_1" }, body: { name: "Favorites" } })
      const res = createRes()

      await updateWishlist(req, res)

      expect(mockService.updateWishlists).toHaveBeenCalledWith("wl_1", { name: "Favorites" })
