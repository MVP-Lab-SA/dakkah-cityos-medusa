...
import { GET as listI18n, POST as createI18n } from "../../../src/api/admin/i18n/route"
import { GET as listNotificationPrefs, POST as createNotificationPref } from "../../../src/api/admin/notification-preferences/route"
...
jest.mock("../../../src/lib/temporal-client", () => ({
  checkTemporalHealth: jest.fn().mockResolvedValue({ healthy: true, namespace: "default" }),
  startWorkflow: jest.fn().mockResolvedValue({ workflowId: "wf_1", runId: "run_1" }),
}))
...
describe("Admin Booking Settings Route", () => {
  it("should return booking configuration", async () => {
    const req = { scope: { resolve: jest.fn() }, query: {}, params: {}, body: {} } as any
    const res = createRes()
    await getBookingSettings(req, res)
    expect(res.json).toHaveBeenCalledWith({ config: expect.objectContaining({ reminder_enabled: expect.any(Boolean) }) })
  })
})
...
describe("Admin I18n Routes", () => {
  describe("GET /admin/i18n", () => {
    it("should list translations with pagination", async () => {
      ...
      const mockService = { listTranslations: jest.fn().mockResolvedValue(items) }
      const req = {
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ items, count: 1 }))
    })
  })

  describe("POST /admin/i18n", () => {
    it("should create translation with valid data", async () => {
      const mockService = { createTranslations: jest.fn().mockResolvedValue({ id: "t_1" }) }
      const req = {
        scope: { resolve: jest.fn(() => mockService) },
        query: {}, params: {},
        body: { tenant_id: "ten_1", locale: "ar", key: "hello", value: "مرحبا" },
      } as any
      const res = createRes()
...
describe("Admin Notification Preferences Routes", () => {
  it("should list notification preferences", async () => {
    ...
    const mockService = { listNotificationPreferences: jest.fn().mockResolvedValue(items) }
    const req = {
      scope: { resolve: jest.fn(() => mockService) },
      query: { limit: "20", offset: "0" }, params: {}, body: {},
    } as any
...
describe("Admin Temporal Routes", () => {
  describe("GET /admin/temporal", () => {
    it("should return temporal health status", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ healthy: true }))
    })
  })
