...
import { GET as listCarriers, POST as createCarrier } from "../../../src/api/admin/shipping-ext/carriers/route"
...
describe("Admin Inventory Extension Routes", () => {
  describe("GET /admin/inventory-ext/stock-alerts", () => {
    ...
    it("should filter by alert_type", async () => {
      ...
      const req = createReq(mockService, { query: { alert_type: "out_of_stock" } })
      const res = createRes()

      await listStockAlerts(req, res)

      expect(mockService.listStockAlerts).toHaveBeenCalledWith({ alert_type: "out_of_stock" })
    })
    ...
    it("should filter by is_resolved=true", async () => {
      ...
      expect(mockService.listStockAlerts).toHaveBeenCalledWith({ is_resolved: true })
    })
    ...
    it("should filter by is_resolved=false", async () => {
      ...
      expect(mockService.listStockAlerts).toHaveBeenCalledWith({ is_resolved: false })
    })
  ...
  describe("GET /admin/inventory-ext/transfers", () => {
    ...
    it("should filter by status", async () => {
      ...
      expect(mockService.listWarehouseTransfers).toHaveBeenCalledWith({ status: "pending" })
    })
...
describe("Admin Shipping Extension Routes", () => {
  describe("GET /admin/shipping-ext/carriers", () => {
    ...
    it("should filter by is_active=true", async () => {
      ...
      expect(mockService.listCarrierConfigs).toHaveBeenCalledWith({ is_active: true })
    })
    ...
    it("should filter by is_active=false", async () => {
      ...
      expect(mockService.listCarrierConfigs).toHaveBeenCalledWith({ is_active: false })
    })
  ...
  describe("POST /admin/shipping-ext/carriers", () => {
    ...
    it("should create a carrier and return 201", async () => {
      ...
      const req = createReq(mockService, { body: { name: "UPS", code: "ups" } })
      const res = createRes()

      await createCarrier(req, res)

      expect(mockService.createCarrierConfigs).toHaveBeenCalledWith({ name: "UPS", code: "ups" })
    ...
    it("should return 400 on creation error", async () => {
      const mockService = createMockService()
      mockService.createCarrierConfigs.mockRejectedValue(new Error("Duplicate"))
