import { GET as listPages, POST as createPage } from "../../../src/api/admin/cms/pages/route"
import { GET as getPage, POST as updatePage, DELETE as deletePage } from "../../../src/api/admin/cms/pages/[id]/route"
...
describe("Admin CMS Pages Routes", () => {
  ...
  describe("GET /admin/cms/pages", () => {
    ...
    it("should filter by status", async () => {
      ...
      expect(mockService.listCmsPages).toHaveBeenCalledWith({ status: "published" })
    })
    ...
    it("should filter by search query", async () => {
      ...
      expect(mockService.listCmsPages).toHaveBeenCalledWith({ title: { $like: "%about%" } })
    })
  ...
  describe("POST /admin/cms/pages", () => {
    it("should create a page and return 201", async () => {
      ...
      const req = createReq(mockService, { body: { title: "About Us", slug: "about-us" } })
      const res = createRes()

      await createPage(req, res)

      expect(mockService.createCmsPages).toHaveBeenCalledWith({ title: "About Us", slug: "about-us" })
  ...
  describe("POST /admin/cms/pages/:id (update)", () => {
    it("should update a page", async () => {
      ...
      const req = createReq(mockService, { params: { id: "page_1" }, body: { title: "Updated" } })
      const res = createRes()

      await updatePage(req, res)

      expect(mockService.updateCmsPages).toHaveBeenCalledWith("page_1", { title: "Updated" })
    ...
    it("should return 400 on update error", async () => {
      ...
      const req = createReq(mockService, { params: { id: "page_1" }, body: {} })
      const res = createRes()
