import { GET as listDisputes, POST as createDispute } from "../../../src/api/admin/disputes/route"
import { GET as getDispute, POST as updateDispute, DELETE as deleteDispute } from "../../../src/api/admin/disputes/[id]/route"
...
describe("Admin Disputes Routes", () => {
  ...
  describe("GET /admin/disputes", () => {
    it("should list disputes with default pagination", async () => {
      ...
      expect(mockService.listDisputes).toHaveBeenCalledWith({}, { take: 20, skip: 0 })
      expect(res.json).toHaveBeenCalledWith({ disputes, count: 2, limit: 20, offset: 0 })
    })

    it("should parse custom limit and offset", async () => {
      mockService.listDisputes.mockResolvedValue([])
      const req = createReq(mockService, { query: { limit: "10", offset: "5" } })
      const res = createRes()

      await listDisputes(req, res)

      expect(mockService.listDisputes).toHaveBeenCalledWith({}, { take: 10, skip: 5 })
      expect(res.json).toHaveBeenCalledWith({ disputes: [], count: 0, limit: 10, offset: 5 })
    })
    ...
    it("should filter by status", async () => {
      ...
      expect(mockService.listDisputes).toHaveBeenCalledWith({ status: "open" }, { take: 20, skip: 0 })
    })

    it("should filter by customer_id and order_id", async () => {
      mockService.listDisputes.mockResolvedValue([])
      const req = createReq(mockService, { query: { customer_id: "cust_1", order_id: "ord_1" } })
      const res = createRes()
    ...
    it("should handle non-array result", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ disputes: [single], count: 1, limit: 20, offset: 0 })
    })
    ...
    it("should return 400 on service error", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: "Service failure" })
    })
  })
  ...
  describe("POST /admin/disputes", () => {
    it("should create a dispute and return 201", async () => {
      ...
      const req = createReq(mockService, { body: { reason: "damaged", order_id: "ord_1" } })
      const res = createRes()

      await createDispute(req, res)

      expect(mockService.createDisputes).toHaveBeenCalledWith({ reason: "damaged", order_id: "ord_1" })
      expect(res.status).toHaveBeenCalledWith(201)
      expect(res.json).toHaveBeenCalledWith({ dispute })
    })

    it("should return 400 on creation error", async () => {
      mockService.createDisputes.mockRejectedValue(new Error("Invalid data"))
  ...
  describe("POST /admin/disputes/:id (update)", () => {
    it("should update a dispute", async () => {
      ...
      const req = createReq(mockService, { params: { id: "disp_1" }, body: { status: "resolved" } })
      const res = createRes()

      await updateDispute(req, res)

      expect(mockService.updateDisputes).toHaveBeenCalledWith("disp_1", { status: "resolved" })
    ...
    it("should return 400 on update error", async () => {
      ...
      const req = createReq(mockService, { params: { id: "disp_1" }, body: {} })
      const res = createRes()
  ...
  describe("POST /admin/disputes/:id/escalate", () => {
    it("should escalate a dispute", async () => {
      ...
      const req = createReq(mockService, { params: { id: "disp_1" }, body: { reason: "unresolved" } })
      const res = createRes()
    ...
    it("should return 400 on escalation error", async () => {
      mockService.escalate.mockRejectedValue(new Error("Already escalated"))
      const req = createReq(mockService, { params: { id: "disp_1" } })
      const res = createRes()
  ...
  describe("POST /admin/disputes/:id/resolve", () => {
    it("should resolve a dispute with full body", async () => {
      ...
      const req = createReq(mockService, {
        params: { id: "disp_1" },
        body: { resolution: "refund", resolution_amount: 50, resolved_by: "admin_user", notes: "Full refund issued" },
      })
      const res = createRes()
    ...
    it("should default resolved_by to admin", async () => {
      ...
      const req = createReq(mockService, { params: { id: "disp_1" }, body: { resolution: "refund" } })
      const res = createRes()
    ...
    it("should return 400 on resolve error", async () => {
