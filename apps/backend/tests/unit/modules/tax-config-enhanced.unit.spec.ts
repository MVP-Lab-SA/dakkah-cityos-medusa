  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listTaxRules(_filter: any, _options?: any): Promise<any> { return [] }
        async retrieveTaxRule(_id: string): Promise<any> { return null }
        async createTaxRules(_data: any): Promise<any> { return {} }
        async listTaxExemptions(_filter: any, _options?: any): Promise<any> { return [] }
        async retrieveTaxExemption(_id: string): Promise<any> { return null }
        async createTaxExemptions(_data: any): Promise<any> { return {} }
  ...
  describe("getEffectiveTaxRate", () => {
    it("returns combined tax rate from applicable rules", async () => {
      jest.spyOn(service, "listTaxRules" as any).mockResolvedValue([
        { id: "r1", tax_rate: 10, region_code: "CA", status: "active", tax_type: "standard", priority: 1 },
        { id: "r2", tax_rate: 5, region_code: "CA", status: "active", tax_type: "standard", priority: 0 },
      ])
    ...
    it("returns zero rate when exempt rule applies", async () => {
      jest.spyOn(service, "listTaxRules" as any).mockResolvedValue([
        { id: "r1", tax_rate: 0, region_code: "CA", status: "active", tax_type: "exempt", priority: 10 },
      ])
    ...
    it("filters by product category", async () => {
      jest.spyOn(service, "listTaxRules" as any).mockResolvedValue([
        { id: "r1", tax_rate: 8, region_code: "CA", status: "active", tax_type: "standard", category: "food" },
        { id: "r2", tax_rate: 15, region_code: "CA", status: "active", tax_type: "standard", category: "electronics" },
      ])
