  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listUtilityAccounts(_filter: any, _options?: any): Promise<any> { return [] }
        async retrieveUtilityAccount(_id: string): Promise<any> { return null }
        async createUtilityAccounts(_data: any): Promise<any> { return {} }
        async listUtilityBills(_filter: any, _options?: any): Promise<any> { return [] }
        async retrieveUtilityBill(_id: string): Promise<any> { return null }
        async createUtilityBills(_data: any): Promise<any> { return {} }
        async updateUtilityBills(_data: any): Promise<any> { return {} }
        async listMeterReadings(_filter: any, _options?: any): Promise<any> { return [] }
        async createMeterReadings(_data: any): Promise<any> { return {} }
        async listUsageRecords(_filter: any, _options?: any): Promise<any> { return [] }
        async createUsageRecords(_data: any): Promise<any> { return {} }
      },
    model: {
      define: () => ({ indexes: () => ({}) }),
      id: chainable,
...
describe("UtilitiesModuleService – Enhanced", () => {
  ...
  describe("getActiveAccounts", () => {
    it("returns active accounts for a tenant", async () => {
      jest.spyOn(service, "listUtilityAccounts" as any).mockResolvedValue([
    ...
    it("filters by utility type when provided", async () => {
      const spy = jest.spyOn(service, "listUtilityAccounts" as any).mockResolvedValue([])
  ...
  describe("calculateUsageCharges", () => {
    it("calculates charges based on meter readings", async () => {
      jest.spyOn(service, "retrieveUtilityAccount" as any).mockResolvedValue({ id: "acc-1" })
      jest.spyOn(service, "listMeterReadings" as any).mockResolvedValue([
    ...
    it("returns zero charges when no readings exist", async () => {
      ...
      const result = await service.calculateUsageCharges("acc-1", new Date(), new Date())
  ...
  describe("generateBillingSummary", () => {
    it("generates billing summary for a given month", async () => {
      jest.spyOn(service, "retrieveUtilityAccount" as any).mockResolvedValue({
        id: "acc-1", account_number: "AN-001",
      })
      jest.spyOn(service, "listUtilityBills" as any).mockResolvedValue([
  ...
  describe("detectAnomalousUsage", () => {
    it("detects anomalous usage when recent consumption exceeds threshold", async () => {
      ...
      jest.spyOn(service, "listMeterReadings" as any)
        .mockResolvedValueOnce([
          { reading_value: 100, reading_date: new Date(Date.now() - 80 * 86400000).toISOString() },
          { reading_value: 200, reading_date: new Date(Date.now() - 40 * 86400000).toISOString() },
        ])
        .mockResolvedValueOnce([
          { reading_value: 200, reading_date: new Date(Date.now() - 20 * 86400000).toISOString() },
  ...
  describe("getServiceOutages", () => {
    it("detects outages from reading gaps with no consumption change", async () => {
      jest.spyOn(service, "listUtilityAccounts" as any).mockResolvedValue([
        { id: "acc-1", account_number: "AN-001", utility_type: "electricity" },
      ])
      jest.spyOn(service, "listMeterReadings" as any).mockResolvedValue([
        { reading_value: 500, reading_date: new Date("2025-01-01").toISOString() },
        { reading_value: 500, reading_date: new Date("2025-01-05").toISOString() },
      ])

      const result = await service.getServiceOutages(
        "t-1", new Date("2025-01-01"), new Date("2025-01-31")
      )
    ...
    it("returns zero outages when readings are normal", async () => {
      ...
      jest.spyOn(service, "listMeterReadings" as any).mockResolvedValue([
        { reading_value: 500, reading_date: new Date("2025-01-01").toISOString() },
        { reading_value: 550, reading_date: new Date("2025-01-02").toISOString() },
      ])
