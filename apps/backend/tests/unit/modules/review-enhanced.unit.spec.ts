  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listReviews(_filter: any, _options?: any): Promise<any> { return [] }
        async retrieveReview(_id: string): Promise<any> { return null }
        async createReviews(_data: any): Promise<any> { return {} }
        async updateReviews(_filter: any, _data?: any): Promise<any> { return {} }
  ...
  describe("getReviewAnalytics", () => {
    it("returns analytics with rating distribution and response rate", async () => {
      jest.spyOn(service, "listReviews" as any).mockResolvedValue([
        { rating: 5, is_verified_purchase: true, vendor_response: "Thanks!" },
    ...
    it("calculates response rate correctly", async () => {
      jest.spyOn(service, "listReviews" as any).mockResolvedValue([
        { rating: 5, vendor_response: "Thank you" },
        { rating: 4, vendor_response: "Appreciated" },
        { rating: 3 },
        { rating: 2 },
      ])
  ...
  describe("flagInappropriateReview", () => {
    it("flags a review and returns flag count", async () => {
      jest.spyOn(service, "retrieveReview" as any).mockResolvedValue({
        id: "r1", flags: [], metadata: {},
      })
      jest.spyOn(service, "updateReviews" as any).mockResolvedValue({})

      const result = await service.flagInappropriateReview("r1", "spam", "reporter-1")
    ...
    it("triggers moderation after 3 flags", async () => {
      ...
      const result = await service.flagInappropriateReview("r1", "harassment", "reporter-3")
    ...
    it("throws when reporter already flagged the review", async () => {
      ...
      await expect(service.flagInappropriateReview("r1", "spam", "reporter-1"))
        .rejects.toThrow("You have already flagged this review")
    })

    it("throws when required parameters are missing", async () => {
      await expect(service.flagInappropriateReview("", "reason", "reporter"))
