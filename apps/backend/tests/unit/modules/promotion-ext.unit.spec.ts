  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listGiftCardExts(_filter: any, _options?: any): Promise<any> { return [] }
        ...
        async listReferrals(_filter: any, _options?: any): Promise<any> { return [] }
        async listProductBundles(_filter: any, _options?: any): Promise<any> { return [] }
        async listCustomerSegments(_filter: any, _options?: any): Promise<any> { return [] }
      },
    model: {
      define: () => ({ indexes: () => ({}) }),
      id: chainable,
...
describe("PromotionExtModuleService", () => {
  ...
  describe("validatePromotionRules", () => {
    it("returns valid for an active promotion with budget", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        id: "promo-1", is_active: true, remaining_value: 5000,
        expires_at: new Date(Date.now() + 86400000).toISOString(),
      })

      const result = await service.validatePromotionRules("promo-1", { totalAmount: 1000 })
      expect(result.isValid).toBe(true)
    })

    it("returns invalid for inactive promotion", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        id: "promo-1", is_active: false, remaining_value: 5000,
      })
    ...
    it("returns partial apply when budget is less than cart total", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        id: "promo-1", is_active: true, remaining_value: 500,
      })
  ...
  describe("calculateStackedDiscounts", () => {
    it("stacks discounts from multiple promotions", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any)
        .mockResolvedValueOnce({ id: "p1", is_active: true, remaining_value: 100 })
        .mockResolvedValueOnce({ id: "p2", is_active: true, remaining_value: 200 })
    ...
    it("skips inactive promotions when stacking", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any)
        .mockResolvedValueOnce({ id: "p1", is_active: false, remaining_value: 100 })
        .mockResolvedValueOnce({ id: "p2", is_active: true, remaining_value: 200 })
  ...
  describe("getCustomerEligiblePromotions", () => {
    it("returns eligible promotions for customer", async () => {
      ...
      const result = await service.getCustomerEligiblePromotions("cust-1", "t-1")
    ...
    it("excludes customer from excluded list", async () => {
      jest.spyOn(service, "listGiftCardExts" as any).mockResolvedValue([
        { id: "p1", is_active: true, remaining_value: 100, metadata: { excluded_customers: ["cust-1"] } },
      ])
  ...
  describe("trackRedemption", () => {
    it("tracks a redemption successfully", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        id: "p1", is_active: true, metadata: { redemptions: [] },
      })
      jest.spyOn(service as any, "updateGiftCardExts").mockResolvedValue({})
    ...
    it("throws when promotion is not active", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        id: "p1", is_active: false, metadata: {},
      })

      await expect(service.trackRedemption("p1", "order-1", "cust-1"))
