  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listNotificationPreferences(_filter: any): Promise<any> { return [] }
        async retrieveNotificationPreference(_id: string): Promise<any> { return null }
        async createNotificationPreferences(_data: any): Promise<any> { return {} }
        async updateNotificationPreferences(_data: any): Promise<any> { return {} }
        async deleteNotificationPreferences(_id: string): Promise<any> { return {} }
      },
    model: {
      define: () => ({ indexes: () => ({}) }),
      id: chainable,
...
describe("NotificationPreferencesModuleService", () => {
  ...
  describe("getByCustomer", () => {
    it("returns preferences for a customer", async () => {
      const prefs = [
        { id: "pref-1", channel: "email", event_type: "order_update", enabled: true },
        { id: "pref-2", channel: "sms", event_type: "shipping", enabled: false },
      ]
      jest.spyOn(service, "listNotificationPreferences" as any).mockResolvedValue(prefs)
    ...
    it("returns empty array when no preferences exist", async () => {
      jest.spyOn(service, "listNotificationPreferences" as any).mockResolvedValue([])
  ...
  describe("updatePreference", () => {
    it("updates an existing preference", async () => {
      jest.spyOn(service, "listNotificationPreferences" as any).mockResolvedValue([
        { id: "pref-1", channel: "email", event_type: "order_update", enabled: true, frequency: "immediate" },
      ])
      const updateSpy = jest.spyOn(service as any, "updateNotificationPreferences").mockResolvedValue({ id: "pref-1", enabled: false })
      ...
      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({ id: "pref-1", enabled: false }))
    })

    it("creates a new preference when none exists", async () => {
      jest.spyOn(service, "listNotificationPreferences" as any).mockResolvedValue([])
      const createSpy = jest.spyOn(service as any, "createNotificationPreferences").mockResolvedValue({ id: "pref-new" })
  ...
  describe("getEnabledChannelsForEvent", () => {
    it("returns only enabled channels for an event type", async () => {
      ...
      const result = await service.getEnabledChannelsForEvent("cust-1", "tenant-1", "order_update")
  ...
  describe("updateChannelPreference", () => {
    it("throws on invalid channel", async () => {
      await expect(service.updateChannelPreference("cust-1", "pigeon", true))
    ...
    it("updates all preferences for a valid channel", async () => {
      ...
      const updateSpy = jest.spyOn(service as any, "updateNotificationPreferences").mockResolvedValue({})

      const result = await service.updateChannelPreference("cust-1", "email", false)
  ...
  describe("updateCategoryPreference", () => {
    it("throws on invalid category", async () => {
      await expect(service.updateCategoryPreference("cust-1", "unknown", true))
  ...
  describe("shouldNotify", () => {
    it("returns true when preference is enabled", async () => {
      ...
      const result = await service.shouldNotify("cust-1", "tenant-1", "email", "transactional")
    ...
    it("returns false when preference is disabled by user override", async () => {
      ...
      const result = await service.shouldNotify("cust-1", "tenant-1", "email", "marketing")
  ...
  describe("bulkOptOut", () => {
    it("opts out of all channels when none specified", async () => {
      ...
      jest.spyOn(service as any, "updateNotificationPreferences").mockResolvedValue({})
