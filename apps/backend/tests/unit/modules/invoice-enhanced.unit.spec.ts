jest.mock("@medusajs/framework/utils", () => {
  ...
  return {
    MedusaService: () => class MockMedusaBase {
      async listInvoices(_filter: any): Promise<any> { return [] }
      async retrieveInvoice(_id: string): Promise<any> { return null }
      async createInvoices(_data: any): Promise<any> { return {} }
      async updateInvoices(_data: any): Promise<any> { return {} }
      async listInvoiceItems(_filter: any): Promise<any> { return [] }
...
describe("InvoiceModuleService – Enhanced Methods", () => {
  ...
  describe("calculateInvoiceTotals", () => {
    it("calculates totals from invoice items", async () => {
      jest.spyOn(service, "retrieveInvoice" as any).mockResolvedValue({
        id: "inv-1", amount_paid: 0,
      })
      jest.spyOn(service, "listInvoiceItems" as any).mockResolvedValue([
        { quantity: 2, unit_price: 1000, tax_total: 200 },
        { quantity: 1, unit_price: 500, tax_total: 50 },
      ])
      const updateSpy = jest.spyOn(service as any, "updateInvoices").mockResolvedValue({})
    ...
    it("throws when invoice not found", async () => {
      jest.spyOn(service, "retrieveInvoice" as any).mockResolvedValue(null)
      await expect(service.calculateInvoiceTotals("inv-bad")).rejects.toThrow("not found")
    })
  })
  ...
  describe("markOverdue", () => {
    it("marks overdue invoices and returns count", async () => {
      ...
      jest.spyOn(service as any, "updateInvoices").mockResolvedValue({})
  ...
  describe("getPaymentSummary", () => {
    it("returns fully paid summary", async () => {
      jest.spyOn(service, "retrieveInvoice" as any).mockResolvedValue({
        id: "inv-1", status: "paid", total: 1000, amount_paid: 1000,
        paid_at: "2025-01-15", due_date: "2025-01-30",
      })
    ...
    it("returns partially paid summary with balance", async () => {
      jest.spyOn(service, "retrieveInvoice" as any).mockResolvedValue({
        id: "inv-1", status: "sent", total: 1000, amount_paid: 400,
        paid_at: null, due_date: "2099-12-31",
      })
    ...
    it("detects overdue invoices", async () => {
      jest.spyOn(service, "retrieveInvoice" as any).mockResolvedValue({
        id: "inv-1", status: "sent", total: 500, amount_paid: 0,
        paid_at: null, due_date: "2020-01-01",
      })
    ...
    it("throws when invoice not found", async () => {
      jest.spyOn(service, "retrieveInvoice" as any).mockResolvedValue(null)
      await expect(service.getPaymentSummary("inv-bad")).rejects.toThrow("not found")
    })
  })
})
