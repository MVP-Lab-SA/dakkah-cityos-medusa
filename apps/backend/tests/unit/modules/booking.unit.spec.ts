  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listBookings(_filter: any, _options?: any): Promise<any> { return [] }
...
describe("BookingModuleService", () => {
  ...
  describe("createBooking", () => {
    it("creates a booking when slot is available", async () => {
      jest.spyOn(service, "retrieveServiceProduct" as any).mockResolvedValue({
        id: "svc-1", duration_minutes: 60, location_type: "in_person", location_address: "123 Main St",
      })
      ...
      const createSpy = jest.spyOn(service as any, "createBookings").mockResolvedValue({ id: "bk-1", booking_number: "BK-TEST-1234" })
  ...
  describe("cancelBooking", () => {
    it("cancels a pending booking", async () => {
      jest.spyOn(service, "retrieveBooking" as any).mockResolvedValue({
        id: "bk-1", status: "pending", service_product_id: "svc-1",
        start_time: new Date(Date.now() + 48 * 60 * 60 * 1000).toISOString(), total: 10000,
      })
      jest.spyOn(service, "retrieveServiceProduct" as any).mockResolvedValue({
        id: "svc-1", cancellation_policy_hours: 24,
      })
      ...
      const result = await service.cancelBooking("bk-1", "customer", "Changed plans")
    ...
    it("throws when booking is already completed", async () => {
      ...
      await expect(service.cancelBooking("bk-1", "customer"))
    ...
    it("throws when booking is already cancelled", async () => {
      ...
      await expect(service.cancelBooking("bk-1", "admin"))
        .rejects.toThrow("Booking cannot be cancelled")
    })
  })
  ...
  describe("confirmBooking", () => {
    ...
    it("throws when booking is not pending", async () => {
      ...
      await expect(service.confirmBooking("bk-1"))
  ...
  describe("completeBooking", () => {
    ...
    it("throws when booking cannot be completed from current status", async () => {
      ...
      await expect(service.completeBooking("bk-1"))
        .rejects.toThrow("Booking cannot be completed from current status")
    })
  })
  ...
  describe("markNoShow", () => {
    ...
    it("throws when booking is not confirmed", async () => {
      ...
      await expect(service.markNoShow("bk-1"))
        .rejects.toThrow("Only confirmed bookings can be marked as no-show")
    })
  })
})
