...
describe("I18nModuleService", () => {
  ...
  describe("getTranslations", () => {
    ...
    it("filters by namespace when provided", async () => {
      const spy = jest.spyOn(service, "listTranslations" as any).mockResolvedValue([])

      await service.getTranslations("tenant-1", "en", "checkout")

      expect(spy).toHaveBeenCalledWith(
        expect.objectContaining({ namespace: "checkout", locale: "en", status: "published" })
      )
    })
  })
  ...
  describe("getTranslation", () => {
    it("returns single translation by key", async () => {
      jest.spyOn(service, "listTranslations" as any).mockResolvedValue([
        { id: "t-1", key: "welcome", value: "Hello" },
      ])
    ...
    it("returns null when translation not found", async () => {
      ...
      const result = await service.getTranslation("tenant-1", "en", "nonexistent")
  ...
  describe("upsertTranslation", () => {
    it("creates new translation when not existing", async () => {
      jest.spyOn(service, "listTranslations" as any).mockResolvedValue([])
      const createSpy = jest.spyOn(service as any, "createTranslations").mockResolvedValue({ id: "t-new" })

      await service.upsertTranslation("tenant-1", "en", "greeting", "Hi")

      expect(createSpy).toHaveBeenCalledWith(
        expect.objectContaining({ key: "greeting", value: "Hi", namespace: "common" })
      )
    })

    it("updates existing translation", async () => {
      jest.spyOn(service, "listTranslations" as any).mockResolvedValue([
        { id: "t-1", key: "greeting", value: "Hello" },
      ])
      const updateSpy = jest.spyOn(service as any, "updateTranslations").mockResolvedValue({ id: "t-1" })
  ...
  describe("bulkUpsert", () => {
    it("processes multiple translations", async () => {
      jest.spyOn(service, "listTranslations" as any).mockResolvedValue([])
      jest.spyOn(service as any, "createTranslations").mockResolvedValue({ id: "t-new" })
