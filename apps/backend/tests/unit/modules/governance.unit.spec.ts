  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listGovernanceAuthorities(_filter: any, _options?: any): Promise<any> { return [] }
        async retrieveGovernanceAuthority(_id: string): Promise<any> { return null }
        async createGovernanceAuthoritys(_data: any): Promise<any> { return {} }
        async updateGovernanceAuthoritys(_data: any): Promise<any> { return {} }
      },
    model: {
      define: () => ({ indexes: () => ({}) }),
      id: chainable,
...
describe("GovernanceModuleService", () => {
  ...
  describe("evaluatePolicyChain", () => {
    it("returns effective policy merged from authority chain", async () => {
      jest.spyOn(service, "retrieveGovernanceAuthority" as any)
        .mockResolvedValueOnce({
          id: "auth-1", type: "region", parent_authority_id: null,
          policies: { commerce: { tax: { rules: { rate: 10 }, status: "active" } } },
        })
    ...
    it("merges policies from parent to child in chain", async () => {
      jest.spyOn(service, "retrieveGovernanceAuthority" as any)
        .mockResolvedValueOnce({
          id: "auth-2", type: "country", parent_authority_id: "auth-1",
          policies: { commerce: { local_tax: { rules: { rate: 5 }, status: "active" } } },
        })
        .mockResolvedValueOnce({
          id: "auth-1", type: "region", parent_authority_id: null,
          policies: { commerce: { global_tax: { rules: { rate: 10 }, status: "active" } } },
        })
  ...
  describe("checkComplianceStatus", () => {
    it("returns compliant when all policies are satisfied", async () => {
      jest.spyOn(service, "listGovernanceAuthorities" as any).mockResolvedValue([
        {
          id: "auth-1", type: "region",
          policies: { commerce: { tax_policy: { rules: { has_tax_id: true }, status: "active" } } },
        },
      ])
    ...
    it("returns violations for non-compliant policies", async () => {
      jest.spyOn(service, "listGovernanceAuthorities" as any).mockResolvedValue([
        {
          id: "auth-1", type: "region",
          policies: { commerce: { bad_policy: { rules: { data_encrypted: false }, status: "active" } } },
        },
      ])
  ...
  describe("getPolicyAuditTrail", () => {
    it("returns audit trail for existing policy", async () => {
      jest.spyOn(service, "listGovernanceAuthorities" as any).mockResolvedValue([
        {
          id: "auth-1", type: "region", created_at: "2025-01-01T00:00:00Z",
          policies: { commerce: { tax_policy: { rules: { rate: 10 }, status: "active", created_at: "2025-01-01T00:00:00Z" } } },
        },
      ])
    ...
    it("throws when policy is not found", async () => {
      ...
      await expect(service.getPolicyAuditTrail("nonexistent"))
        .rejects.toThrow("No audit trail found")
    })
  })
})
