  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        ...
        async listMeterReadings(_filter: any): Promise<any> { return [] }
        async retrieveMeterReading(_id: string): Promise<any> { return null }
        async listUtilityBills(_filter: any): Promise<any> { return [] }
...
describe("UtilitiesModuleService", () => {
  ...
  describe("getActiveAccounts", () => {
    it("returns active accounts for a tenant", async () => {
      ...
      jest.spyOn(service, "listUtilityAccounts" as any).mockResolvedValue(accounts)
    ...
    it("filters by utility type when provided", async () => {
      const listSpy = jest.spyOn(service, "listUtilityAccounts" as any).mockResolvedValue([])
  ...
  describe("generateBill", () => {
    it("generates a bill for a billing period", async () => {
      jest.spyOn(service, "retrieveUtilityAccount" as any).mockResolvedValue({
        id: "acc-1", tenant_id: "tenant-1", provider_name: "PowerCo",
      })
      jest.spyOn(service, "listMeterReadings" as any).mockResolvedValue([
        { reading_value: 100, reading_date: "2025-01-01", unit: "kWh" },
        { reading_value: 200, reading_date: "2025-01-31", unit: "kWh" },
      ])
      const createSpy = jest.spyOn(service as any, "createUtilityBills").mockResolvedValue({ id: "bill-1" })
  ...
  describe("processPayment", () => {
    it("marks bill as paid when full amount is paid", async () => {
      jest.spyOn(service, "retrieveUtilityBill" as any).mockResolvedValue({
        id: "bill-1", amount: "500", status: "generated", tenant_id: "t1", account_id: "a1", currency_code: "USD",
      })
      const updateSpy = jest.spyOn(service as any, "updateUtilityBills").mockResolvedValue({})
      jest.spyOn(service as any, "createUsageRecords").mockResolvedValue({})

      const result = await service.processPayment("bill-1", 500)

      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({ status: "paid" }))
    ...
    it("throws when payment exceeds bill amount", async () => {
      ...
      await expect(service.processPayment("bill-1", 600)).rejects.toThrow("Payment amount exceeds bill amount")
    })
  })
  ...
  describe("getUsageSummary", () => {
    it("returns usage summary for an account", async () => {
      jest.spyOn(service, "retrieveUtilityAccount" as any).mockResolvedValue({
        id: "acc-1", account_number: "ACC001", utility_type: "electricity",
        provider_name: "PowerCo", status: "active",
      })
      jest.spyOn(service, "listMeterReadings" as any).mockResolvedValue([
        { reading_value: 100, reading_date: "2025-01-01", unit: "kWh" },
