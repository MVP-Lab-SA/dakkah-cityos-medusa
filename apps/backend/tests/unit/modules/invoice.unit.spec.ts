  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listInvoices(_filter: any, _options?: any): Promise<any> { return [[], 0] }
...
describe("InvoiceModuleService", () => {
  ...
  describe("generateInvoiceNumber", () => {
    ...
    it("increments sequence number based on existing invoices", async () => {
      jest.spyOn(service, "listInvoices" as any).mockResolvedValue([[], 5])
  ...
  describe("createInvoiceWithItems", () => {
    it("creates an invoice with items and calculates totals", async () => {
      ...
      const createInvSpy = jest.spyOn(service as any, "createInvoices").mockResolvedValue({ id: "inv-1" })
      const createItemsSpy = jest.spyOn(service as any, "createInvoiceItems").mockResolvedValue([{ id: "item-1" }])
    ...
    it("uses default currency and payment terms", async () => {
      jest.spyOn(service, "generateInvoiceNumber" as any).mockResolvedValue("INV-TEST-202501-0001")
      const createInvSpy = jest.spyOn(service as any, "createInvoices").mockResolvedValue({ id: "inv-1" })
      jest.spyOn(service as any, "createInvoiceItems").mockResolvedValue([])
  ...
  describe("markAsPaid", () => {
    ...
    it("throws when invoice is not found", async () => {
      ...
      await expect(service.markAsPaid("inv-999"))
        .rejects.toThrow("Invoice inv-999 not found")
    })
  })
