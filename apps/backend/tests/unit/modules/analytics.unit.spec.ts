  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listAnalyticsEvents(_filter: any, _config?: any): Promise<any> { return [] }
...
describe("AnalyticsModuleService", () => {
  ...
  describe("trackEvent", () => {
    it("creates an analytics event", async () => {
      const createSpy = jest.spyOn(service as any, "createAnalyticsEvents").mockResolvedValue({ id: "ev-1" })
  ...
  describe("getSalesMetrics", () => {
    it("calculates revenue and average order value", async () => {
      jest.spyOn(service, "listAnalyticsEvents" as any).mockResolvedValue([
        { event_type: "purchase", revenue: 100, created_at: "2025-01-10T00:00:00Z" },
        { event_type: "purchase", revenue: 200, created_at: "2025-01-15T00:00:00Z" },
      ])
  ...
  describe("getTopProducts", () => {
    it("returns products sorted by revenue", async () => {
      jest.spyOn(service, "listAnalyticsEvents" as any).mockResolvedValue([
        { entity_type: "product", entity_id: "p1", revenue: 100, created_at: "2025-01-10T00:00:00Z" },
        { entity_type: "product", entity_id: "p1", revenue: 200, created_at: "2025-01-11T00:00:00Z" },
        { entity_type: "product", entity_id: "p2", revenue: 500, created_at: "2025-01-12T00:00:00Z" },
      ])
  ...
  describe("trackPageView", () => {
    ...
    it("throws when required fields are missing", async () => {
      await expect(service.trackPageView({
        sessionId: "",
        pageUrl: "/products",
        tenantId: "t1",
  ...
  describe("generateReport", () => {
    it("updates report generation timestamp", async () => {
      jest.spyOn(service, "retrieveReport" as any).mockResolvedValue({ id: "r1", title: "Monthly" })
      const updateSpy = jest.spyOn(service as any, "updateReports").mockResolvedValue({})
  ...
  describe("getDashboard", () => {
    it("returns dashboard by slug", async () => {
      jest.spyOn(service, "listDashboards" as any).mockResolvedValue([
        { id: "d1", slug: "overview", tenant_id: "t1" },
      ])

      const result = await service.getDashboard("t1", "overview")
      expect(result).toEqual(expect.objectContaining({ id: "d1", slug: "overview" }))
    })

    it("throws when dashboard not found", async () => {
      jest.spyOn(service, "listDashboards" as any).mockResolvedValue([])

      await expect(service.getDashboard("t1", "nonexistent")).rejects.toThrow('Dashboard "nonexistent" not found')
    })
  })
})
