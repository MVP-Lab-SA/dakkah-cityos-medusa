  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listSalesChannelMappings(_filter: any, _options?: any): Promise<any> { return [] }
...
describe("ChannelModuleService", () => {
  ...
  describe("getChannelForRequest", () => {
    it("returns matching channel for tenant and type", async () => {
      jest.spyOn(service, "listSalesChannelMappings" as any).mockResolvedValue([
        { id: "ch-1", tenant_id: "t-1", channel_type: "web", is_active: true },
      ])
    ...
    it("falls back to channel without nodeId when nodeId match fails", async () => {
      jest.spyOn(service, "listSalesChannelMappings" as any)
        ...
        .mockResolvedValueOnce([
          { id: "ch-fallback", tenant_id: "t-1", channel_type: "web", is_active: true },
        ])
  ...
  describe("getChannelAnalytics", () => {
    it("returns analytics for a valid channel", async () => {
      jest.spyOn(service, "retrieveSalesChannelMapping" as any).mockResolvedValue({
        id: "ch-1", name: "Web Store", channel_type: "web", is_active: true,
        tenant_id: "t-1", config: {}, metadata: {},
      })
      jest.spyOn(service, "listSalesChannelMappings" as any).mockResolvedValue([
        { id: "ch-1" }, { id: "ch-2" },
      ])
    ...
    it("returns null when channel not found", async () => {
      jest.spyOn(service, "retrieveSalesChannelMapping" as any).mockResolvedValue(null)
  ...
  describe("syncChannelInventory", () => {
    it("syncs new products to channel", async () => {
      jest.spyOn(service, "retrieveSalesChannelMapping" as any).mockResolvedValue({
        id: "ch-1", is_active: true, config: { synced_products: ["p-1"] },
      })
      jest.spyOn(service as any, "updateSalesChannelMappings").mockResolvedValue({})

      const result = await service.syncChannelInventory("ch-1", ["p-1", "p-2", "p-3"])
    ...
    it("throws when channel is not active", async () => {
      jest.spyOn(service, "retrieveSalesChannelMapping" as any).mockResolvedValue({
        id: "ch-1", is_active: false, config: {},
      })

      await expect(service.syncChannelInventory("ch-1", ["p-1"]))
  ...
  describe("validateChannelConfig", () => {
    it("returns valid true when channel has all required fields", async () => {
      jest.spyOn(service, "retrieveSalesChannelMapping" as any).mockResolvedValue({
        id: "ch-1", name: "Web Store", channel_type: "web",
        tenant_id: "t-1", medusa_sales_channel_id: "msc-1",
        config: { theme: "dark" },
      })
    ...
    it("returns errors when name is missing", async () => {
      jest.spyOn(service, "retrieveSalesChannelMapping" as any).mockResolvedValue({
        id: "ch-1", name: "", channel_type: "web",
        tenant_id: "t-1", config: {},
      })
