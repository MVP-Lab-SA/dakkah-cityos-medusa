  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        ...
        async listCartMetadatas(_filter: any, _options?: any): Promise<any> { return [] }
        async retrieveCartMetadata(_id: string): Promise<any> { return null }
        async createCartMetadatas(_data: any): Promise<any> { return {} }
        async updateCartMetadatas(_data: any): Promise<any> { return {} }
      },
    model: {
      define: () => ({ indexes: () => ({}) }),
      id: chainable,
...
describe("CartExtensionModuleService", () => {
  ...
  describe("calculateCartInsights", () => {
    it("returns insights with savings and recommendations", async () => {
      ;(service as any).manager_ = {
        findOne: jest.fn().mockResolvedValue({
          id: "cart-1",
          items: [
            { id: "i1", unit_price: 1000, quantity: 2, compare_at_unit_price: 1500 },
    ...
    it("suggests bundle discount when 2 items present", async () => {
      ...
      const result = await service.calculateCartInsights("cart-1")
      const bundleSuggestion = result.recommendations.find(r => r.type === "bundle_suggestion")
    ...
    it("detects category bundles and applies discount", async () => {
      ;(service as any).manager_ = {
        findOne: jest.fn().mockResolvedValue({
          id: "cart-1",
          items: [
            { id: "i1", unit_price: 1000, quantity: 1, metadata: { category: "electronics" } },
            { id: "i2", unit_price: 2000, quantity: 1, metadata: { category: "electronics" } },
            { id: "i3", unit_price: 500, quantity: 1, metadata: { category: "books" } },
          ],
        }),
      }
    ...
    it("applies mix-and-match discount for 3+ items", async () => {
      ;(service as any).manager_ = {
        findOne: jest.fn().mockResolvedValue({
          id: "cart-1",
          items: [
            { id: "i1", unit_price: 1000, quantity: 1, metadata: { category: "a" } },
            { id: "i2", unit_price: 1000, quantity: 1, metadata: { category: "b" } },
            { id: "i3", unit_price: 1000, quantity: 1, metadata: { category: "c" } },
          ],
        }),
      }

      const result = await service.applyBundleDiscounts("cart-1")
      const mixMatch = result.bundlesDetected.find(b => b.name === "mix_and_match")
    ...
    it("returns valid when cart is within all limits", async () => {
      ;(service as any).manager_ = {
        findOne: jest.fn().mockResolvedValue({
          id: "cart-1",
          items: [
            { id: "i1", unit_price: 1000, quantity: 2, weight: 500 },
          ],
        }),
      }
    ...
    it("returns invalid when item count exceeds limit", async () => {
      ;(service as any).manager_ = {
        findOne: jest.fn().mockResolvedValue({
          id: "cart-1",
          items: [
            { id: "i1", unit_price: 100, quantity: 60, weight: 10 },
          ],
        }),
      }
