  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listCommissionRules(_filter: any): Promise<any> { return [] }
...
describe("CommissionModuleService", () => {
  ...
  describe("calculateCommission", () => {
    it("calculates percentage-based commission", async () => {
      jest.spyOn(service, "listCommissionRules" as any).mockResolvedValue([{
        id: "rule-1",
    ...
    it("throws when no commission rule found", async () => {
      ...
      await expect(service.calculateCommission({
        vendorId: "v1",
        orderId: "o1",
        lineItemId: "li1",
        orderSubtotal: 900,
  ...
  describe("createCommissionTransaction", () => {
    it("creates a transaction with calculated commission", async () => {
      jest.spyOn(service, "calculateCommission" as any).mockResolvedValue({
      ...
      const createSpy = jest.spyOn(service, "createCommissionTransactions" as any).mockResolvedValue({ id: "tx-1" })
    ...
    it("includes store ID when provided", async () => {
      jest.spyOn(service, "calculateCommission" as any).mockResolvedValue({
        commissionAmount: 50, commissionRate: 5, commissionFlat: null, netAmount: 950, ruleId: "rule-1",
      })
      const createSpy = jest.spyOn(service, "createCommissionTransactions" as any).mockResolvedValue({ id: "tx-1" })
