  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listAuctionListings(_filter: any): Promise<any> { return [] }
        async retrieveAuctionListing(_id: string): Promise<any> { return null }
        async createAuctionListings(_data: any): Promise<any> { return {} }
...
describe("AuctionModuleService", () => {
  ...
  describe("placeBid", () => {
    it("places a valid bid on an active auction", async () => {
      jest.spyOn(service, "retrieveAuctionListing" as any).mockResolvedValue({
        id: "auc-1", status: "active", ends_at: "2099-12-31", bid_increment: 10, starting_price: 100,
      })
      jest.spyOn(service, "getHighestBid" as any).mockResolvedValue(null)
      const createSpy = jest.spyOn(service as any, "createBids").mockResolvedValue({ id: "bid-1" })
    ...
    it("throws when bid amount is zero", async () => {
      await expect(service.placeBid("auc-1", "user-1", 0)).rejects.toThrow("Bid amount must be greater than zero")
    })

    it("throws when auction is not active", async () => {
      jest.spyOn(service, "retrieveAuctionListing" as any).mockResolvedValue({
        id: "auc-1", status: "ended", ends_at: "2099-12-31",
      })

      await expect(service.placeBid("auc-1", "user-1", 100)).rejects.toThrow("Auction is not active")
    })

    it("throws when auction has ended by time", async () => {
      jest.spyOn(service, "retrieveAuctionListing" as any).mockResolvedValue({
        id: "auc-1", status: "active", ends_at: "2020-01-01",
      })

      await expect(service.placeBid("auc-1", "user-1", 100)).rejects.toThrow("Auction has ended")
    })

    it("throws when bid is below minimum increment", async () => {
      jest.spyOn(service, "retrieveAuctionListing" as any).mockResolvedValue({
        id: "auc-1", status: "active", ends_at: "2099-12-31", bid_increment: 10,
      })
      jest.spyOn(service, "getHighestBid" as any).mockResolvedValue({ id: "bid-0", amount: 100 })

      await expect(service.placeBid("auc-1", "user-1", 105)).rejects.toThrow("Bid must be at least 110")
    })
  })
  ...
  describe("closeAuction", () => {
    it("closes auction with winner when reserve is met", async () => {
      jest.spyOn(service, "retrieveAuctionListing" as any).mockResolvedValue({
        id: "auc-1", status: "active", reserve_price: 100,
      })
      jest.spyOn(service, "getHighestBid" as any).mockResolvedValue({
        id: "bid-1", bidder_id: "user-1", amount: 150,
      })
      jest.spyOn(service as any, "updateAuctionListings").mockResolvedValue({})
      const resultSpy = jest.spyOn(service as any, "createAuctionResults").mockResolvedValue({ id: "res-1" })
    ...
    it("closes auction without winner when reserve is not met", async () => {
      jest.spyOn(service, "retrieveAuctionListing" as any).mockResolvedValue({
        id: "auc-1", status: "active", reserve_price: 500,
      })
      jest.spyOn(service, "getHighestBid" as any).mockResolvedValue({
        id: "bid-1", bidder_id: "user-1", amount: 200,
      })
      jest.spyOn(service as any, "updateAuctionListings").mockResolvedValue({})

      const result = await service.closeAuction("auc-1")

      expect(result).toEqual({ auctionId: "auc-1", status: "ended", winner: null })
    })

    it("throws when auction is not active", async () => {
      jest.spyOn(service, "retrieveAuctionListing" as any).mockResolvedValue({
        id: "auc-1", status: "ended",
      })

      await expect(service.closeAuction("auc-1")).rejects.toThrow("Auction is not active")
    })
  })
