...
describe("RentalModuleService – Enhanced", () => {
  ...
  describe("checkAvailability", () => {
    it("returns true when item is available and no conflicts", async () => {
      jest.spyOn(service, "retrieveRentalProduct" as any).mockResolvedValue({ id: "rp-1", status: "available" })
    ...
    it("throws when start date is after end date", async () => {
      ...
      await expect(service.checkAvailability("rp-1", start, end))
  ...
  describe("calculateRentalPrice", () => {
    it("calculates daily rate * duration", async () => {
      jest.spyOn(service, "retrieveRentalProduct" as any).mockResolvedValue({ daily_rate: 50 })
    ...
    it("applies 10% weekly discount for 7+ days", async () => {
      jest.spyOn(service, "retrieveRentalProduct" as any).mockResolvedValue({ daily_rate: 100 })
    ...
    it("throws when duration is zero or negative", async () => {
      await expect(service.calculateRentalPrice("rp-1", 0))
  ...
  describe("processReturn", () => {
    it("processes a return for an active rental", async () => {
      jest.spyOn(service, "retrieveRentalAgreement" as any).mockResolvedValue({
        id: "ra-1", status: "active", rental_product_id: "rp-1",
      })
      const returnSpy = jest.spyOn(service as any, "createRentalReturns").mockResolvedValue({ id: "ret-1" })
    ...
    it("throws when rental is not active", async () => {
      jest.spyOn(service, "retrieveRentalAgreement" as any).mockResolvedValue({ id: "ra-1", status: "returned" })

      await expect(service.processReturn("ra-1"))
