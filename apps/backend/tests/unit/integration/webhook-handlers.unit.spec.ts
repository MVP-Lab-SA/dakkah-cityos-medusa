...
const mockErpNextService = {
  handleSalesInvoiceCreated: jest.fn(),
  handlePaymentEntrySubmitted: jest.fn(),
}

const mockPayloadCmsService = {
  handleCollectionCreate: jest.fn(),
  handleCollectionUpdate: jest.fn(),
}

function verifyStripeSignature(payload: string, signature: string, secret: string): boolean {
  const timestamp = signature.split(",").find((s: string) => s.startsWith("t="))?.split("=")[1]
  const sig = signature.split(",").find((s: string) => s.startsWith("v1="))?.split("=").slice(1).join("=")
  if (!timestamp || !sig) return false
  const expected = crypto.createHmac("sha256", secret).update(`${timestamp}.${payload}`).digest("hex")
  return expected === sig
}

function verifyErpNextSignature(payload: string, signature: string, secret: string): boolean {
...
async function handleStripeWebhook(req: any, res: any, config: { secret: string; service: any }) {
  try {
    ...
    if (!signature || !verifyStripeSignature(payload, signature, config.secret)) {
...
async function handleErpNextWebhook(req: any, res: any, config: { secret: string; service: any }) {
...
async function handlePayloadCmsWebhook(req: any, res: any, config: { apiKey: string; service: any }) {
...
describe("Webhook Handlers", () => {
  ...
  describe("Stripe Webhook Handler", () => {
    it("should verify webhook signature", async () => {
      const body = JSON.stringify({ type: "payment_intent.succeeded", data: { id: "pi_123" } })
      const signature = createStripeSignature(body, stripeSecret)
      const req = createReq({ headers: { "stripe-signature": signature }, body })
      const res = createRes()
    ...
    it("should process payment_intent.succeeded event", async () => {
      ...
      const body = JSON.stringify({ type: "payment_intent.succeeded", data: eventData })
      const signature = createStripeSignature(body, stripeSecret)
      const req = createReq({ headers: { "stripe-signature": signature }, body })
      const res = createRes()
      await handleStripeWebhook(req, res, { secret: stripeSecret, service: mockStripeService })
      expect(mockStripeService.handlePaymentIntentSucceeded).toHaveBeenCalledWith(eventData)
    })

    it("should process checkout.session.completed event", async () => {
      const eventData = { id: "cs_123", payment_status: "paid" }
      const body = JSON.stringify({ type: "checkout.session.completed", data: eventData })
      const signature = createStripeSignature(body, stripeSecret)
      const req = createReq({ headers: { "stripe-signature": signature }, body })
      const res = createRes()
      await handleStripeWebhook(req, res, { secret: stripeSecret, service: mockStripeService })
      expect(mockStripeService.handleCheckoutSessionCompleted).toHaveBeenCalledWith(eventData)
    })
  ...
  describe("ERPNext Webhook Handler", () => {
    ...
    it("should verify HMAC signature", async () => {
      const body = JSON.stringify({ event: "sales_invoice.created", data: { name: "SINV-001" } })
    ...
    it("should process sales_invoice.created event", async () => {
      ...
      const body = JSON.stringify({ event: "sales_invoice.created", data: eventData })
      const signature = createErpNextSignature(body, erpNextSecret)
      const req = createReq({ headers: { "x-erpnext-signature": signature }, body })
      const res = createRes()
      await handleErpNextWebhook(req, res, { secret: erpNextSecret, service: mockErpNextService })
      expect(mockErpNextService.handleSalesInvoiceCreated).toHaveBeenCalledWith(eventData)
    })

    it("should process payment_entry.submitted event", async () => {
      const eventData = { name: "PE-001", paid_amount: 500 }
      const body = JSON.stringify({ event: "payment_entry.submitted", data: eventData })
      const signature = createErpNextSignature(body, erpNextSecret)
      const req = createReq({ headers: { "x-erpnext-signature": signature }, body })
      const res = createRes()
      await handleErpNextWebhook(req, res, { secret: erpNextSecret, service: mockErpNextService })
      expect(mockErpNextService.handlePaymentEntrySubmitted).toHaveBeenCalledWith(eventData)
    })
  ...
  describe("Payload CMS Webhook Handler", () => {
    it("should verify API key", async () => {
      const body = JSON.stringify({ event: "collection.create", data: { collection: "products", id: "p_1" } })
      const req = createReq({ headers: { "x-api-key": payloadApiKey }, body })
      const res = createRes()
      await handlePayloadCmsWebhook(req, res, { apiKey: payloadApiKey, service: mockPayloadCmsService })
      expect(res.status).toHaveBeenCalledWith(200)
      expect(res.json).toHaveBeenCalledWith({ received: true })
    })

    it("should process collection.create event", async () => {
      const eventData = { collection: "products", id: "p_1", title: "New Product" }
      const body = JSON.stringify({ event: "collection.create", data: eventData })
      const req = createReq({ headers: { "x-api-key": payloadApiKey }, body })
      const res = createRes()
      await handlePayloadCmsWebhook(req, res, { apiKey: payloadApiKey, service: mockPayloadCmsService })
      expect(mockPayloadCmsService.handleCollectionCreate).toHaveBeenCalledWith(eventData)
    })

    it("should process collection.update event", async () => {
      const eventData = { collection: "products", id: "p_1", title: "Updated Product" }
      const body = JSON.stringify({ event: "collection.update", data: eventData })
      const req = createReq({ headers: { "x-api-key": payloadApiKey }, body })
      const res = createRes()
      await handlePayloadCmsWebhook(req, res, { apiKey: payloadApiKey, service: mockPayloadCmsService })
      expect(mockPayloadCmsService.handleCollectionUpdate).toHaveBeenCalledWith(eventData)
    })
