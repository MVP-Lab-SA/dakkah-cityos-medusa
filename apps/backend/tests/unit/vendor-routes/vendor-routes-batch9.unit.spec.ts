import { GET as productsGET, POST as productsPOST } from "../../../src/api/vendor/products/route"
...
describe("Vendor Routes Batch 9", () => {
  ...
  describe("Products /vendor/products", () => {
    ...
    it("GET returns products when vendor_id present", async () => {
      const products = [{ id: "vp_1", vendor_id: "vendor-123", product_id: "prod_1", product: { title: "Test", handle: "test", thumbnail: null, variants: [], images: [] }, status: "active", vendor_sku: "SKU-1", vendor_cost: 10, commission_override: null, inventory_quantity: 50, is_primary_vendor: true, created_at: "2026-01-01" }]
      mockQuery.graph.mockResolvedValue({ data: products })
      const req = createReq({ scope: { resolve: jest.fn(() => mockQuery) } })
      const res = createRes()
      await productsGET(req as any, res)
    ...
    it("POST returns 401 when vendor_id missing", async () => {
      const req = createReq({ vendor_id: undefined, scope: { resolve: jest.fn(() => ({})) } })
      const res = createRes()
      await productsPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(401)
    })
  })
  ...
  describe("Payouts /vendor/payouts", () => {
    ...
    it("GET returns payouts when vendor_id present", async () => {
      ...
      const res = createRes()
      await payoutsGET(req as any, res)
      expect(res.json).toHaveBeenCalled()
    })
  ...
  describe("Payouts Request /vendor/payouts/request", () => {
    const mockQuery = { graph: jest.fn() }
    const mockPayoutModule = { createPayouts: jest.fn(), createPayoutTransactionLinks: jest.fn() }
    ...
    it("POST returns 401 when vendor_id missing", async () => {
      ...
      const res = createRes()
      await payoutsRequestPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(401)
    })
    ...
    it("POST returns 400 when no funds available", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: "No funds available for payout" })
    })

    it("POST creates payout when funds available", async () => {
      const payout = { id: "pay_1", amount: 100, status: "pending" }
      mockQuery.graph.mockResolvedValue({ data: [{ id: "ct_1", net_amount: 100 }] })
  ...
  describe("Commissions /vendor/commissions", () => {
    ...
    it("GET returns commissions when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockQuery) } })
      const res = createRes()
      await commissionsGET(req as any, res)
      expect(res.json).toHaveBeenCalled()
    })
    ...
    it("GET includes summary in response", async () => {
      const transactions = [{ id: "ct_1", gross_amount: 100, commission_amount: 10, net_amount: 90, commission_rate: 0.1, order: { display_id: 1 } }]
  ...
  describe("Rentals /vendor/rentals", () => {
    ...
    it("POST creates rental with valid data", async () => {
      ...
      const req = createReq({
        ...
        body: { product_id: "prod_1", rental_type: "daily", base_price: 50, currency_code: "usd" },
      })
      const res = createRes()
  ...
  describe("Digital Products /vendor/digital-products", () => {
    ...
    it("POST creates digital product with valid data", async () => {
      ...
      const req = createReq({
        scope: { resolve: jest.fn(() => mockService) },
        body: { product_id: "prod_1", title: "Course Video", file_url: "https://example.com/file.pdf", file_type: "pdf" },
      })
      const res = createRes()
