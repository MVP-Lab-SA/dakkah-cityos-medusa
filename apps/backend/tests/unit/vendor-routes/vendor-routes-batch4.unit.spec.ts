import { GET as inventoryExtensionGET, POST as inventoryExtensionPOST } from "../../../src/api/vendor/inventory-extension/route"
import { GET as volumePricingGET, POST as volumePricingPOST } from "../../../src/api/vendor/volume-pricing/route"
import { GET as dropshippingGET, POST as dropshippingPOST } from "../../../src/api/vendor/dropshipping/route"
import { GET as printOnDemandGET, POST as printOnDemandPOST } from "../../../src/api/vendor/print-on-demand/route"
import { GET as whiteLabelGET, POST as whiteLabelPOST } from "../../../src/api/vendor/white-label/route"
import { GET as tryBeforeYouBuyGET, POST as tryBeforeYouBuyPOST } from "../../../src/api/vendor/try-before-you-buy/route"
import { GET as creditGET, POST as creditPOST } from "../../../src/api/vendor/credit/route"
import { GET as walletGET } from "../../../src/api/vendor/wallet/route"
import { GET as tradeInGET, POST as tradeInPOST } from "../../../src/api/vendor/trade-in/route"
import { GET as cartExtensionGET, POST as cartExtensionPOST } from "../../../src/api/vendor/cart-extension/route"
...
describe("Vendor Routes Batch 4", () => {
  ...
  describe("Inventory Extension /vendor/inventory-extension", () => {
    const mockService = { listReservationHolds: jest.fn(), createReservationHolds: jest.fn() }
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const res = createRes()
      await inventoryExtensionGET(req as any, res)
    ...
    it("POST creates item with valid data", async () => {
      ...
      const res = createRes()
      await inventoryExtensionPOST(req as any, res)
  ...
  describe("Volume Pricing /vendor/volume-pricing", () => {
    const mockService = { listVolumePricings: jest.fn(), createVolumePricings: jest.fn() }
    ...
    it("POST creates item with valid data", async () => {
      ...
      const req = createReq({
        scope: { resolve: jest.fn(() => mockService) },
        body: { name: "Bulk Tier 2", min_quantity: 10, discount_type: "percentage", discount_value: 10 },
      })
      const res = createRes()
      await volumePricingPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(201)
      expect(res.json).toHaveBeenCalledWith({ item })
    })
  ...
  describe("Dropshipping /vendor/dropshipping", () => {
    const mockService = { listVendorProducts: jest.fn(), createVendorProducts: jest.fn() }
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await dropshippingGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
    ...
    it("POST creates item with valid data", async () => {
      ...
      const res = createRes()
      await dropshippingPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(201)
      expect(res.json).toHaveBeenCalledWith({ item })
    })
  ...
  describe("Print on Demand /vendor/print-on-demand", () => {
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await printOnDemandGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
    ...
    it("POST creates item with valid data", async () => {
      ...
      const req = createReq({
        scope: { resolve: jest.fn(() => mockService) },
        body: { title: "Custom Mug", design_url: "https://example.com/design.png", product_type: "mug" },
      })
      const res = createRes()
      await printOnDemandPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(201)
      expect(res.json).toHaveBeenCalledWith({ item })
    })
  ...
  describe("White Label /vendor/white-label", () => {
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await whiteLabelGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
    ...
    it("POST creates item with valid data", async () => {
      ...
      const res = createRes()
      await whiteLabelPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(201)
      expect(res.json).toHaveBeenCalledWith({ item })
    })
  ...
  describe("Try Before You Buy /vendor/try-before-you-buy", () => {
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await tryBeforeYouBuyGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
    ...
    it("POST creates item with valid data", async () => {
      ...
      const req = createReq({
        scope: { resolve: jest.fn(() => mockService) },
        body: { product_id: "prod_1", title: "Trial Shoes", retail_price: 99.99 },
      })
      const res = createRes()
      await tryBeforeYouBuyPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(201)
      expect(res.json).toHaveBeenCalledWith({ item })
    })
  ...
  describe("Credit /vendor/credit", () => {
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await creditGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
    ...
    it("POST creates item with valid data", async () => {
      ...
      const req = createReq({
        scope: { resolve: jest.fn(() => mockService) },
        body: { name: "Business Loan", loan_type: "business", min_amount: 1000, max_amount: 50000, currency_code: "usd", interest_rate_min: 5, interest_rate_max: 15, interest_type: "fixed", min_term_months: 6, max_term_months: 36 },
      })
      const res = createRes()
      await creditPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(201)
      expect(res.json).toHaveBeenCalledWith({ item })
    })
  ...
  describe("Cart Extension /vendor/cart-extension", () => {
    const mockService = { listCartMetadatas: jest.fn(), createCartMetadatas: jest.fn() }
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await cartExtensionGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
    ...
    it("POST creates item with valid data", async () => {
      ...
      const res = createRes()
      await cartExtensionPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(201)
      expect(res.json).toHaveBeenCalledWith({ item })
    })
