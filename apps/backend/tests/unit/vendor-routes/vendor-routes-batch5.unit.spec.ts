import { GET as disputesGET, POST as disputesPOST } from "../../../src/api/vendor/disputes/route"
import { GET as invoicesGET, POST as invoicesPOST } from "../../../src/api/vendor/invoices/route"
import { GET as quotesGET, POST as quotesPOST } from "../../../src/api/vendor/quotes/route"
import { GET as reviewsGET, POST as reviewsPOST } from "../../../src/api/vendor/reviews/route"
import { GET as eventTicketingGET, POST as eventTicketingPOST } from "../../../src/api/vendor/event-ticketing/route"
...
describe("Vendor Routes Batch 5", () => {
  ...
  describe("Disputes /vendor/disputes", () => {
    const mockService = { listDisputes: jest.fn(), updateDisputes: jest.fn(), retrieveDispute: jest.fn() }
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const res = createRes()
      await disputesGET(req as any, res)
    ...
    it("POST returns 401 when vendor_id missing", async () => {
      const req = createReq({ vendor_id: undefined, scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await disputesPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(401)
    })
  })
  ...
  describe("Invoices /vendor/invoices", () => {
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await invoicesGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
    ...
    it("POST returns 401 when vendor_id missing", async () => {
      const req = createReq({ vendor_id: undefined, scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await invoicesPOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(401)
    })
  })
  ...
  describe("Quotes /vendor/quotes", () => {
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await quotesGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
    ...
    it("POST creates quote with valid data", async () => {
      ...
      const req = createReq({
        ...
        body: { customer_id: "cust_1", title: "Custom Quote", items: [{ description: "Widget", quantity: 2, unit_price: 100 }], currency_code: "usd", valid_until: "2026-12-31" },
      })
      const res = createRes()
      await quotesPOST(req as any, res)
  ...
  describe("Reviews /vendor/reviews", () => {
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await reviewsGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
  ...
  describe("Event Ticketing /vendor/event-ticketing", () => {
    const mockService = { listEventTickets: jest.fn(), createEventTickets: jest.fn() }
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await eventTicketingGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
    ...
    it("POST returns 400 for invalid data", async () => {
      ...
      const res = createRes()
      await eventTicketingPOST(req as any, res)
  ...
  describe("Analytics /vendor/analytics", () => {
    ...
    it("GET returns analytics data when vendor_id present", async () => {
      ...
      const res = createRes()
      await analyticsGET(req as any, res)
  ...
  describe("Dashboard /vendor/dashboard", () => {
    ...
    it("GET returns dashboard data when cityosContext present", async () => {
      mockVendorModule.retrieveVendor.mockResolvedValue({ id: "vendor-123", total_sales: 1000, total_commission_paid: 100 })
      ...
      const res = createRes()
      await dashboardGET(req as any, res)
      expect(res.json).toHaveBeenCalled()
    })
    ...
    it("GET returns vendor stats in response", async () => {
      mockVendorModule.retrieveVendor.mockResolvedValue({ id: "vendor-123", total_sales: 5000, total_commission_paid: 500 })
      mockCommissionModule.listCommissions.mockResolvedValue([{ net_amount: 100, commission_amount: 10 }])
      mockPayoutModule.listPayouts.mockResolvedValue([{ id: "pay_1", amount: 50 }])
      const req = createReq({
        cityosContext: { vendorId: "vendor-123" },
        scope: {
          resolve: jest.fn((name: string) => {
  ...
  describe("Transactions /vendor/transactions", () => {
    ...
    it("GET returns transactions when cityosContext present", async () => {
      ...
      const res = createRes()
      await transactionsGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ transactions, count: 1, limit: 20, offset: 0 })
    })
