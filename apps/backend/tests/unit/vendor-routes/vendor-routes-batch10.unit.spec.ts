...
import { GET as affiliateGET, POST as affiliatePOST } from "../../../src/api/vendor/affiliate/route"
import { GET as auctionsGET, POST as auctionsPOST } from "../../../src/api/vendor/auctions/route"
import { GET as b2bGET, POST as b2bPOST } from "../../../src/api/vendor/b2b/route"
...
import { GET as socialCommerceGET, POST as socialCommercePOST } from "../../../src/api/vendor/social-commerce/route"
...
describe("Vendor Routes Batch 10", () => {
  ...
  describe("Affiliate /vendor/affiliate", () => {
    ...
    it("GET returns items with pagination when vendor_id present", async () => {
      ...
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) } })
      const res = createRes()
      await affiliateGET(req as any, res)
      expect(res.json).toHaveBeenCalledWith({ items, count: 1, limit: 20, offset: 0 })
    })
    ...
    it("POST returns 400 for invalid data", async () => {
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, body: {} })
      const res = createRes()
      await affiliatePOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(400)
    })
  })
  ...
  describe("Auctions /vendor/auctions", () => {
    ...
    it("POST creates auction with valid data", async () => {
      ...
      const req = createReq({
        ...
        body: { product_id: "prod_1", title: "Art Piece", auction_type: "english", starting_price: 100, currency_code: "usd", bid_increment: 10, starts_at: "2026-01-01", ends_at: "2026-02-01" },
      })
      const res = createRes()
  ...
  describe("Social Commerce /vendor/social-commerce", () => {
    const mockService = { listLiveStreams: jest.fn(), createLiveStreams: jest.fn() }
    ...
    it("POST returns 400 for invalid data", async () => {
      const req = createReq({ scope: { resolve: jest.fn(() => mockService) }, body: {} })
      const res = createRes()
      await socialCommercePOST(req as any, res)
      expect(res.status).toHaveBeenCalledWith(400)
    })
  })
})
