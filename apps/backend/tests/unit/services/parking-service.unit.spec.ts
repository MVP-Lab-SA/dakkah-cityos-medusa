  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listParkingZones(_filter: any): Promise<any> { return [] }
        ...
        async listShuttleRoutes(_filter: any): Promise<any> { return [] }
        async listRideRequests(_filter: any): Promise<any> { return [] }
      },
    model: {
      define: () => ({ indexes: () => ({}) }),
      id: chainable,
...
describe("ParkingModuleService", () => {
  ...
  describe("checkIn", () => {
    it("checks in a reserved session", async () => {
      jest.spyOn(service, "retrieveParkingSession" as any).mockResolvedValue({ id: "s1", status: "reserved" })
      const updateSpy = jest.spyOn(service as any, "updateParkingSessions").mockResolvedValue({ id: "s1", status: "active" })
    ...
    it("throws when session is already completed", async () => {
      jest.spyOn(service, "retrieveParkingSession" as any).mockResolvedValue({ id: "s1", status: "completed" })

      await expect(service.checkIn("s1")).rejects.toThrow("This session has already been completed")
    })

    it("throws when already checked in", async () => {
      jest.spyOn(service, "retrieveParkingSession" as any).mockResolvedValue({ id: "s1", status: "active", checked_in_at: new Date() })
  ...
  describe("checkOut", () => {
    it("calculates fee based on duration", async () => {
      ...
      jest.spyOn(service, "retrieveParkingSession" as any).mockResolvedValue({
        id: "s1", status: "active", checked_in_at: startTime, zone_id: "z1",
      })
      jest.spyOn(service, "retrieveParkingZone" as any).mockResolvedValue({ id: "z1", hourly_rate: 10 })
      jest.spyOn(service as any, "updateParkingSessions").mockResolvedValue({})
    ...
    it("throws when session is already completed", async () => {
      ...
      await expect(service.checkOut("s1")).rejects.toThrow("This session has already been completed")
    })

    it("throws when session is not active", async () => {
      jest.spyOn(service, "retrieveParkingSession" as any).mockResolvedValue({ id: "s1", status: "reserved" })

      await expect(service.checkOut("s1")).rejects.toThrow("Session must be active to check out")
    })
  })

  describe("getAvailableSpots", () => {
    it("returns available spots count", async () => {
      jest.spyOn(service, "retrieveParkingZone" as any).mockResolvedValue({ id: "z1", total_spots: 100 })
  ...
  describe("calculateDynamicFee", () => {
    ...
    it("uses default multiplier for unknown vehicle type", async () => {
      jest.spyOn(service, "retrieveParkingZone" as any).mockResolvedValue({ id: "z1", hourly_rate: 5 })
    ...
    it("rounds up to minimum 1 hour", async () => {
      jest.spyOn(service, "retrieveParkingZone" as any).mockResolvedValue({ id: "z1", hourly_rate: 8 })
