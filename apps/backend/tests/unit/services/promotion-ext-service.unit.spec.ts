jest.mock("@medusajs/framework/utils", () => {
  ...
  return {
    MedusaService: () => class MockMedusaBase {
      async listGiftCardExts(_filter: any, _opts?: any): Promise<any> { return [] }
      async retrieveGiftCardExt(_id: string): Promise<any> { return null }
      async createGiftCardExts(_data: any): Promise<any> { return {} }
      async updateGiftCardExts(_data: any): Promise<any> { return {} }
      async listReferrals(_filter: any): Promise<any> { return [] }
      async listProductBundles(_filter: any): Promise<any> { return [] }
      async listCustomerSegments(_filter: any): Promise<any> { return [] }
    },
...
describe("PromotionExtModuleService", () => {
  ...
  describe("getActivePromotions", () => {
    it("returns active promotions filtered by tenant", async () => {
      ...
      jest.spyOn(service, "listGiftCardExts" as any).mockResolvedValue(promotions)
  ...
  describe("validatePromotionRules", () => {
    it("returns invalid when promotion not active", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({ is_active: false })
    ...
    it("returns invalid when promotion expired", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        is_active: true, expires_at: new Date(Date.now() - 100000), remaining_value: 100,
      })
    ...
    it("returns invalid when budget exhausted", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        is_active: true, expires_at: null, remaining_value: 0,
      })
    ...
    it("returns partial apply when budget less than total", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        is_active: true, expires_at: null, remaining_value: 50,
      })

      const result = await service.validatePromotionRules("p1", { totalAmount: 100 })
    ...
    it("returns valid when all rules pass", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        is_active: true, expires_at: null, remaining_value: 200,
      })
  ...
  describe("calculateDiscount", () => {
    it("calculates discount across line items", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        is_active: true, remaining_value: 50,
      })
    ...
    it("returns zero discount when promotion inactive", async () => {
      ...
      const result = await service.calculateDiscount("p1", [{ productId: "prod_1", quantity: 1, price: 50 }])
    ...
    it("caps discount at remaining value", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        is_active: true, remaining_value: 10,
      })
  ...
  describe("getPromotionUsageStats", () => {
    it("calculates usage statistics", async () => {
      jest.spyOn(service, "retrieveGiftCardExt" as any).mockResolvedValue({
        initial_value: 100, remaining_value: 25, is_active: true, expires_at: null, delivered_at: null,
      })
  ...
  describe("deactivateExpiredPromotions", () => {
    it("deactivates expired promotions", async () => {
      jest.spyOn(service, "listGiftCardExts" as any).mockResolvedValue([
        { id: "p1" }, { id: "p2" },
      ])
      jest.spyOn(service as any, "updateGiftCardExts").mockResolvedValue({ updated: true })
