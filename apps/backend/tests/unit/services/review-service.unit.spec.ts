jest.mock("@medusajs/framework/utils", () => {
  ...
  return {
    MedusaService: () => class MockMedusaBase {
      async listReviews(_filter: any, _opts?: any): Promise<any> { return [] }
      async retrieveReview(_id: string): Promise<any> { return null }
      async createReviews(_data: any): Promise<any> { return {} }
      async updateReviews(_sel: any, _data?: any): Promise<any> { return {} }
      async deleteReviews(_id: string): Promise<any> { return {} }
    },
...
describe("ReviewModuleService", () => {
  ...
  describe("createReview", () => {
    it("creates a review with default values", async () => {
      const created = { id: "r1", rating: 5, is_approved: false, helpful_count: 0 }
      jest.spyOn(service, "createReviews" as any).mockResolvedValue(created)

      const result = await service.createReview({
        rating: 5, content: "Great!", customer_id: "c1", product_id: "p1",
      })
    ...
    it("throws when rating out of range (too low)", async () => {
      await expect(service.createReview({
        rating: 0, content: "Bad", customer_id: "c1", product_id: "p1",
      })).rejects.toThrow("Rating must be between 1 and 5")
    })

    it("throws when rating out of range (too high)", async () => {
      await expect(service.createReview({
        rating: 6, content: "Bad", customer_id: "c1", product_id: "p1",
      })).rejects.toThrow("Rating must be between 1 and 5")
    })

    it("throws when neither product nor vendor specified", async () => {
      await expect(service.createReview({
        rating: 3, content: "Test", customer_id: "c1",
      })).rejects.toThrow("Review must be for a product or vendor")
    })

    it("allows vendor review without product_id", async () => {
      jest.spyOn(service, "createReviews" as any).mockResolvedValue({ id: "r1" })

      const result = await service.createReview({
        rating: 4, content: "Good vendor", customer_id: "c1", vendor_id: "v1",
      })
  ...
  describe("listProductReviews", () => {
    it("lists approved reviews by default", async () => {
      const listSpy = jest.spyOn(service, "listReviews" as any).mockResolvedValue([])
  ...
  describe("approveReview", () => {
    it("updates review to approved", async () => {
      const updateSpy = jest.spyOn(service, "updateReviews" as any).mockResolvedValue({ id: "r1", is_approved: true })
  ...
  describe("rejectReview", () => {
    it("deletes the review", async () => {
      const deleteSpy = jest.spyOn(service, "deleteReviews" as any).mockResolvedValue({})
  ...
  describe("markHelpful", () => {
    it("increments helpful count", async () => {
      jest.spyOn(service, "retrieveReview" as any).mockResolvedValue({ id: "r1", helpful_count: 3 })
      const updateSpy = jest.spyOn(service, "updateReviews" as any).mockResolvedValue({})
    ...
    it("handles null helpful_count", async () => {
      jest.spyOn(service, "retrieveReview" as any).mockResolvedValue({ id: "r1", helpful_count: null })
      const updateSpy = jest.spyOn(service, "updateReviews" as any).mockResolvedValue({})
