jest.mock("@medusajs/framework/utils", () => {
  ...
  return {
    MedusaService: () => class MockMedusaBase {
      async listNodes(_filter: any): Promise<any> { return [] }
...
describe("NodeModuleService", () => {
  ...
  describe("listNodesByTenant", () => {
    it("lists nodes filtered by tenant", async () => {
      ...
      jest.spyOn(service, "listNodes" as any).mockResolvedValue(nodes)
    ...
    it("passes additional filters", async () => {
      const listSpy = jest.spyOn(service, "listNodes" as any).mockResolvedValue([])
  ...
  describe("listChildren", () => {
    it("returns children of a node", async () => {
      ...
      jest.spyOn(service, "listNodes" as any).mockResolvedValue(children)
  ...
  describe("getAncestors", () => {
    it("returns ancestors in root-to-parent order", async () => {
      ...
      const district = { id: "dist_1", name: "District A", parent_id: "city_1" }
      const zone = { id: "zone_1", name: "Zone B", parent_id: "dist_1" }

      jest.spyOn(service, "retrieveNode" as any).mockImplementation(async (id: string) => {
    ...
    it("returns empty array for root node", async () => {
      jest.spyOn(service, "retrieveNode" as any).mockResolvedValue({ id: "city_1", parent_id: null })
  ...
  describe("getBreadcrumbs", () => {
    it("returns breadcrumb path from root to node", async () => {
      const city = { id: "city_1", name: "Riyadh", slug: "riyadh", type: "CITY", depth: 0, parent_id: null }
      const district = { id: "dist_1", name: "District A", slug: "district-a", type: "DISTRICT", depth: 1, parent_id: "city_1" }
      ...
      expect(result).toEqual([
        { id: "city_1", name: "Riyadh", slug: "riyadh", type: "CITY", depth: 0 },
        { id: "dist_1", name: "District A", slug: "district-a", type: "DISTRICT", depth: 1 },
      ])
    })
  })
  ...
  describe("createNodeWithValidation", () => {
    it("creates root node without parent", async () => {
      jest.spyOn(service as any, "createNodes").mockResolvedValue({ id: "n1", type: "CITY" })

      const result = await service.createNodeWithValidation({
        tenant_id: "t1", name: "Riyadh", slug: "riyadh", type: "CITY",
      })
      expect(result.type).toBe("CITY")
    })

    it("throws for invalid node type", async () => {
      await expect(service.createNodeWithValidation({
        tenant_id: "t1", name: "Test", slug: "test", type: "INVALID",
      })).rejects.toThrow("Invalid node type: INVALID")
    })

    it("throws when parent required but missing", async () => {
      await expect(service.createNodeWithValidation({
        tenant_id: "t1", name: "Test", slug: "test", type: "DISTRICT",
      })).rejects.toThrow("requires a parent")
    })

    it("throws when root node has parent", async () => {
      await expect(service.createNodeWithValidation({
        tenant_id: "t1", name: "Test", slug: "test", type: "CITY", parent_id: "p1",
    ...
    it("throws for invalid hierarchy", async () => {
      jest.spyOn(service, "retrieveNode" as any).mockResolvedValue({
        id: "p1", type: "CITY", tenant_id: "t1",
      })
      jest.spyOn(service, "listNodes" as any).mockResolvedValue([])

      await expect(service.createNodeWithValidation({
        tenant_id: "t1", name: "Test", slug: "test", type: "ZONE", parent_id: "p1",
      })).rejects.toThrow("Invalid hierarchy")
    })

    it("throws when parent belongs to different tenant", async () => {
      jest.spyOn(service, "retrieveNode" as any).mockResolvedValue({
        id: "p1", type: "CITY", tenant_id: "other_tenant", name: "City", slug: "city", depth: 0, parent_id: null,
      })

      await expect(service.createNodeWithValidation({
        tenant_id: "t1", name: "District", slug: "district", type: "DISTRICT", parent_id: "p1",
