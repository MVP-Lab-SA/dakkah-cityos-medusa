jest.mock("@medusajs/framework/utils", () => {
  ...
  return {
    MedusaService: () => class MockMedusaBase {
      async retrieveVolumePricing(_id: string): Promise<any> { return null }
      async listVolumePricingTiers(_f: any): Promise<any> { return [] }
      async listVolumePricings(_f: any): Promise<any> { return [] }
    },
    model: {
      define: () => ({ indexes: () => ({}) }),
...
describe("VolumePricingModuleService", () => {
  ...
  describe("calculateDiscount", () => {
    it("returns no discount when no tier matches", async () => {
      jest.spyOn(service, "retrieveVolumePricing" as any).mockResolvedValue({ id: "vp-1", pricing_type: "percentage" })
      jest.spyOn(service, "listVolumePricingTiers" as any).mockResolvedValue([
    ...
    it("calculates fixed discount for matching tier", async () => {
      jest.spyOn(service, "retrieveVolumePricing" as any).mockResolvedValue({ id: "vp-1", pricing_type: "fixed" })
      jest.spyOn(service, "listVolumePricingTiers" as any).mockResolvedValue([
    ...
    it("calculates fixed_price discount for matching tier", async () => {
      jest.spyOn(service, "retrieveVolumePricing" as any).mockResolvedValue({ id: "vp-1", pricing_type: "fixed_price" })
      jest.spyOn(service, "listVolumePricingTiers" as any).mockResolvedValue([
  ...
  describe("findApplicableRules", () => {
    it("returns rules sorted by priority", async () => {
      jest.spyOn(service, "listVolumePricings" as any).mockResolvedValue([
        { id: "vp-1", priority: 1 },
        { id: "vp-2", priority: 10 },
        { id: "vp-3", priority: 5 },
      ])
    ...
    it("returns empty array when list returns null", async () => {
      jest.spyOn(service, "listVolumePricings" as any).mockResolvedValue(null)
  ...
  describe("getBestVolumePrice", () => {
    it("returns best discount across multiple rules", async () => {
      jest.spyOn(service, "findApplicableRules" as any).mockResolvedValue([
        { id: "vp-1" }, { id: "vp-2" },
      ])
      jest.spyOn(service, "calculateDiscount" as any)
        .mockResolvedValueOnce({ discountPerUnit: 50n, discountTotal: 500n, finalUnitPrice: 950n, finalTotal: 9500n })
        .mockResolvedValueOnce({ discountPerUnit: 100n, discountTotal: 1000n, finalUnitPrice: 900n, finalTotal: 9000n })

      const result = await service.getBestVolumePrice({
        productId: "p-1", variantId: "v-1", quantity: 10,
        unitPrice: 1000n, currencyCode: "usd", tenantId: "t-1",
      })
    ...
    it("returns original pricing when discounts are all zero", async () => {
      jest.spyOn(service, "findApplicableRules" as any).mockResolvedValue([{ id: "vp-1" }])
      jest.spyOn(service, "calculateDiscount" as any).mockResolvedValue({
        discountPerUnit: 0n, discountTotal: 0n, finalUnitPrice: 1000n, finalTotal: 10000n,
      })
