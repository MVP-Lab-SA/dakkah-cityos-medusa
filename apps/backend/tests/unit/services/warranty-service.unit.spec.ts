  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        ...
        async listSpareParts(_filter: any): Promise<any> { return [] }
        async listServiceCenters(_filter: any): Promise<any> { return [] }
      },
    model: {
      define: () => ({ indexes: () => ({}) }),
      id: chainable,
...
describe("WarrantyModuleService", () => {
  ...
  describe("extendWarranty", () => {
    it("extends warranty expiry by the given months", async () => {
      ...
      const updateSpy = jest.spyOn(service as any, "updateWarrantyClaims").mockResolvedValue({ id: "w-1" })
    ...
    it("throws when warranty is voided", async () => {
      ...
      await expect(service.extendWarranty("w-1", 6, 50))
  ...
  describe("getWarrantyHistory", () => {
    it("returns warranties with computed expired status", async () => {
      ...
      jest.spyOn(service, "listWarrantyClaims" as any).mockResolvedValue([
        {
          id: "w-1",
          product_id: "p1",
          plan_id: "plan-1",
  ...
  describe("scheduleRepair", () => {
    it("schedules a repair for an approved claim with existing repair order", async () => {
      ...
      jest.spyOn(service, "listRepairOrders" as any).mockResolvedValue([
        { id: "ro-1", warranty_claim_id: "claim-1" },
      ])
      const updateSpy = jest.spyOn(service as any, "updateRepairOrders").mockResolvedValue({ id: "ro-1", status: "scheduled" })
    ...
    it("creates a new repair order when none exists", async () => {
      ...
      jest.spyOn(service, "listRepairOrders" as any).mockResolvedValue([])
      const createSpy = jest.spyOn(service as any, "createRepairOrders").mockResolvedValue({ id: "ro-new" })
    ...
    it("throws when claim is not approved", async () => {
      ...
      const futureDate = new Date()
      futureDate.setDate(futureDate.getDate() + 7)
      await expect(service.scheduleRepair("claim-1", {
        scheduledDate: futureDate,
        repairType: "fix",
