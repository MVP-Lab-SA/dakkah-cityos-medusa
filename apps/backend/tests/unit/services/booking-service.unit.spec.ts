...
describe("BookingModuleService", () => {
  ...
  describe("getAvailabilityForDate", () => {
    it("returns availability matching date range", async () => {
      ...
      jest.spyOn(service as any, "listAvailabilities").mockResolvedValue([avail])

      const result = await service.getAvailabilityForDate("service", "svc-1", new Date("2025-06-15"))
      expect(result).toEqual(avail)
    })

    it("returns null when no availability found", async () => {
      jest.spyOn(service as any, "listAvailabilities").mockResolvedValue([])
    ...
    it("falls back to first availability when date filters skip all", async () => {
      jest.spyOn(service as any, "listAvailabilities").mockResolvedValue([
        { id: "av-1", effective_from: "2026-01-01", effective_to: "2026-12-31" },
      ])

      const result = await service.getAvailabilityForDate("service", "svc-1", new Date("2025-06-15"))
      expect(result).toEqual({ id: "av-1", effective_from: "2026-01-01", effective_to: "2026-12-31" })
    })
  })

  describe("filterSlotsWithExceptions", () => {
    it("returns all slots when no exceptions", async () => {
      jest.spyOn(service, "listAvailabilityExceptions" as any).mockResolvedValue([])

      const slots = [
        { start: new Date("2025-06-15T09:00:00"), end: new Date("2025-06-15T10:00:00") },
      ]
      const result = await service.filterSlotsWithExceptions(slots, new Date("2025-06-15"))
    ...
    it("filters slots within time_off exception range", async () => {
      ...
      const slotInRange = { start: new Date(`${dateStr}T10:00:00Z`), end: new Date(`${dateStr}T11:00:00Z`) }
      const slotOutRange = { start: new Date(`${dateStr}T13:00:00Z`), end: new Date(`${dateStr}T14:00:00Z`) }

      const date = new Date(`${dateStr}T10:00:00Z`)
      const result = await service.filterSlotsWithExceptions([slotInRange, slotOutRange], date)
      expect(result).toHaveLength(1)
  ...
  describe("isSlotAvailable", () => {
    it("returns true when slot is available", async () => {
      ...
      jest.spyOn(service, "listBookings" as any).mockResolvedValue([])

      const futureStart = new Date(Date.now() + 24 * 60 * 60 * 1000)
      const futureEnd = new Date(futureStart.getTime() + 60 * 60 * 1000)
      const result = await service.isSlotAvailable("svc-1", futureStart, futureEnd)
  ...
  describe("createBooking", () => {
    it("creates a booking with calculated end time", async () => {
      jest.spyOn(service, "retrieveServiceProduct" as any).mockResolvedValue({
        id: "svc-1", duration_minutes: 60, location_type: "in_person", location_address: "123 Main",
        min_advance_booking_hours: 0, max_advance_booking_days: 60, max_capacity: 10,
      })
      jest.spyOn(service, "isSlotAvailable" as any).mockResolvedValue(true)
      jest.spyOn(service, "generateBookingNumber" as any).mockResolvedValue("BK-TEST-1234")
      const createSpy = jest.spyOn(service as any, "createBookings").mockResolvedValue({ id: "bk-1" })
      jest.spyOn(service as any, "createBookingItems").mockResolvedValue({})
  ...
  describe("confirmBooking", () => {
    it("confirms a pending booking", async () => {
      jest.spyOn(service, "retrieveBooking" as any).mockResolvedValue({ id: "bk-1", status: "pending" })
      const updateSpy = jest.spyOn(service as any, "updateBookings").mockResolvedValue({ id: "bk-1", status: "confirmed" })
    ...
    it("throws when booking is not pending", async () => {
      jest.spyOn(service, "retrieveBooking" as any).mockResolvedValue({ id: "bk-1", status: "confirmed" })

      await expect(service.confirmBooking("bk-1")).rejects.toThrow("not in pending status")
    })
  })
  ...
  describe("checkInBooking", () => {
    it("checks in a confirmed booking", async () => {
      jest.spyOn(service, "retrieveBooking" as any).mockResolvedValue({ id: "bk-1", status: "confirmed" })
      const updateSpy = jest.spyOn(service as any, "updateBookings").mockResolvedValue({})

      await service.checkInBooking("bk-1")

      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({ status: "checked_in" }))
    })

    it("throws when booking is not confirmed", async () => {
      jest.spyOn(service, "retrieveBooking" as any).mockResolvedValue({ id: "bk-1", status: "pending" })

      await expect(service.checkInBooking("bk-1")).rejects.toThrow("must be confirmed")
    })
  })

  describe("completeBooking", () => {
    it("completes a checked_in booking", async () => {
      jest.spyOn(service, "retrieveBooking" as any).mockResolvedValue({ id: "bk-1", status: "checked_in" })
      const updateSpy = jest.spyOn(service as any, "updateBookings").mockResolvedValue({})

      await service.completeBooking("bk-1", "Great session")

      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({ status: "completed", provider_notes: "Great session" }))
    })

    it("throws when booking cannot be completed", async () => {
      jest.spyOn(service, "retrieveBooking" as any).mockResolvedValue({ id: "bk-1", status: "cancelled" })

      await expect(service.completeBooking("bk-1")).rejects.toThrow("cannot be completed")
    })
  })
  ...
  describe("cancelBooking", () => {
    it("cancels a booking and cancels reminders", async () => {
      jest.spyOn(service, "retrieveBooking" as any).mockResolvedValue({
        id: "bk-1", status: "confirmed", service_product_id: "svc-1",
        start_time: new Date(Date.now() + 48 * 60 * 60 * 1000).toISOString(), total: 5000,
      })
      jest.spyOn(service, "retrieveServiceProduct" as any).mockResolvedValue({
        cancellation_policy_hours: 24,
      })
      const updateSpy = jest.spyOn(service as any, "updateBookings").mockResolvedValue({})
      const cancelSpy = jest.spyOn(service, "cancelReminders" as any).mockResolvedValue(undefined)
    ...
    it("throws when booking is already completed", async () => {
      jest.spyOn(service, "retrieveBooking" as any).mockResolvedValue({ id: "bk-1", status: "completed" })

      await expect(service.cancelBooking("bk-1", "customer")).rejects.toThrow("cannot be cancelled")
    })
  })
  ...
  describe("markNoShow", () => {
    it("marks a confirmed booking as no_show", async () => {
      ...
      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({ status: "no_show" }))
    })

    it("throws when booking is not confirmed", async () => {
      jest.spyOn(service, "retrieveBooking" as any).mockResolvedValue({ id: "bk-1", status: "pending" })

      await expect(service.markNoShow("bk-1")).rejects.toThrow("Only confirmed bookings")
    })
  })

  describe("cancelReminders", () => {
    it("cancels all scheduled reminders for a booking", async () => {
      jest.spyOn(service, "listBookingReminders" as any).mockResolvedValue([
        { id: "rem-1", status: "scheduled" },
        { id: "rem-2", status: "scheduled" },
      ])
      const updateSpy = jest.spyOn(service as any, "updateBookingReminders").mockResolvedValue({})
  ...
  describe("getProviderStatistics", () => {
    it("calculates provider statistics correctly", async () => {
      ...
      const result = await service.getProviderStatistics("prov-1", periodStart, periodEnd)
    ...
    it("returns zero rates when no bookings", async () => {
      ...
      const result = await service.getProviderStatistics("prov-1", new Date(), new Date())
  ...
  describe("getCustomerBookings", () => {
    it("returns bookings for customer", async () => {
      jest.spyOn(service, "listBookings" as any).mockResolvedValue([{ id: "b1" }, { id: "b2" }])
