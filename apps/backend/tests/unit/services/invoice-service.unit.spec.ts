jest.mock("@medusajs/framework/utils", () => {
  ...
  return {
    MedusaService: () => class MockMedusaBase {
      async listInvoices(_f: any): Promise<any> { return [] }
      async createInvoices(_data: any): Promise<any> { return null }
      async createInvoiceItems(_data: any): Promise<any> { return [] }
      async updateInvoices(_data: any): Promise<any> { return null }
    },
    model: {
      define: () => ({ indexes: () => ({}) }),
...
describe("InvoiceModuleService", () => {
  ...
  describe("generateInvoiceNumber", () => {
    it("generates a properly formatted invoice number", async () => {
      jest.spyOn(service, "listInvoices" as any).mockResolvedValue([[], 3])
    ...
    it("uses first 4 characters of company ID as prefix", async () => {
      jest.spyOn(service, "listInvoices" as any).mockResolvedValue([[], 0])
  ...
  describe("createInvoiceWithItems", () => {
    it("creates an invoice with calculated totals", async () => {
      jest.spyOn(service, "generateInvoiceNumber" as any).mockResolvedValue("INV-TEST-202501-0001")
      const createInvSpy = jest.spyOn(service, "createInvoices" as any).mockResolvedValue({ id: "inv-1" })
      jest.spyOn(service, "createInvoiceItems" as any).mockResolvedValue([{ id: "ii-1" }])
      ...
      expect(createInvSpy).toHaveBeenCalledWith(expect.objectContaining({
        subtotal: 4000, total: 4000, amount_due: 4000, status: "draft",
      }))
    })
    ...
    it("defaults currency to usd and payment_terms_days to 30", async () => {
      ...
      await service.createInvoiceWithItems({
        company_id: "comp-1",
        issue_date: new Date(), due_date: new Date(),
    ...
    it("creates invoice items linked to the invoice", async () => {
      jest.spyOn(service, "generateInvoiceNumber" as any).mockResolvedValue("INV-TEST-202501-0001")
      jest.spyOn(service, "createInvoices" as any).mockResolvedValue({ id: "inv-1" })
      const createItemsSpy = jest.spyOn(service, "createInvoiceItems" as any).mockResolvedValue([])
      ...
      expect(createItemsSpy).toHaveBeenCalledWith([
        expect.objectContaining({ invoice_id: "inv-1", title: "Test", subtotal: 1500, total: 1500 }),
      ])
    })
  })
  ...
  describe("markAsSent", () => {
    it("updates invoice status to sent", async () => {
      const updateSpy = jest.spyOn(service, "updateInvoices" as any).mockResolvedValue({ id: "inv-1", status: "sent" })
  ...
  describe("markAsPaid", () => {
    it("marks invoice as paid when full amount is paid", async () => {
      jest.spyOn(service, "listInvoices" as any).mockResolvedValue([{ id: "inv-1", total: 5000, amount_paid: 0, status: "sent" }])
      const updateSpy = jest.spyOn(service, "updateInvoices" as any).mockResolvedValue({})

      await service.markAsPaid("inv-1")

      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({
        status: "paid", amount_paid: 5000, amount_due: 0,
      }))
    })
    ...
    it("applies partial payment without changing status", async () => {
      ...
      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({
        status: "sent", amount_paid: 2000, amount_due: 3000,
      }))
    })
    ...
    it("throws when invoice not found", async () => {
      ...
      await expect(service.markAsPaid("nonexistent")).rejects.toThrow("Invoice nonexistent not found")
    })

    it("accumulates payments across multiple calls", async () => {
      jest.spyOn(service, "listInvoices" as any).mockResolvedValue([{ id: "inv-1", total: 5000, amount_paid: 2000, status: "sent" }])
      const updateSpy = jest.spyOn(service, "updateInvoices" as any).mockResolvedValue({})
  ...
  describe("voidInvoice", () => {
    it("voids an invoice with reason", async () => {
      const updateSpy = jest.spyOn(service, "updateInvoices" as any).mockResolvedValue({ id: "inv-1", status: "void" })

      await service.voidInvoice("inv-1", "Duplicate invoice")

      expect(updateSpy).toHaveBeenCalledWith({
        id: "inv-1", status: "void", internal_notes: "Duplicate invoice",
      })
    })
  })
})
