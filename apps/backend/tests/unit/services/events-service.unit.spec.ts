  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listEventOutboxes(_filter: any, _config?: any): Promise<any> { return [] }
        async retrieveEventOutbox(_id: string): Promise<any> { return null }
        async createEventOutboxs(_data: any): Promise<any> { return {} }
        async updateEventOutboxs(_data: any): Promise<any> { return {} }
        async deleteEventOutboxs(_id: string): Promise<any> { return {} }
      },
    model: {
      define: () => ({ indexes: () => ({}) }),
      id: chainable,
...
describe("EventModuleService", () => {
  ...
  describe("retryFailedEvents", () => {
    it("retries events under the retry limit", async () => {
      jest.spyOn(service, "listEventOutboxes" as any).mockResolvedValue([
        { id: "e1", retry_count: 1, status: "failed" },
        { id: "e2", retry_count: 0, status: "failed" },
      ])
      const updateSpy = jest.spyOn(service as any, "updateEventOutboxs").mockResolvedValue({ id: "e1" })
      ...
      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({ status: "pending", error: null }))
    })
    ...
    it("skips events at or over the retry limit", async () => {
      ...
      jest.spyOn(service as any, "updateEventOutboxs").mockResolvedValue({ id: "e3" })
    ...
    it("filters by tenantId when provided", async () => {
      const listSpy = jest.spyOn(service, "listEventOutboxes" as any).mockResolvedValue([])

      await service.retryFailedEvents("tenant-1")

      expect(listSpy).toHaveBeenCalledWith({ status: "failed", tenant_id: "tenant-1" })
    })
  })
  ...
  describe("purgeOldEvents", () => {
    it("deletes published events older than the cutoff", async () => {
      ...
      jest.spyOn(service, "listEventOutboxes" as any).mockResolvedValue([
        ...
        { id: "e2", status: "published", published_at: recentDate.toISOString() },
      ])
      const deleteSpy = jest.spyOn(service as any, "deleteEventOutboxs").mockResolvedValue({})
    ...
    it("throws when olderThanDays is less than 1", async () => {
      await expect(service.purgeOldEvents(0)).rejects.toThrow("olderThanDays must be at least 1")
    })
  })
  ...
  describe("batchPublish", () => {
    it("publishes multiple events", async () => {
      jest.spyOn(service as any, "updateEventOutboxs").mockResolvedValue({ id: "e1" })
    ...
    it("throws when no event IDs provided", async () => {
      await expect(service.batchPublish([])).rejects.toThrow("No event IDs provided")
    })

    it("captures errors for individual events", async () => {
      jest.spyOn(service as any, "updateEventOutboxs")
