  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        ...
        async createPetProfiles(_data: any): Promise<any> { return {} }
        async updatePetProfiles(_data: any): Promise<any> { return {} }
        ...
        async listPetProducts(_filter: any): Promise<any> { return [] }
      },
    model: {
      define: () => ({ indexes: () => ({}) }),
      id: chainable,
...
describe("PetServiceModuleService", () => {
  ...
  describe("addPetProfile", () => {
    it("creates a pet profile successfully", async () => {
      const createSpy = jest.spyOn(service as any, "createPetProfiles").mockResolvedValue({ id: "pet-1", name: "Buddy" })

      const result = await service.addPetProfile("owner-1", { name: "Buddy", species: "dog", breed: "Golden Retriever" })

      expect(result.id).toBe("pet-1")
      expect(createSpy).toHaveBeenCalledWith(expect.objectContaining({ species: "dog", status: "active" }))
    })

    it("throws when name or species is missing", async () => {
      await expect(service.addPetProfile("owner-1", { name: "", species: "dog" }))
        .rejects.toThrow("Pet name and species are required")
    })

    it("throws when species is invalid", async () => {
      await expect(service.addPetProfile("owner-1", { name: "Rex", species: "dinosaur" }))
  ...
  describe("getServiceHistory", () => {
    it("returns sorted service history", async () => {
      jest.spyOn(service, "retrievePetProfile" as any).mockResolvedValue({ id: "pet-1" })
      jest.spyOn(service, "listGroomingBookings" as any).mockResolvedValue([
        { id: "g1", scheduled_date: "2025-01-01" },
        { id: "g2", scheduled_date: "2025-02-01" },
      ])
      jest.spyOn(service, "listVetAppointments" as any).mockResolvedValue([
        { id: "v1", appointment_date: "2025-01-15" },
      ])
  ...
  describe("calculateServiceCost", () => {
    it("calculates grooming cost for large dog", async () => {
      const result = await service.calculateServiceCost("grooming", "large", 60)
    ...
    it("applies duration multiplier for longer sessions", async () => {
      const result = await service.calculateServiceCost("bathing", "small", 120)
    ...
    it("uses default values for unknown service and size", async () => {
      const result = await service.calculateServiceCost("unknown_service", "unknown_size", 60)
