...
describe("LoyaltyModuleService", () => {
  ...
  describe("earnPoints", () => {
    it("adds points to an active account", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        id: "acc-1", status: "active", points_balance: 100, lifetime_points: 500, tenant_id: "t1", program_id: "p1",
      })
      const updateSpy = jest.spyOn(service, "updateLoyaltyAccounts" as any).mockResolvedValue({})
      const createSpy = jest.spyOn(service, "createPointTransactions" as any).mockResolvedValue({ id: "tx-1" })
      jest.spyOn(service, "calculateTier" as any).mockResolvedValue("silver")

      const result = await service.earnPoints({ accountId: "acc-1", points: 50 })

      expect(result).toEqual({ id: "tx-1" })
      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({
        id: "acc-1", points_balance: 150, lifetime_points: 550,
      }))
      expect(createSpy).toHaveBeenCalledWith(expect.objectContaining({
        account_id: "acc-1", type: "earn", points: 50, balance_after: 150,
      }))
    })

    it("throws when account is not active", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        id: "acc-1", status: "suspended", points_balance: 100, lifetime_points: 500,
      })

      await expect(service.earnPoints({ accountId: "acc-1", points: 50 }))
        .rejects.toThrow("Loyalty account is not active")
    })

    it("calls calculateTier after earning points", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        id: "acc-1", status: "active", points_balance: 0, lifetime_points: 0, tenant_id: "t1",
      })
      jest.spyOn(service, "updateLoyaltyAccounts" as any).mockResolvedValue({})
      jest.spyOn(service, "createPointTransactions" as any).mockResolvedValue({ id: "tx-1" })
      const tierSpy = jest.spyOn(service, "calculateTier" as any).mockResolvedValue(null)
  ...
  describe("redeemPoints", () => {
    it("deducts points from an active account", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        id: "acc-1", status: "active", points_balance: 200, lifetime_points: 500, tenant_id: "t1",
      })
      const updateSpy = jest.spyOn(service, "updateLoyaltyAccounts" as any).mockResolvedValue({})
      jest.spyOn(service, "createPointTransactions" as any).mockResolvedValue({ id: "tx-2" })

      const result = await service.redeemPoints({ accountId: "acc-1", points: 80 })

      expect(result).toEqual({ id: "tx-2" })
      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({
        id: "acc-1", points_balance: 120,
      }))
    })

    it("throws when account is not active", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        id: "acc-1", status: "frozen", points_balance: 200,
      })

      await expect(service.redeemPoints({ accountId: "acc-1", points: 50 }))
        .rejects.toThrow("Loyalty account is not active")
    })

    it("throws when insufficient balance", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        id: "acc-1", status: "active", points_balance: 30,
      })
    ...
    it("creates a redeem transaction with negative points", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        id: "acc-1", status: "active", points_balance: 100, tenant_id: "t1",
      })
      jest.spyOn(service, "updateLoyaltyAccounts" as any).mockResolvedValue({})
      const createSpy = jest.spyOn(service, "createPointTransactions" as any).mockResolvedValue({ id: "tx-3" })

      await service.redeemPoints({ accountId: "acc-1", points: 40 })

      expect(createSpy).toHaveBeenCalledWith(expect.objectContaining({
        type: "redeem", points: -40, balance_after: 60,
      }))
    })
  })
  ...
  describe("getBalance", () => {
    it("returns account balance info", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        points_balance: 250, lifetime_points: 1000, tier: "gold",
        tier_expires_at: "2025-12-31", status: "active",
      })
  ...
  describe("getTransactionHistory", () => {
    it("returns transactions with default pagination", async () => {
      ...
      jest.spyOn(service, "listPointTransactions" as any).mockResolvedValue(mockTxs)
    ...
    it("filters by type when provided", async () => {
      const listSpy = jest.spyOn(service, "listPointTransactions" as any).mockResolvedValue([])
  ...
  describe("calculateTier", () => {
    it("upgrades tier based on lifetime points", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        id: "acc-1", lifetime_points: 5000, tier: "bronze", program_id: "p1",
      })
      ...
      expect(updateSpy).toHaveBeenCalledWith(expect.objectContaining({ tier: "gold" }))
    })

    it("returns current tier when program has no tiers", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        id: "acc-1", lifetime_points: 100, tier: "basic", program_id: "p1",
      })
      jest.spyOn(service, "retrieveLoyaltyProgram" as any).mockResolvedValue({ tiers: null })
    ...
    it("does not update when tier has not changed", async () => {
      jest.spyOn(service, "retrieveLoyaltyAccount" as any).mockResolvedValue({
        id: "acc-1", lifetime_points: 500, tier: "bronze", program_id: "p1",
      })
      jest.spyOn(service, "retrieveLoyaltyProgram" as any).mockResolvedValue({
  ...
  describe("calculatePoints", () => {
    it("calculates points based on amount and program rules", async () => {
      jest.spyOn(service, "retrieveLoyaltyProgram" as any).mockResolvedValue({
        points_per_currency_unit: 2, multiplier: 1.5,
      })
    ...
    it("throws when amount is zero or negative", async () => {
      await expect(service.calculatePoints("p1", 0)).rejects.toThrow("Amount must be greater than zero")
      await expect(service.calculatePoints("p1", -10)).rejects.toThrow("Amount must be greater than zero")
    })
  })
  ...
  describe("getOrCreateAccount", () => {
    it("returns existing account if found", async () => {
      ...
      jest.spyOn(service, "listLoyaltyAccounts" as any).mockResolvedValue([existing])
    ...
    it("creates new account if none exists", async () => {
      ...
      const createSpy = jest.spyOn(service, "createLoyaltyAccounts" as any).mockResolvedValue({ id: "acc-new" })

      const result = await service.getOrCreateAccount("p1", "c1", "t1")

      expect(result).toEqual({ id: "acc-new" })
      expect(createSpy).toHaveBeenCalledWith(expect.objectContaining({
        program_id: "p1", customer_id: "c1", tenant_id: "t1",
        points_balance: 0, lifetime_points: 0, status: "active",
      }))
    })
  })
})
