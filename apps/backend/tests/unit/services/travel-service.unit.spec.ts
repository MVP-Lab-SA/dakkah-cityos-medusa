  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async listTravelProperties(_filter: any): Promise<any> { return [] }
        async retrieveTravelProperty(_id: string): Promise<any> { return null }
        async listRoomTypes(_filter: any): Promise<any> { return [] }
        async retrieveRoomType(_id: string): Promise<any> { return null }
        async listRooms(_filter: any): Promise<any> { return [] }
        async listTravelReservations(_filter: any): Promise<any> { return [] }
        async retrieveTravelReservation(_id: string): Promise<any> { return null }
        async createTravelReservations(_data: any): Promise<any> { return {} }
        async updateTravelReservations(_data: any): Promise<any> { return {} }
        async listAmenities(_filter: any): Promise<any> { return [] }
        async retrieveAmenity(_id: string): Promise<any> { return null }
        async listGuestProfiles(_filter: any): Promise<any> { return [] }
        async retrieveGuestProfile(_id: string): Promise<any> { return null }
        async listRatePlans(_filter: any): Promise<any> { return [] }
      },
    model: {
      define: () => ({ indexes: () => ({}) }),
      id: chainable,
...
describe("TravelModuleService", () => {
  ...
  describe("createBooking", () => {
    it("creates a booking with valid data", async () => {
      jest.spyOn(service, "retrieveRoomType" as any).mockResolvedValue({
        id: "rt-1",
        max_occupancy: 4,
        base_price: 100,
      })
      jest.spyOn(service, "listRooms" as any).mockResolvedValue([{ id: "room-1", status: "available" }])
      jest.spyOn(service, "listAmenities" as any).mockResolvedValue([])
      const createSpy = jest.spyOn(service as any, "createTravelReservations").mockResolvedValue({ id: "res-1", status: "confirmed" })
    ...
    it("throws when travelers exceed max occupancy", async () => {
      ...
      await expect(service.createBooking("rt-1", {
        customerId: "cust-1",
        travelers: 5,
        startDate: new Date("2025-06-01"),
        endDate: new Date("2025-06-05"),
  ...
  describe("cancelBooking", () => {
    it("cancels with 100% refund when check-in is far away", async () => {
      ...
      jest.spyOn(service, "retrieveTravelReservation" as any).mockResolvedValue({
        id: "res-1",
        status: "confirmed",
        check_in: futureCheckIn,
        total_price: 500,
      })
      jest.spyOn(service as any, "updateTravelReservations").mockResolvedValue({ id: "res-1", status: "cancelled" })
    ...
    it("cancels with 50% refund when check-in is within 3 days", async () => {
      ...
      jest.spyOn(service as any, "updateTravelReservations").mockResolvedValue({})
    ...
    it("throws when booking is already cancelled", async () => {
      ...
      await expect(service.cancelBooking("res-1")).rejects.toThrow("Booking is already cancelled")
    })
    ...
    it("throws when booking is checked out", async () => {
      ...
      await expect(service.cancelBooking("res-1")).rejects.toThrow("Cannot cancel a completed booking")
    })
  })
  ...
  describe("getItinerary", () => {
    it("returns full itinerary with property and amenities", async () => {
      ...
      jest.spyOn(service, "retrieveTravelProperty" as any).mockResolvedValue({
        id: "prop-1",
        name: "Grand Hotel",
        location: "Paris",
      ...
      jest.spyOn(service, "retrieveGuestProfile" as any).mockResolvedValue({ id: "guest-1", name: "John" })
  ...
  describe("calculatePackagePrice", () => {
    ...
    it("includes extras cost with per-traveler pricing", async () => {
      ...
      jest.spyOn(service, "retrieveAmenity" as any).mockResolvedValue({
        id: "a1",
        price: 20,
      })
