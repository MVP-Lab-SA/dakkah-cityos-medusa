...
function CreditManagementSection({ companyId }: { companyId: string }) {
  ...
  const [adjustType, setAdjustType] = useState<"add" | "subtract" | "reset">("subtract")
  ...
  const utilizationPercent = credit.credit_limit > 0 
    ...
    <Container className="p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="p-4 bg-ui-bg-subtle rounded-lg">
          ...
          <Text className="text-2xl font-bold">${credit.credit_limit.toLocaleString()}</Text>
        </div>
        <div className="p-4 bg-ui-bg-subtle rounded-lg">
          <Text className="text-ui-fg-subtle text-sm">Credit Used</Text>
          <Text className="text-2xl font-bold">${credit.credit_used.toLocaleString()}</Text>
        </div>
        <div className="p-4 bg-ui-bg-subtle rounded-lg">
          <Text className="text-ui-fg-subtle text-sm">Available Credit</Text>
          <Text className="text-2xl font-bold text-ui-fg-success">${credit.available_credit.toLocaleString()}</Text>
        </div>
        <div className="p-4 bg-ui-bg-subtle rounded-lg">
          <Text className="text-ui-fg-subtle text-sm">Payment Terms</Text>
          <Text className="text-2xl font-bold">Net {credit.payment_terms_days}</Text>
        </div>
      </div>
      
      <div className="mb-4">
        <Text className="text-sm text-ui-fg-subtle mb-1">Credit Utilization</Text>
        <div className="w-full bg-ui-bg-subtle rounded-full h-3">
          <div 
            className={`h-3 rounded-full ${
              utilizationPercent > 80 ? 'bg-ui-tag-red-bg' : 
              utilizationPercent > 50 ? 'bg-ui-tag-orange-bg' : 
              'bg-ui-tag-green-bg'
            }`}
            style={{ width: `${Math.min(utilizationPercent, 100)}%` }}
          />
        </div>
        <Text className="text-sm text-ui-fg-muted mt-1">{utilizationPercent}% utilized</Text>
      </div>
      
      <div className="grid grid-cols-2 gap-6 mt-6">
        <div className="p-4 border rounded-lg">
          <Heading level="h3" className="mb-3">Update Credit Limit</Heading>
          <div className="space-y-3">
            ...
            <Button onClick={handleUpdateLimit} isLoading={updateCreditLimit.isPending}>
              Update Limit
            </Button>
          </div>
        </div>
        ...
        <div className="p-4 border rounded-lg">
          <Heading level="h3" className="mb-3">Manual Credit Adjustment</Heading>
          <div className="space-y-3">
            <div>
              ...
              >
                <option value="subtract">Release Credit (Payment Received)</option>
            ...
            <Button onClick={handleAdjustCredit} isLoading={adjustCredit.isPending}>
              Apply Adjustment
            </Button>
          </div>
        </div>
    ...
    <Container className="p-6">
      ...
      <div className="mb-4 p-3 bg-ui-bg-subtle rounded-lg">
        <Text className="text-sm">
          Company Auto-Approve Limit: <strong>${parseFloat(data.company_auto_approve_limit || "0").toLocaleString()}</strong>
          {" | "}
          Requires Approval: <strong>{data.requires_approval ? "Yes" : "No"}</strong>
        </Text>
      </div>
              ...
              <tr key={user.id} className="border-b">
                ...
                <td className="p-3">
                  {editingUser === user.id ? (
                    ...
                    user.spending_limit ? `$${parseFloat(user.spending_limit).toLocaleString()}` : "No limit"
                  )}
                </td>
                <td className="p-3">
                  {user.can_approve ? <Check className="text-ui-fg-success" /> : <XMark className="text-ui-fg-muted" />}
                </td>
                <td className="p-3">
                  {editingUser === user.id ? (
                    <div className="flex gap-2">
                      <Button size="small" onClick={() => handleUpdate(user.id)}>Save</Button>
                      <Button size="small" variant="secondary" onClick={() => setEditingUser(null)}>Cancel</Button>
                    </div>
                  ) : (
...
function TaxExemptionsSection({ companyId }: { companyId: string }) {
  ...
  const handleVerify = async (exemption: TaxExemption, status: "verified" | "rejected") => {
    try {
  ...
  const handleDelete = async (exemptionId: string) => {
    ...
    try {
      await deleteExemption.mutateAsync({ companyId, exemption_id: exemptionId })
    ...
    <Container className="p-6">
      ...
      {showForm && (
        <div className="p-4 border rounded-lg mb-4 bg-ui-bg-subtle">
          <Heading level="h3" className="mb-3">Add Tax Exemption Certificate</Heading>
          ...
          <div className="flex gap-2 mt-4">
            <Button onClick={handleCreate} isLoading={createExemption.isPending}>
              Add Certificate
            </Button>
      ...
      {!exemptions?.length ? (
        ...
        <div className="space-y-3">
          {exemptions.map((exemption) => (
            <div key={exemption.id} className="p-4 border rounded-lg flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <Text className="font-medium">{exemption.certificate_number}</Text>
                  <Badge color={
                    exemption.status === "verified" ? "green" :
                    exemption.status === "rejected" ? "red" :
                    exemption.status === "expired" ? "orange" :
                    "grey"
                  }>
                    {exemption.status}
                  </Badge>
                ...
                <Text className="text-sm text-ui-fg-subtle">
                  {exemption.issuing_state && `State: ${exemption.issuing_state} | `}
                  {exemption.expiration_date && `Expires: ${new Date(exemption.expiration_date).toLocaleDateString()}`}
                </Text>
                {exemption.verified_at && (
                  <Text className="text-xs text-ui-fg-muted">
                    Verified on {new Date(exemption.verified_at).toLocaleDateString()}
                  </Text>
                )}
              </div>
              <div className="flex gap-2">
                {exemption.status === "pending" && (
                  <>
                    <Button size="small" onClick={() => handleVerify(exemption, "verified")}>
                      <Check /> Verify
                    </Button>
                    <Button size="small" variant="secondary" onClick={() => handleVerify(exemption, "rejected")}>
                      <XMark /> Reject
                    </Button>
                  </>
                )}
                <Button size="small" variant="transparent" onClick={() => handleDelete(exemption.id)}>
...
const CompanyDetailPage = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {company.status === "pending" && (
            <Button onClick={handleApprove} isLoading={approveCompany.isPending}>
      ...
      <Container className="p-6">
        <Heading level="h2" className="mb-4">Company Details</Heading>
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <Text className="font-medium">{new Date(company.created_at).toLocaleDateString()}</Text>
          </div>
        </div>
      </Container>
