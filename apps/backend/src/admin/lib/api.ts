...
export interface PurchaseOrder {
  ...
  customer_id: string
  po_number: string
  status: "draft" | "pending_approval" | "approved" | "rejected" | "partially_fulfilled" | "fulfilled" | "cancelled" | "closed"
  submitted_at?: string
  approved_at?: string
  approved_by?: string
  rejected_at?: string
...
export interface Subscription {
  ...
  customer_id: string
  plan_id?: string
  status: "trialing" | "active" | "paused" | "past_due" | "cancelled" | "expired"
...
export const companyApi = {
  list: async (params?: { limit?: number; offset?: number; status?: string }) => {
    const response = await fetch(`/admin/companies?${new URLSearchParams(params as any)}`)
...
export const vendorApi = {
  list: async (params?: { limit?: number; offset?: number; status?: string }) => {
    const response = await fetch(`/admin/vendors?${new URLSearchParams(params as any)}`)
  ...
  approve: async (id: string) => {
    const response = await fetch(`/admin/vendors/${id}/approve`, { method: "POST" })
    return response.json() as Promise<{ vendor: Vendor }>
  },
...
export const subscriptionApi = {
  list: async (params?: { limit?: number; offset?: number; status?: string }) => {
    const response = await fetch(`/admin/subscriptions?${new URLSearchParams(params as any)}`)
...
export const bookingApi = {
  list: async (params?: { limit?: number; offset?: number; status?: string; provider_id?: string }) => {
    const response = await fetch(`/admin/bookings?${new URLSearchParams(params as any)}`)
...
export const tenantApi = {
  list: async (params?: { limit?: number; offset?: number; status?: string }) => {
    const response = await fetch(`/admin/tenants?${new URLSearchParams(params as any)}`)
...
export const quoteApi = {
  list: async (params?: { limit?: number; offset?: number; status?: string }) => {
    const response = await fetch(`/admin/quotes?${new URLSearchParams(params as any)}`)
...
export const purchaseOrderApi = {
  list: async (params?: { limit?: number; offset?: number; status?: string; company_id?: string }) => {
    const response = await fetch(`/admin/purchase-orders?${new URLSearchParams(params as any)}`)
  ...
  approve: async (id: string) => {
    const response = await fetch(`/admin/purchase-orders/${id}/approve`, { method: "POST" })
    return response.json() as Promise<{ purchase_order: PurchaseOrder }>
  },
  reject: async (id: string, reason: string) => {
