...
export function useWarrantyTemplates(params?: { status?: string; coverage_type?: string }) {
  return useQuery({
    queryKey: ["warranty-templates", params],
    queryFn: async () => {
      const searchParams = new URLSearchParams()
      if (params?.status) searchParams.set("status", params.status)
      if (params?.coverage_type) searchParams.set("coverage_type", params.coverage_type)

      const query = searchParams.toString()
      const response = await sdk.client.fetch(`/admin/warranties${query ? `?${query}` : ""}`)
...
export function useUpdateWarrantyTemplate() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async ({ id, ...data }: Partial<WarrantyTemplate> & { id: string }) => {
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["warranty-templates"] })
      queryClient.invalidateQueries({ queryKey: ["warranty-templates", variables.id] })
    },
  })
}
