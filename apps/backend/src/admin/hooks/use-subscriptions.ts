...
export type Subscription = {
  ...
  customer_id: string
  plan_id: string
  status: "trialing" | "active" | "paused" | "canceled" | "past_due" | "expired"
...
export type SubscriptionEvent = {
  id: string
  subscription_id: string
  type: "created" | "activated" | "paused" | "resumed" | "canceled" | "renewed" | "plan_changed" | "payment_failed"
...
export function useSubscriptionPlans(params?: { is_active?: boolean }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await sdk.client.fetch(`/admin/subscription-plans${query ? `?${query}` : ""}`)
...
export function useSubscriptionPlan(id: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch(`/admin/subscription-plans/${id}`)
...
// Subscriptions hooks
export function useSubscriptions(params?: { status?: string; plan_id?: string; customer_id?: string }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const query = searchParams.toString()
      const response = await sdk.client.fetch(`/admin/subscriptions${query ? `?${query}` : ""}`)
...
export function usePauseSubscription() {
  ...
  return useMutation({
    mutationFn: async ({ id, pause_ends_at }: { id: string; pause_ends_at?: string }) => {
      const response = await sdk.client.fetch(`/admin/subscriptions/${id}/pause`, {
        method: "POST",
        body: { pause_ends_at },
      })
      return response as { subscription: Subscription }
...
export function useResumeSubscription() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response = await sdk.client.fetch(`/admin/subscriptions/${id}/resume`, {
        method: "POST",
      })
      return response as { subscription: Subscription }
    },
...
export function useCancelSubscription() {
  ...
  return useMutation({
    mutationFn: async ({ id, reason, immediate }: { id: string; reason?: string; immediate?: boolean }) => {
      const response = await sdk.client.fetch(`/admin/subscriptions/${id}/cancel`, {
        method: "POST",
...
export function useChangePlan() {
  ...
  return useMutation({
    mutationFn: async ({ id, plan_id, prorate }: { id: string; plan_id: string; prorate?: boolean }) => {
      const response = await sdk.client.fetch(`/admin/subscriptions/${id}/change-plan`, {
        method: "POST",
