...
export type Persona = {
  id: string
  name: string
  type: "consumer" | "merchant" | "government" | "enterprise" | "visitor" | "operator"
...
export function usePersonas(params?: { type?: string; status?: string; search?: string }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await sdk.client.fetch(`/admin/personas${query ? `?${query}` : ""}`)
