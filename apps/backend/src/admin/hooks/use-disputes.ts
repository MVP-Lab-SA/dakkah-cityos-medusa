...
export function useDisputes(params?: { status?: string; customer_id?: string; order_id?: string }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await sdk.client.fetch(`/admin/disputes${query ? `?${query}` : ""}`)
...
export function useEscalateDispute() {
  ...
  return useMutation({
    mutationFn: async ({ id, reason }: { id: string; reason?: string }) => {
      const response = await sdk.client.fetch(`/admin/disputes/${id}/escalate`, {
        method: "POST",
        body: { reason },
      })
      return response as { dispute: Dispute }
...
export function useResolveDispute() {
  ...
  return useMutation({
    mutationFn: async ({ id, resolution, resolution_type }: { id: string; resolution: string; resolution_type: string }) => {
