...
// Tenants hooks
export function useTenants(params?: { status?: string; plan?: string; search?: string }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await sdk.client.fetch(`/admin/tenants${query ? `?${query}` : ""}`)
...
export function useTenantUsers(tenantId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch(`/admin/tenants/${tenantId}/users`)
...
export function useAddTenantUser() {
  ...
  return useMutation({
    mutationFn: async ({ tenantId, ...data }: { tenantId: string; email: string; name?: string; role: string }) => {
      const response = await sdk.client.fetch(`/admin/tenants/${tenantId}/users`, {
        method: "POST",
        body: data,
      })
...
export function useTenantBilling(tenantId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch(`/admin/tenants/${tenantId}/billing`)
...
// Tenant usage hooks
export function useTenantUsage(tenantId: string, params?: { date_from?: string; date_to?: string }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const query = searchParams.toString()
      const response = await sdk.client.fetch(`/admin/tenants/${tenantId}/usage${query ? `?${query}` : ""}`)
