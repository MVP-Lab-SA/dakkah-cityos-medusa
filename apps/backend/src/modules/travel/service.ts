...
class TravelModuleService extends MedusaService({
  ...
  async createBooking(packageId: string, data: {
    customerId: string
    travelers: number
    startDate: Date
    endDate: Date
    ...
    if ((roomType.max_occupancy || 2) < data.travelers) {
      throw new Error(`Package supports maximum ${roomType.max_occupancy || 2} travelers`)
    }

    const available = await this.checkAvailability(packageId, new Date(data.startDate))
    if (!available) {
      throw new Error("Package is not available for the selected dates")
    }
  ...
  async cancelBooking(bookingId: string, reason?: string): Promise<any> {
    ...
    const updated = await (this as any).updateTravelReservations({
      id: bookingId,
      status: "cancelled",
    ...
    return {
      booking: updated,
      refundPercentage,
      refundAmount: Number(reservation.total_price || 0) * (refundPercentage / 100),
    }
  }
  ...
  async getItinerary(bookingId: string): Promise<any> {
    ...
    const amenities = await this.listAmenities({ room_type_id: reservation.room_type_id }) as any
    const amenityList = Array.isArray(amenities) ? amenities : [amenities].filter(Boolean)
  ...
  async calculatePackagePrice(packageId: string, travelers: number, extras?: string[]): Promise<{
    basePrice: number
    extrasCost: number
    discount: number
    total: number
