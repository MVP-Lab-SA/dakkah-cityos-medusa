...
class EducationModuleService extends MedusaService({
  ...
  async enrollStudent(courseId: string, studentId: string): Promise<any> {
    ...
    const existing = await this.listEnrollments({ course_id: courseId, student_id: studentId }) as any
    const list = Array.isArray(existing) ? existing : [existing].filter(Boolean)
    if (list.length > 0) {
      throw new Error("Student is already enrolled in this course")
    }

    if (course.max_students && Number(course.enrolled_count || 0) >= Number(course.max_students)) {
      throw new Error("Course is at full capacity")
    }

    const enrollment = await (this as any).createEnrollments({
      course_id: courseId,
    ...
    await (this as any).updateCourses({
      id: courseId,
  ...
  async trackProgress(enrollmentId: string, lessonId: string): Promise<any> {
    ...
    const lessons = await this.listLessons({ course_id: enrollment.course_id }) as any
    ...
    return await (this as any).updateEnrollments({
      id: enrollmentId,
  ...
  async issueCertificate(enrollmentId: string): Promise<any> {
    ...
    if (Number(enrollment.progress) < 100) {
      throw new Error("Course must be fully completed before issuing a certificate")
    }

    const existing = await this.listCertificates({ enrollment_id: enrollmentId }) as any
    const certList = Array.isArray(existing) ? existing : [existing].filter(Boolean)
    ...
    const certificate = await (this as any).createCertificates({
    ...
    await (this as any).updateEnrollments({
      id: enrollmentId,
      status: "completed",
  ...
  async getCompletionRate(courseId: string): Promise<{ total: number; completed: number; rate: number }> {
