...
class RentalModuleService extends MedusaService({
  ...
  async checkAvailability(itemId: string, startDate: Date, endDate: Date): Promise<boolean> {
    ...
    const list = Array.isArray(agreements) ? agreements : [agreements].filter(Boolean)
  ...
  async createRental(itemId: string, customerId: string, startDate: Date, endDate: Date): Promise<any> {
    ...
    const agreement = await (this as any).createRentalAgreements({
      rental_product_id: itemId,
  ...
  async calculateRentalPrice(itemId: string, durationDays: number): Promise<number> {
    if (durationDays <= 0) {
  ...
  async processReturn(rentalId: string, condition?: string, notes?: string): Promise<any> {
    ...
    const returnRecord = await (this as any).createRentalReturns({
    ...
    await (this as any).updateRentalAgreements({
      id: rentalId,
      status: "returned",
      actual_return_date: new Date(),
    })

    await (this as any).updateRentalProducts({
