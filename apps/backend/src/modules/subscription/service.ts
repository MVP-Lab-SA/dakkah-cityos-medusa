...
class SubscriptionModuleService extends MedusaService({
  ...
  async createSubscriptionFromPlan(
    ...
    // Create subscription
    const subscription = await (this as any).createSubscriptions({
    ...
    await (this as any).createSubscriptionItems({
    ...
    if (options?.discountCode) {
      await this.applyDiscountToSubscription(subscription.id, options.discountCode);
    }
  ...
  async activateSubscription(subscriptionId: string): Promise<any> {
    ...
    const updated = await (this as any).updateSubscriptions({
      id: subscriptionId,
      status: "active",
      start_date: now,
  ...
  async pauseSubscription(
    ...
    // Create pause record
    await (this as any).createSubscriptionPauses({
  ...
  async resumeSubscription(subscriptionId: string): Promise<any> {
    ...
    if (activePause) {
      ...
      await (this as any).updateSubscriptionPauses({
        id: activePause.id,
        resumed_at: new Date(),
      ...
      if (activePause.extends_billing_period && subscription.current_period_end) {
        const currentEnd = new Date(subscription.current_period_end);
        currentEnd.setDate(currentEnd.getDate() + daysPaused);
        
        await (this as any).updateSubscriptions({
          id: subscriptionId,
  ...
  async cancelSubscription(
    ...
    const updated = await (this as any).updateSubscriptions(updates);
   ...
   */
  async createBillingCycleForSubscription(subscriptionId: string): Promise<any> {
    const subscription = await this.retrieveSubscription(subscriptionId);

    return await (this as any).createBillingCycles({
      subscription_id: subscriptionId,
  ...
  async processBillingCycle(billingCycleId: string): Promise<any> {
    const cycle = await this.retrieveBillingCycle(billingCycleId);
    ...
    // Mark as processing
    await (this as any).updateBillingCycles({
      id: billingCycleId,
      status: "processing",
    });
    ...
    // Log payment success
    await this.logSubscriptionEvent(cycle.subscription_id, "payment_succeeded", {
  ...
  async handleFailedBilling(
    ...
    const cycle = await this.retrieveBillingCycle(billingCycleId);
    const subscription = await this.retrieveSubscription(cycle.subscription_id);
  ...
  async changePlan(
    ...
    const subscription = await this.retrieveSubscription(subscriptionId);
    const newPlan = await this.retrieveSubscriptionPlan(newPlanId);
    ...
    if (options?.effectiveDate === "next_period") {
      ...
      if (options?.prorate && subscription.current_period_end && subscription.current_period_start) {
        // Calculate proration
      ...
      for (const item of itemList) {
        await (this as any).deleteSubscriptionItems(item.id);
      }
    ...
    return await this.retrieveSubscription(subscriptionId);
  }
  ...
  async applyDiscountToSubscription(
    ...
    const discountList = Array.isArray(discounts) ? discounts : [discounts].filter(Boolean);
    ...
    if (discount.discount_type === "percentage") {
      discountAmount = Number(subscription.subtotal) * (Number(discount.discount_value) / 100);
    ...
    await (this as any).updateSubscriptionDiscounts({
      id: discount.id,
  ...
  async logSubscriptionEvent(
    ...
    return await (this as any).createSubscriptionEvents({
      subscription_id: subscriptionId,
      tenant_id: subscription.tenant_id,
  ...
  async getCustomerSubscriptions(customerId: string): Promise<any[]> {
    ...
    return Array.isArray(subscriptions) ? subscriptions : [subscriptions].filter(Boolean);
  }
  ...
  async getSubscriptionsDueForBilling(beforeDate: Date): Promise<any[]> {
    ...
    const cycleList = (Array.isArray(cycles) ? cycles : [cycles].filter(Boolean))
