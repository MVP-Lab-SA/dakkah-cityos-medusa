...
class FinancialProductModuleService extends MedusaService({
  ...
  async applyForProduct(customerId: string, productId: string, data: {
    amount: number
    term: number
    purpose?: string
  }): Promise<any> {
    ...
    const product = await this.retrieveLoanProduct(productId)
    ...
    return await (this as any).createLoanApplications({
      customer_id: customerId,
      product_id: productId,
      amount: data.amount,
    ...
    if (application.product_id) {
      try {
        ...
        if (product.max_amount && amount > Number(product.max_amount)) {
          reasons.push(`Amount exceeds product maximum of ${product.max_amount}`)
          score -= 25
        }
      } catch {
  ...
  async approveApplication(applicationId: string, approvedBy: string, terms?: {
    interestRate?: number
    fees?: number
  }): Promise<any> {
    const application = await this.retrieveLoanApplication(applicationId) as any
    ...
    return await (this as any).updateLoanApplications({
      id: applicationId,
      status: "approved",
  ...
  calculateRepaymentSchedule(principal: number, interestRate: number, termMonths: number): {
