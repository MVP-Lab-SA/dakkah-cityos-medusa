...
class AdvertisingModuleService extends MedusaService({
  ...
  async createCampaign(data: {
    ...
    return await (this as any).createAdCampaigns({
  ...
  async trackImpression(adCreativeId: string, metadata?: Record<string, unknown>): Promise<any> {
    const creative = await this.retrieveAdCreative(adCreativeId) as any

    const log = await (this as any).createImpressionLogs({
    ...
    if (creative.campaign_id || creative.ad_campaign_id) {
      ...
      await (this as any).updateAdCampaigns({
        id: campaignId,
  ...
  /** Track an ad click */
  async trackClick(adCreativeId: string, metadata?: Record<string, unknown>): Promise<any> {
    const creative = await this.retrieveAdCreative(adCreativeId) as any
  ...
  async calculateCTR(campaignId: string): Promise<{ impressions: number; clicks: number; ctr: number }> {
    const campaign = await this.retrieveAdCampaign(campaignId) as any
