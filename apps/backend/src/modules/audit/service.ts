...
class AuditModuleService extends MedusaService({
  ...
  async getAuditSummary(tenantId: string, dateRange: { start: Date; end: Date }) {
    const logs = await this.listAuditLogs({ tenant_id: tenantId }, {
      take: 5000,
    ...
    const riskFlags: Array<{ type: string; description: string; severity: string }> = []
    ...
    const deleteCount = eventsByType["delete"] || eventsByType["bulk_delete"] || 0
    if (deleteCount > 10) {
      riskFlags.push({
    ...
    const restrictedAccess = result.filter((l: any) => l.data_classification === "restricted").length
  ...
  async flagSuspiciousActivity(tenantId: string) {
    ...
    for (const [actorId, actions] of Object.entries(actorActions)) {
      ...
      const deleteActions = actions.filter((a: any) =>
      ...
      const restrictedAccess = actions.filter((a: any) =>
        a.data_classification === "restricted" || a.data_classification === "confidential"
      )
      ...
      const uniqueIps = new Set(actions.map((a: any) => a.ip_address).filter(Boolean))
      if (uniqueIps.size > 5) {
        flags.push({
          type: "multiple_ips",
          actorId,
  ...
  async exportAuditReport(tenantId: string, format: string) {
    ...
    const summary = await this.getAuditSummary(tenantId, { start: thirtyDaysAgo, end: now })
    ...
    if (format === "csv") {
      const headers = ["id", "action", "resource_type", "resource_id", "actor_id", "actor_role", "ip_address", "data_classification", "created_at"]
