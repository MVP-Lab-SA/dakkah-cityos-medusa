...
class I18nModuleService extends MedusaService({
  ...
  async getTranslations(tenantId: string, locale: string, namespace?: string) {
    ...
    return Array.isArray(translations) ? translations : [translations].filter(Boolean)
  }

  async getTranslation(tenantId: string, locale: string, key: string, namespace?: string) {
    const query: Record<string, any> = {
      tenant_id: tenantId,
      locale,
      key,
    ...
    const translations = await this.listTranslations(query) as any
    const list = Array.isArray(translations) ? translations : [translations].filter(Boolean)
    return list[0] || null
  }
  ...
  async upsertTranslation(
    ...
    if (existing) {
      return await (this as any).updateTranslations({
        id: existing.id,
        value,
        status: "published",
      })
    ...
    return await (this as any).createTranslations({
      tenant_id: tenantId,
      locale,
      namespace: ns,
      key,
