...
class LegalModuleService extends MedusaService({
  ...
  async addDocument(caseId: string, data: { title: string; documentType: string; fileUrl: string; uploadedBy: string }): Promise<any> {
    ...
    const retainer = await (this as any).createRetainerAgreements({
      case_id: caseId,
      title: data.title,
  ...
  async getBillingSummary(caseId: string): Promise<{
    ...
    const outstandingBalance = Math.round((totalBillable - paidAmount) * 100) / 100
  ...
  async scheduleConsultation(caseId: string, data: { date: Date; duration: number; attendees: string[] }): Promise<any> {
    ...
    const consultation = await (this as any).createLegalConsultations({
      case_id: caseId,
