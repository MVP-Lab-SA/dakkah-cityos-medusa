...
const MANAGED_SYSTEMS = ["stripe", "erpnext", "fleetbase", "payload-cms", "waltid"] as const
...
export class OutboxProcessor {
  async processOutbox(container: any): Promise<{ processed: number; failed: number; skipped: number; errors: string[] }> {
    let processed = 0
    let failed = 0
    let skipped = 0
    const errors: string[] = []
    ...
    if (processed > 0 || failed > 0) {
      console.log(`[OutboxProcessor] Processed: ${processed}, Failed: ${failed}, Skipped: ${skipped}`)
    }
  ...
  async processEvent(event: any, container: any): Promise<void> {
    ...
    if (!system) {
      console.log(`[OutboxProcessor] No system mapping for event type: ${event.event_type}`)
      return
    }
  ...
  private routeEventToSystem(eventType: string): ManagedSystem | null {
    if (eventType.startsWith("payment") || eventType.startsWith("charge") || eventType.startsWith("invoice") || eventType.startsWith("checkout")) {
      return "stripe"
    }
    if (eventType.startsWith("erp") || eventType.includes("invoice") || eventType.includes("accounting")) {
      return "erpnext"
    }
    if (eventType.startsWith("fulfillment") || eventType.startsWith("shipment") || eventType.startsWith("delivery")) {
      return "fleetbase"
    }
    if (eventType.startsWith("content") || eventType.startsWith("product.") || eventType.startsWith("page.")) {
      return "payload-cms"
    }
    if (eventType.startsWith("credential") || eventType.startsWith("did") || eventType.startsWith("kyc")) {
      return "waltid"
    }
    return null
  }
  ...
  private async dispatchToSystem(system: ManagedSystem, event: any, container: any): Promise<void> {
    ...
    console.log(`[OutboxProcessor] Dispatching ${event.event_type} to ${system} (correlation: ${correlationId})`)
  ...
  private computeHash(event: any): string {
    const data = JSON.stringify({ type: event.event_type, payload: event.payload, tenant: event.tenant_id })
