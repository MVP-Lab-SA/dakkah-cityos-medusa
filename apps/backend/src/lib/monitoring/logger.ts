...
class CityOSLogger {
  ...
  private log(level: LogLevel, message: string, context?: LogContext, extra?: Partial<LogEntry>) {
  ...
  async timed<T>(
    ...
    try {
      ...
      this.log("info", `${operation} completed`, context, { duration_ms: duration })
      return result
    } catch (error) {
      const duration = Date.now() - start
   ...
   */
  payout(action: string, vendorId: string, amount: number, details?: Record<string, unknown>) {
