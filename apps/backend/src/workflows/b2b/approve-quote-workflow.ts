...
const validateQuoteStep = createStep(
  ...
  async (input: ApproveQuoteInput, { container }) => {
    ...
    const quote = await (quoteService as { retrieveQuote: (id: string) => Promise<Record<string, unknown>> }).retrieveQuote(input.quote_id);
...
const applyDiscountStep = createStep(
  ...
  async ({ input, quote }: { input: ApproveQuoteInput; quote: Record<string, unknown> }, { container }) => {
    if (input.custom_discount_percentage || input.custom_discount_amount) {
      ...
      await (quoteService as { applyCustomDiscount: (id: string, pct?: number, amt?: bigint, reason?: string) => Promise<void> }).applyCustomDiscount(
        input.quote_id,
...
const updateQuoteStatusStep = createStep(
  ...
  async ({ input, quote }: { input: ApproveQuoteInput; quote: Record<string, unknown> }, { container }) => {
    ...
    const approvedQuote = await (quoteService as { updateQuotes: (data: Record<string, unknown>) => Promise<Record<string, unknown>> }).updateQuotes({
      id: input.quote_id,
      status: "approved",
...
const sendNotificationStep = createStep(
  ...
  async ({ approvedQuote }: { approvedQuote: Record<string, unknown> }, { container }) => {
    try {
      ...
      const notificationService = container.resolve("notification") as Record<string, unknown> | null;
      
      if (notificationService && typeof (notificationService as { send?: unknown }).send === "function") {
        await (notificationService as { send: (data: Record<string, unknown>) => Promise<void> }).send({
        ...
        logger.info("quote-approved-notification", `Notification sent for quote ${approvedQuote.quote_number}`);
      } else {
        // Fallback: emit event for subscriber to handle
        const eventBus = container.resolve("event_bus") as { emit: (event: string, data: Record<string, unknown>) => Promise<void> };
