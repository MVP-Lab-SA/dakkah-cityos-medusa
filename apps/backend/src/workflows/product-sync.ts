...
const upsertProductsStep = createStep(
  ...
  async (input: { transformed: any[]; targetSystem: string }, { container }) => {
...
const verifySyncStep = createStep(
  ...
  async (input: { syncResults: any[] }) => {
    ...
    return new StepResponse({ verified: true, failedCount: failed.length, totalCount: input.syncResults.length })
  }
)
...
export const productSyncWorkflow = createWorkflow(
  ...
  (input: ProductSyncInput) => {
    ...
    const { transformed } = transformProductsStep({ products, targetSystem: input.targetSystem })
    const { syncResults } = upsertProductsStep({ transformed, targetSystem: input.targetSystem })
