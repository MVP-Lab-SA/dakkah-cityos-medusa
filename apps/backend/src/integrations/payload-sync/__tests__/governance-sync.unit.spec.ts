...
describe("MedusaToPayloadSync - syncGovernancePolicies", () => {
  ...
  beforeEach(() => {
    ...
    mockGovernanceModule.resolveEffectivePolicies.mockResolvedValue(mockEffectivePolicies)
    mockGovernanceModule.listGovernanceAuthorities.mockResolvedValue([mockAuthority])
  })
  ...
  it("creates new policy when none exists (POST)", async () => {
    ...
    expect(mockGovernanceModule.resolveEffectivePolicies).toHaveBeenCalledWith(TEST_TENANT_ID)
    expect(mockGovernanceModule.listGovernanceAuthorities).toHaveBeenCalledWith({ tenant_id: TEST_TENANT_ID })
  ...
  it("throws on API error", async () => {
    ...
    await expect(sync.syncGovernancePolicies(TEST_TENANT_ID)).rejects.toThrow("Network Error")
  })
