...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    const { data: currentOrders } = await query.graph({
      entity: "vendor_order",
      fields: ["id", "total", "commission_amount", "net_amount", "created_at", "status"],
      filters: {
        vendor_id: vendorId,
    ...
    // Calculate summaries
    const totalRevenue = currentOrders.reduce((sum: number, o: any) => sum + (Number(o.total) || 0), 0)
    const totalCommission = currentOrders.reduce((sum: number, o: any) => sum + (Number(o.commission_amount) || 0), 0)
    const netEarnings = currentOrders.reduce((sum: number, o: any) => sum + (Number(o.net_amount) || 0), 0)
    ...
    const productMap = new Map<string, { title: string; units_sold: number; revenue: number }>()
