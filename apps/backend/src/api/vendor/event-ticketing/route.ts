...
const createSchema = z.object({
  ...
  ticket_type: z.enum(["general", "vip", "early_bird", "group", "student", "reserved"]).optional(),
...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  const mod = req.scope.resolve("eventTicketing") as any
  const { limit = "20", offset = "0", event_id, status } = req.query as Record<string, string | undefined>
