...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    const items = await mod.listGiftCardExts(filters, {
      skip: Number(offset),
      take: Number(limit),
    ...
    handleApiError(res, error, "GET vendor gift-cards")
  }
}
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    const item = await mod.createGiftCardExts({
      ...validation.data,
      remaining_value: validation.data.initial_value,
      vendor_id: vendorId,
    })
    ...
    handleApiError(res, error, "POST vendor gift-cards")
  }
}
