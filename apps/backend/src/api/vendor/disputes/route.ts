...
const createSchema = z.object({
  ...
  reason: z.enum(["product_not_received", "product_damaged", "wrong_item", "quality_issue", "refund_request", "other"]),
...
const updateSchema = z.object({
  ...
  proposed_resolution: z.enum(["full_refund", "partial_refund", "replacement", "repair", "reject"]).optional(),
  proposed_amount: z.number().nullable().optional(),
  status: z.enum(["pending", "vendor_responded", "escalated", "resolved", "closed"]).optional(),
  metadata: z.record(z.string(), z.unknown()).nullable().optional(),
})
...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  const mod = req.scope.resolve("dispute") as any
  const { limit = "20", offset = "0", status, reason } = req.query as Record<string, string | undefined>
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const disputeId = (req.query as any).id
  if (!disputeId) {
