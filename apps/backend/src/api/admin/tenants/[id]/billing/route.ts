...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  let billing = null
  try {
    const { data: [billingData] } = await query.graph({
  ...
  res.json({
    ...
    usage,
    limits: {
      max_orders_per_month: billing?.plan === "enterprise" ? -1 : billing?.plan === "pro" ? 10000 : 100,
      max_products: billing?.plan === "enterprise" ? -1 : billing?.plan === "pro" ? 10000 : 100,
      max_storage_mb: billing?.plan === "enterprise" ? -1 : billing?.plan === "pro" ? 10000 : 500,
      max_users: billing?.plan === "enterprise" ? -1 : billing?.plan === "pro" ? 50 : 5,
    },
  })
}
