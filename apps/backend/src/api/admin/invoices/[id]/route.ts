...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  const { data: invoices } = await query.graph({
    ...
    fields: [
      ...
      "period_start", "period_end", "payment_terms", "payment_terms_days",
      "notes", "internal_notes", "pdf_url", "items.*", "metadata",
  ...
  if (invoice.company_id) {
    const { data: companies } = await query.graph({
      entity: "company",
      fields: ["id", "name", "email", "phone", "address", "city", "state", "postal_code", "country"],
