...
export async function PUT(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    logger.info(`Credit adjusted for company ${id}: ${JSON.stringify({ credit_limit, payment_terms_days, reason })}`)
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    logger.info(`Credit usage adjusted for company ${id}: ${type} ${amount}, reason: ${reason}`)
