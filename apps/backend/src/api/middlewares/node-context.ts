...
export async function nodeContextMiddleware(
  ...
  const tenantSlug = req.headers["x-tenant-slug"] as string || req.query?.tenant_slug as string
  const locale = req.headers["x-locale"] as string || req.query?.locale as string || "en"
  const channel = req.headers["x-channel"] as string || req.query?.channel as string || "web"
  const nodeId = req.headers["x-node-id"] as string || req.query?.node_id as string
  const personaId = req.headers["x-persona-id"] as string || req.query?.persona_id as string
  ...
  try {
    ...
    req.nodeContext = {
      ...
      nodeId,
      nodeType,
      userId: (req as any).auth_context?.actor_id,
      personaId,
