...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    res.status(500).json({ message: "Failed to fetch digital products", error: error.message })
  }
}
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    res.status(400).json({ message: error.message || "Failed to create digital product" })
  }
}
