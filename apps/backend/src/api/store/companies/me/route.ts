...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    // Count employees
    const allEmployees = await companyModule.listCompanyEmployees({ 
      company_id: company.id 
    })
    stats.employee_count = (Array.isArray(allEmployees) ? allEmployees : [allEmployees].filter(Boolean)).length
    ...
    stats.pending_orders = (Array.isArray(pendingPOs) ? pendingPOs : [pendingPOs].filter(Boolean)).length
    ...
    res.json({
      company: {
        ...
        available_credit: Number(company.credit_limit || 0) - Number(company.credit_used || 0),
