...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    const { limit = "20", offset = "0", tenant_id, make, model, year, condition } = req.query as Record<string, string | undefined>
    ...
    res.status(500).json({ message: "Failed to fetch vehicle listings", error: error.message })
  }
}
