...
describe("GET /store/cityos/governance", () => {
  ...
  it("resolves tenantId from nodeContext", async () => {
    ...
    mockGovernanceModule.listGovernanceAuthorities.mockResolvedValue(authorities)

    await GET(mockReq, mockRes)

    expect(mockGovernanceModule.resolveEffectivePolicies).toHaveBeenCalledWith("tenant_123")
    expect(mockGovernanceModule.listGovernanceAuthorities).toHaveBeenCalledWith({ tenant_id: "tenant_123" })
  ...
  it("resolves tenantId from query param when nodeContext is absent", async () => {
    ...
    expect(mockGovernanceModule.resolveEffectivePolicies).toHaveBeenCalledWith("tenant_456")
    expect(mockGovernanceModule.listGovernanceAuthorities).toHaveBeenCalledWith({ tenant_id: "tenant_456" })
    expect(mockRes.json).toHaveBeenCalledWith({
      authorities,
      effective_policies: policies,
    })
  ...
  it("wraps a single authority object in an array", async () => {
    ...
    mockGovernanceModule.resolveEffectivePolicies.mockResolvedValue({})
    mockGovernanceModule.listGovernanceAuthorities.mockResolvedValue(singleAuthority)
  ...
  it("defaults authorities to empty array when listGovernanceAuthorities throws", async () => {
    mockGovernanceModule.resolveEffectivePolicies.mockResolvedValue({ policy: "value" })
    mockGovernanceModule.listGovernanceAuthorities.mockRejectedValue(new Error("DB error"))
  ...
  it("returns 500 when resolveEffectivePolicies throws", async () => {
    mockGovernanceModule.resolveEffectivePolicies.mockRejectedValue(new Error("Fatal error"))
  ...
  it("returns correct response shape with authorities and effective_policies", async () => {
    const policies = { featureFlags: { billing: true }, limits: { products: 100 } }
    const authorities = [{ id: "auth_1", type: "municipal" }, { id: "auth_2", type: "regional" }]
    mockGovernanceModule.resolveEffectivePolicies.mockResolvedValue(policies)
    mockGovernanceModule.listGovernanceAuthorities.mockResolvedValue(authorities)
