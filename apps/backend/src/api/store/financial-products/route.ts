...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    const { limit = "20", offset = "0", tenant_id, product_type } = req.query as Record<string, string | undefined>
    ...
    res.status(500).json({ message: "Failed to fetch financial products", error: error.message })
  }
}
