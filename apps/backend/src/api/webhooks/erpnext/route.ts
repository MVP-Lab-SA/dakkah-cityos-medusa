...
async function handleInventoryUpdate(data: any, correlationId: string) {
  console.log(`[Webhook:ERPNext] inventory_update: item=${data.item_code || "unknown"}, warehouse=${data.warehouse || "unknown"}, correlation: ${correlationId}`)
}

async function handleInvoiceStatusChange(data: any, correlationId: string) {
  console.log(`[Webhook:ERPNext] invoice_status_change: invoice=${data.name || "unknown"}, status=${data.status || "unknown"}, correlation: ${correlationId}`)
}

async function handlePaymentEntry(data: any, correlationId: string) {
  console.log(`[Webhook:ERPNext] payment_entry: name=${data.name || "unknown"}, amount=${data.paid_amount || 0}, correlation: ${correlationId}`)
}

async function handleStockReconciliation(data: any, correlationId: string) {
  console.log(`[Webhook:ERPNext] stock_reconciliation: name=${data.name || "unknown"}, items=${data.items?.length || 0}, correlation: ${correlationId}`)
}
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    if (secret) {
      ...
      if (!signature) {
        console.log(`[Webhook:ERPNext] Missing signature header (correlation: ${correlationId})`)
      ...
      try {
        if (!verifyHMAC(rawBody, signature, secret)) {
          console.log(`[Webhook:ERPNext] HMAC verification failed (correlation: ${correlationId})`)
          return res.status(400).json({ error: "Invalid signature" })
        }
      } catch {
        console.log(`[Webhook:ERPNext] Signature verification error (correlation: ${correlationId})`)
        return res.status(400).json({ error: "Invalid signature" })
      }
    }
    ...
    console.log(`[Webhook:ERPNext] Received event: ${event} (correlation: ${correlationId})`)
    ...
    switch (event) {
      ...
      case "stock_reconciliation":
        ...
        break
      default:
        console.log(`[Webhook:ERPNext] Unhandled event: ${event} (correlation: ${correlationId})`)
        break
    }
    ...
    try {
      ...
      console.log(`[Webhook:ERPNext] Temporal dispatch skipped: ${err.message}`)
    }
