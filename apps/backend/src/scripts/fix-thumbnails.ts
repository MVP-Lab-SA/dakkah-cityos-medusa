...
function buildReverseMap(): Map<string, { category: string; index: number }> {
  const map = new Map<string, { category: string; index: number }>()
  for (const [category, ids] of Object.entries(IMAGE_CATALOG)) {
    for (let i = 0; i < ids.length; i++) {
      if (!map.has(ids[i])) {
        map.set(ids[i], { category, index: i })
      }
    }
  }
  return map
...
function makeBucketUrl(category: string, index: number, isThumb: boolean): string {
  const suffix = isThumb ? "-thumb" : ""
  const objectPath = `media/products/${category}/${category}-${index}${suffix}.jpg`
...
export default async function fixThumbnails({ container }: ExecArgs) {
  ...
  logger.info("=== Fix Thumbnails: Unsplash → Bucket URLs ===")

  const reverseMap = buildReverseMap()
  logger.info(`Built reverse map with ${reverseMap.size} unique Unsplash IDs`)

  const productRows = await pgConnection.raw(
    `SELECT id, thumbnail FROM product WHERE thumbnail LIKE '%unsplash%'`
  )
  const products = productRows.rows || productRows
  logger.info(`Found ${products.length} products with Unsplash thumbnails`)

  let thumbUpdated = 0
  let thumbSkipped = 0

  for (const product of products) {
    const unsplashId = extractUnsplashId(product.thumbnail)
    if (!unsplashId) {
      thumbSkipped++
      continue
    }

    const mapping = reverseMap.get(unsplashId)
    if (!mapping) {
      thumbSkipped++
      logger.warn(`  No mapping for thumbnail ID: ${unsplashId} (product ${product.id})`)
      continue
    }

    const newUrl = makeBucketUrl(mapping.category, mapping.index, true)
  ...
  logger.info(`Thumbnails: ${thumbUpdated} updated, ${thumbSkipped} skipped`)

  const imageRows = await pgConnection.raw(
    `SELECT id, url FROM image WHERE url LIKE '%unsplash%'`
  )
  const images = imageRows.rows || imageRows
  logger.info(`Found ${images.length} image records with Unsplash URLs`)

  let imgUpdated = 0
  let imgSkipped = 0

  for (const img of images) {
    const unsplashId = extractUnsplashId(img.url)
    if (!unsplashId) {
      imgSkipped++
      continue
    }
    ...
    const newUrl = makeBucketUrl(mapping.category, mapping.index, false)
    await pgConnection.raw(
  ...
  logger.info(`Images: ${imgUpdated} updated, ${imgSkipped} skipped`)
  logger.info(`=== Fix Thumbnails Complete ===`)
  logger.info(`  Product thumbnails: ${thumbUpdated}/${products.length}`)
  logger.info(`  Product images: ${imgUpdated}/${images.length}`)
}
