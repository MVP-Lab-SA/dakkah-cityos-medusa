...
const logger = createLogger("scripts:seed-verticals-5")

export default async function seedVerticals5({ container }: ExecArgs) {
  logger.info("========================================")
  logger.info("Seeding Vertical Modules - Batch 5")
  logger.info("========================================\n")
  ...
  // ============================================================
  logger.info("Step 1/21: Seeding check_in...")
  try {
    ...
    `)
    logger.info("  Created 3 check_in rows")
  } catch (err: any) {
    logger.error(`  check_in error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 2/21: Seeding wellness_plan...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 2 wellness_plan rows")
  } catch (err: any) {
    logger.error(`  wellness_plan error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  try {
    ...
    if (cnt > 0) {
      ...
      await dataSource.raw(`
        ...
        ON CONFLICT (id) DO NOTHING
      `)
      logger.info("  Created 3 class_booking rows")
    }
  } catch (err: any) {
    logger.error(`  class_booking error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 4/21: Seeding freelance_contract...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 3 freelance_contract rows")
  } catch (err: any) {
    logger.error(`  freelance_contract error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 5/21: Seeding freelance_dispute...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 2 freelance_dispute rows")
  } catch (err: any) {
    logger.error(`  freelance_dispute error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }

  // ============================================================
  // 6. CITIZEN_PROFILE (3 rows)
  // ============================================================
  logger.info("Step 6/21: Seeding citizen_profile...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 3 citizen_profile rows")
  } catch (err: any) {
    logger.error(`  citizen_profile error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 7/21: Seeding municipal_license...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 3 municipal_license rows")
  } catch (err: any) {
    logger.error(`  municipal_license error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 8/21: Seeding fine...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 2 fine rows")
  } catch (err: any) {
    logger.error(`  fine error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 9/21: Seeding delivery_slot...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 4 delivery_slot rows")
  } catch (err: any) {
    logger.error(`  delivery_slot error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }

  // ============================================================
  // 10. SUBSTITUTION_RULE (2 rows)
  // ============================================================
  logger.info("Step 10/21: Seeding substitution_rule...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 2 substitution_rule rows")
  } catch (err: any) {
    logger.error(`  substitution_rule error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 11/21: Seeding medical_record...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 3 medical_record rows")
  } catch (err: any) {
    logger.error(`  medical_record error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 12/21: Seeding prescription...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 2 prescription rows")
  } catch (err: any) {
    logger.error(`  prescription error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 13/21: Seeding lab_order...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 2 lab_order rows")
  } catch (err: any) {
    logger.error(`  lab_order error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 14/21: Seeding legal_consultation...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 3 legal_consultation rows")
  } catch (err: any) {
    logger.error(`  legal_consultation error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 15/21: Seeding membership...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 3 membership rows")
  } catch (err: any) {
    logger.error(`  membership error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 16/21: Seeding parking_session...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 3 parking_session rows")
  } catch (err: any) {
    logger.error(`  parking_session error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 17/21: Seeding pet_product...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 3 pet_product rows")
  } catch (err: any) {
    logger.error(`  pet_product error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }

  // ============================================================
  // 18. VET_APPOINTMENT (2 rows)
  // ============================================================
  logger.info("Step 18/21: Seeding vet_appointment...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 2 vet_appointment rows")
  } catch (err: any) {
    logger.error(`  vet_appointment error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }

  // ============================================================
  // 19. LEASE_AGREEMENT (2 rows)
  // ============================================================
  logger.info("Step 19/21: Seeding lease_agreement...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 2 lease_agreement rows")
  } catch (err: any) {
    logger.error(`  lease_agreement error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }

  // ============================================================
  // 20. PROPERTY_VALUATION (2 rows)
  // ============================================================
  logger.info("Step 20/21: Seeding property_valuation...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 2 property_valuation rows")
  } catch (err: any) {
    logger.error(`  property_valuation error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }
  ...
  // ============================================================
  logger.info("Step 21/21: Seeding property_document...")
  try {
    await dataSource.raw(`
      ...
      ON CONFLICT (id) DO NOTHING
    `)
    logger.info("  Created 3 property_document rows")
  } catch (err: any) {
    logger.error(`  property_document error: ${err.detail || err.constraint || err.message?.substring(0, 200)}`)
  }

  logger.info("\n========================================")
  logger.info("Seed Verticals 5 Complete!")
  logger.info("========================================")
}
